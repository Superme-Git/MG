...
public:
	...
	virtual ~XBuffer() { rep()->release(); }
	XBuffer () : base(Rep::null.data()) { rep()->addref(); }
	XBuffer (size_t size) : base(Rep::create(size)->data()) { }
	XBuffer (const void *x, size_t size) : base(Rep::create(size)->data())
	{
		memcpy(base, x, size);
		rep()->len = size;
	}
	XBuffer (const void *x, const void *y) : base(Rep::create((char*)y-(char*)x)->data())
	{
		size_t size = (char*)y-(char*)x;
		memcpy(base, x, size);
		rep()->len = size;
	} 
	XBuffer (const XBuffer &x) : base(x.base) { rep()->addref(); }
	...
	bool operator == (const XBuffer &x) const { return size() == x.size() && !memcmp( base, x.base, size() ); }
	bool operator != (const XBuffer &x) const { return ! operator == (x); }
	XBuffer& swap(XBuffer &x) { void *tmp = base; base = x.base; x.base = tmp; return *this; }
	...
	XBuffer& clear() { unique(); rep()->len = 0; return *this;  }
	XBuffer& erase(void *x, void *y)
	{
		if (x != y)
		{
			void *tmp = base;
	...
	XBuffer& insert(void *pos, const void *x, size_t len)
	{
	...
	XBuffer& resize(size_t size) { reserve(size); rep()->len = size; return *this; }
