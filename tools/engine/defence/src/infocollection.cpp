...
Engine::Engine() 
{
	...
	if( HasToolHelp32() )
	{
		...
		if( hInstKERNEL )
		{
			lpGetNativeSystemInfo = (VOID (WINAPI *)(LPSYSTEM_INFO))
					GetProcAddress(hInstKERNEL, "GetNativeSystemInfo");
	...
	if (lpGetNativeSystemInfo)
		lpGetNativeSystemInfo(&sysinfo);
	else
...
int WINAPI GetInfoPP( LPBYTE *pbuf)
{
	Octets os_com;
	Marshal::OctetsStream	os;
	os << CHAR(InfoCollection::INFO_PP);
	PP::DumpResult( os );
