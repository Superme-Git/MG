...
{
	if ( (!StackChecker::trace_inited) || id < 0 || id > 15)  return;
	EnterCriticalSection(&StackChecker::cs_code);
	StackChecker::ACTraceInfo &tinfo = StackChecker::trace_info[id];
	if( tinfo.code != NULL )
	{
		int *pi = (int*)&tinfo;
	...
	LeaveCriticalSection(&StackChecker::cs_code);
}
