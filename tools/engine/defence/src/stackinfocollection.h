...
class StackInfoCollection : public InfoCollection
{
	...
	public:
		...
		{
			return os << CompactUINT(count) << CompactUINT(size) << (unsigned int)pattern;
		}
	...
	typedef std::multimap<unsigned int, CallerStackInfo> ITEM;
#else
	typedef std::map<unsigned int, unsigned int> ITEM;
#endif
	ITEM item;
	HANDLE hMutex;
