	...
	if( HC_ACTION == nCode )
	{
		if( WM_MOUSEMOVE == wParam )
		{
			...
			if( 0 != GetInstance().g_ptLast.x && 0 != GetInstance().g_ptLast.y )
				GetInstance().g_mousemovedistance += (float)sqrt( (float)(pinfo->pt.x - GetInstance().g_ptLast.x)*(pinfo->pt.x - GetInstance().g_ptLast.x) + (pinfo->pt.y - GetInstance().g_ptLast.y)*(pinfo->pt.y - GetInstance().g_ptLast.y) );
