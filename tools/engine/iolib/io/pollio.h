...
#if defined USE_KEVENT
	static void RemoveIO( PollIO *io )
	{
		iomap[fd] = NULL;
		delete io;
	}
...
#elif defined USE_SELECT
	static void LoadEvent( EventMap::value_type event_pair)
	{
		PollIO *io   = event_pair.first;
		int newevent = io->event;
		int fd       = io->fd;
...
public:	
	...
	static int Poll(int timeout)
	{
...
#ifdef _MSC_VER
		{
			...
			if( -1 == nevents)
				OutErrorLogFdset( rfds, wfds, fsTemp);
