...
protected:
	...
	NetSession(const NetSession &rhs) : assoc_io(rhs.assoc_io), sending(false), closing(false), close_discard(rhs.close_discard),
	...
	bool Output(const Octets &data)
	{
		...
		{
			Log::log(LOG_INFO,"Output:: data overflow MAXIUM buffer size! obuffer=%d(total=%d),osecbuf=%d,data=%d",obuffer.size(),obuffer.capacity(),osecbuf.size(),data.size());
		   	return false;
		}
...
public:
	NetSession() : assoc_io(NULL), sending(false), closing(false), close_discard(false), ibuffer(DEFAULTIOBUF), obuffer(DEFAULTIOBUF), 
	...
	void LoadConfig()
	{
		...
		if ((size = atoi(conf->find(section, "isec").c_str()))) 
		{
			...
			SetISecurity( (Security::Type)size, Octets(&key[0], key.size()));
		}
		if ((size = atoi(conf->find(section, "osec").c_str()))) 
		{
			Conf::value_type key = conf->find(section, "oseckey");
			SetOSecurity( (Security::Type)size, Octets(&key[0], key.size()));
		}
	}
...
class DgramClientIO : public NetIO
{
	...
	void PollOut()
	{
		...
		session->OnSend();
		Octets& obuf = session->GetOBuffer();
		sendto(fd, (const char*)obuf.begin(), (int)obuf.size(), 0, peer, peer.GetLen());
		obuf.clear();
		if ( session->closing )
			Close();
		else
...
class DgramServerIO : public NetIO
{
	...
	void PollIn()
	{
		...
		do
		{
			...
			while ((recv_bytes = recvfrom(fd, (char*)ibuf.begin(), (int)ibuf.capacity(), 0, (struct sockaddr*)&peer, &peer_len)) > 0)
			{
				Thread::Mutex::Scoped l(session->locker);
	...
	void PollOut()
	{
		...
		for (Map::iterator it = map.begin(), ie = map.end(); it != ie; ++it)
		{
			...
			if (obuf.size())
			{
				sendto(fd, (const char*)obuf.begin(), (int)obuf.size(), 0, sa, sa.GetLen());
				obuf.clear();
			}
			if (session->closing)
			{
