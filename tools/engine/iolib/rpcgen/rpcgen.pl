...
sub generate_protocol_inl
{
	...
	print_tracemethods($file, $vars, $name);
	print_protocol_methods($file, $protocol->getAttribute('maxsize'), $protocol->getAttribute('prior'));
}
...
sub print_protocol_constructor
{
	...
	if ($count_default == $n)
        {       print "\t\tError: all parameters have default value in $classname\n"; }	
...
EOF
	...
	Session::ID GetSessionID() { return sid; }
};
...
PProtocolProcess	g_pProtocolProcess = NULL;
PNetEventNotify 	g_pNetEventNotify = NULL;
...
EOF
		...
		$file_cpp->print(<<EOF);
	if( g_pNetEventNotify)
		g_pNetEventNotify( NEC_CONNECTED);
	//TODO
}
...
EOF
		...
		$file_cpp->print(<<EOF);
	if( g_pNetEventNotify)
		g_pNetEventNotify( NEC_DISCONNECTED);
	//TODO
}
...
EOF
		...
		$file_cpp->print(<<EOF);
	if( g_pNetEventNotify)
		g_pNetEventNotify( NEC_CONNECT_FAILED);
	//TODO
}
...
class $name : public GNET::Protocol
{
	...
	{
		if( g_pProtocolProcess)
			g_pProtocolProcess( this);
		// TODO
	}
};
...
class Protocol;
typedef void (*PProtocolProcess)( Protocol* p);
extern PProtocolProcess	g_pProtocolProcess;
...
typedef void (*PNetEventNotify)( NETEVENTCODE nec);
extern PNetEventNotify g_pNetEventNotify;
