...
public:
	...
	{
		count_read++;
		if ( pread( fd, data, PAGESIZE, idx * PAGESIZE ) != PAGESIZE )
			throw Exception();
	}
	...
	{
		count_write++;
		if ( pwrite( fd, data, PAGESIZE, idx * PAGESIZE ) != PAGESIZE )
			throw Exception();
	}
...
class Page
{
	...
	ptrdiff_t	snapshot;
	enum { CLEAN = 0, DIRTY = 1, SNAPSHOT = 2 };

	static void _sort_lru( Page **it, Page **ie )
	{
		for ( size_t i = ie - it; --i; )
			for ( size_t j = i; j--; )
				if ( it[i]->lru < it[j]->lru )
					std::swap( it[i], it[j] );
	}
public:
	static void sort_lru( Page **it, Page **ie )
	{
		Page **bgn = it, **end = ie - 1;
		for( lru_t lru = it[rand() % (ie - it)]->lru; bgn < end; std::swap(*bgn, *end) )
		{
			while ( (*bgn)->lru < lru ) bgn ++;
			while ( (*end)->lru > lru ) end --;
		}
		if ( bgn - it > 8 ) sort_lru( it, bgn );     else if ( bgn - it > 1 ) _sort_lru( it, bgn );
		if ( ie - bgn > 9 ) sort_lru( bgn + 1, ie ); else if ( ie - bgn > 2 ) _sort_lru( bgn + 1, ie );
	}
	...
	bool clr_snapshot()
	{
		if ( snapshot & DIRTY )
		{
			...
			snapshot = (ptrdiff_t)layout | DIRTY;
			return true;
		}
		snapshot &= PAGEMASK;
		return false;
	...
	Page* set_dirty()
	{
		...
		{
			snapshot &= ~SNAPSHOT;
			memcpy( layout = (PageLayout *)PageMemory::alloc(), (void *)snapshot, PAGESIZE );
		}
		snapshot |= DIRTY;
		return this;
	}
	template<typename T>
	void set_dirty(T* &pointer)
	{
		if ( snapshot & SNAPSHOT )
		{
			snapshot &= ~SNAPSHOT;
			memcpy( layout = (PageLayout *)PageMemory::alloc(), (void *)snapshot, PAGESIZE );
			pointer = (T*)((byte_t *)pointer + ((ptrdiff_t)layout - snapshot));
		}
		snapshot |= DIRTY;
	}
	...
	void snapshot_write( PageLayout *pl ) const 
	{
		memcpy( pl, (void *)(snapshot & PAGEMASK), PAGESIZE );
		*(page_index_t *)(pl + 1) = idx;
	}
...
public:
	...
	{
		int pos;
		return find_key( key, key_len, pos ); 
	}
...
public:
	...
	void* put_replace( index_hdr* found, const void *key, size_t key_len, const void *val, size_t& save_val_len, bool need_origin )
	{
		...
		{
			Layout2Page(found)->set_dirty(found);
			free_data(hdr);
...
class Logger
{
	static void save_page( const Page *page, PageFile *log_file, page_index_t page_index, PageLayout *layout )
	{
		page->snapshot_write(layout);
...
public:
	...
	static void save_page( Iterator it, Iterator ie, PageFile *log_file )
	{
		...
		for ( ; it != ie; ++it )
			save_page( *it, log_file, (*it)->index(), layout );
	...
	static page_index_t save_page( Iterator it, Iterator ie, PageFile *log_file, page_index_t page_index )
	{
		PageLayout *layout = (PageLayout *)PageMemory::alloc();
		for ( ; it != ie; ++it )
			save_page( *it, log_file, page_index++, layout );
		PageMemory::free(layout);
		return page_index;
	}
...
class GlobalLogger : public Logger
{
	...
	public:
		...
		void commit( PageFile *log_file, time_t timestamp, page_index_t rec_index )
		{
			PageLayout *layout = (PageLayout *)PageMemory::alloc();
			...
			if ( it > ie )
			{
				...
				layout->logger_id = page_cache->magic_layout_ptr()->logger_id;
			}
			else
			{
		...
		size_t init( PageFile *log_file, page_index_t page_index, time_t logger_id )
		{
			...
			*lp = (len + sizeof(size32_t) + 15) & ~15;
			...
			page_cache->magic_layout_ptr()->logger_id = logger_id;
			return *lp;
		}
	...
	void init_log_file( time_t new_time_stamp )
	{
		...
		for ( LogInfoVec::iterator it = loginfo_vec.begin(), ie = loginfo_vec.end(); it != ie; ++it )
			used_max = std::max( used_max, (*it)->init( log_file, page_index++, time_stamp ) );
	...
	State restore( time_t timestamp, page_index_t rec_index )
	{
		...
		{
			for ( LogInfoVec::iterator it = loginfo_vec.begin(), ie = loginfo_vec.end(); it != ie; ++it )
				(*it)->commit( log_file, timestamp, rec_index );
			return CLEAN;
		}
		catch ( PageFile::Exception e ) { }
		return CORRUPT;
...
public:
	...
	State redo( time_t timestamp )
	{
		try
		{
			...
			for ( LogInfoVec::iterator it = loginfo_vec.begin(), ie = loginfo_vec.end(); it != ie; ++it )
				if ( (*it)->logger_id() )
					(*it)->commit( log_file, time_stamp, rec_index );
...
inline void free_data ( data_hdr *hdr )
{
	...
	do
	{
		...
		page_index_t next_page_index = hdr->next_page_index;
		if ( hdr != cur )
		{
...
public:
	...
	DBCollection(size_t pages = 1024) : datadir(NULL), logdir(NULL), logpages(pages), logger(NULL) { }
	...
	{
		std::vector<time_t> r;
		time_t time_stamp = logger->get_time_stamp();
		if ( DIR *dir = opendir( logdir ) )
		{
			while ( dirent *d = readdir( dir ) )
				if ( d->d_name[0] != '.' )
