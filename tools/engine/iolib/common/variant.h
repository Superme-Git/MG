...
#ifdef _MSC_VER
//		if ( sscanf( v.c_str(), "%I64d", &value ) != 1 ) 
		if ( sscanf_s( v.c_str(), "%I64d", &value, sizeof( long long) ) != 1 ) 
#else
		if ( sscanf( v.c_str(), "%lld", &value ) != 1 ) 
#endif
			throw Exception();
	}
	void Encode(Octets &os) const
	{
		char buf[32];
#ifdef _MSC_VER
		sprintf_s( buf, sizeof( buf), "i%llde", value);
#else
	...
	{ 
#ifdef _MSC_VER
		TABPRINT(level, "(i) %I64d\n", value); 
#else
		TABPRINT(level, "(i) %lld\n", value); 
#endif
	}
};
...
#ifdef _MSC_VER
		sprintf_s(buf, sizeof( buf), "%d:", size);
#else
		sprintf(buf, "%d:", size);
#endif
		os.insert( os.end(), buf, buf + strlen(buf) );
