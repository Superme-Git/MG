...
#elif defined _MSC_VER && defined _USE_REAL_LOCKER
...
namespace GNET
{
	...
	namespace Thread
	{
		...
		public:
			...
			explicit Mutex(const char* id,bool recursive=false) { InitializeCriticalSection( &m_cs); }
			void Lock()   
			{  
				EnterCriticalSection( &m_cs); 
			}
			void UNLock() {  LeaveCriticalSection( &m_cs); }
			bool TryLock() { return TRUE == TryEnterCriticalSection( &m_cs); }
			class Scoped 
			{ 
			public:
				~Scoped() {  _m.UNLock(); } 
				explicit Scoped(Mutex& m) : _m( m) {  _m.Lock(); } 
			private:
				Mutex& _m;
			};
		private:
		...
		{
		public:
			~RWLock() { DeleteCriticalSection( &m_cs); }
			void WRLock() { EnterCriticalSection( &m_cs); }
			void RDLock() { EnterCriticalSection( &m_cs); }
			void UNLock() { LeaveCriticalSection( &m_cs); }
			RWLock() { InitializeCriticalSection( &m_cs); }
			RWLock(const char* id) { InitializeCriticalSection( &m_cs); }
			class WRScoped 
			{ 
			public:
				~WRScoped() { rw.UNLock(); } 
				WRScoped(RWLock &l) : rw( l) { rw.WRLock(); } 
			private:
				RWLock&	rw;
			};
			class RDScoped 
			...
			public:
				~RDScoped() { rw.UNLock(); } 
				RDScoped( RWLock &l) : rw( l) { rw.RDLock(); } 
			private:
				RWLock&	rw;
			};
		private:
		...
		public:
			...
			{ 
				...
				impl.enter_wait();
				mutex.UNLock();
				ret = impl.do_timed_wait( nseconds) ? 0 : -1;
				mutex.Lock();
				return ret; 
			}
			int NotifyOne( ) 
