...
namespace Thread
{
	...
	public:
		...
		{ 
			if( m_hPoolThread)
			{
				TerminateThread( m_hPoolThread, -1);
				CloseHandle( m_hPoolThread);
				m_hPoolThread = NULL;
			}
		}
#else
...
#ifdef _MSC_VER
#	ifdef _REAL_MULTI_THREAD
			m_hPoolThread = (HANDLE)_beginthread( (void (__cdecl *)(void *))&Pool::RunThread, 0, NULL);
#	else
			Pool::RunThread( NULL );
#	endif
#else
