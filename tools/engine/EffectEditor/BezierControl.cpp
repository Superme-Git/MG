...
namespace bezier {
	...
	IMPLEMENT_DYNAMIC(CBezierControl, CDialog)
		...
		BEGIN_MESSAGE_MAP(CBezierControl, CDialog)
			...
			ON_CBN_SELENDOK(IDC_BZ_COMBO_FILTER, &CBezierControl::OnCbnSelendokBzComboFilter)
		END_MESSAGE_MAP()


		void CBezierControl::OnCbnSelendokBzComboFilter()
	{
	...
	void CBezierControl::ResetFilter()
	{
		...
		m_ComboFilter.SetCurSel(static_cast<int>(m_TPCTangentPoints.size()));
	...
	void CBezierControl::DrawSamplePoint(CDC* pDC)
	{
		...
		{
			if (m_SPCSamplePoints.size() > lineID)	
			{
