...
// Implementation
	typedef enum {
		MODE_READONLY = 0x00000001,
		MODE_DIGITAL_EDIT = 0x00000002,
		MODE_COMBO = 0x00000004,
		MODE_CHECKBOX = 0x00000008,
	...
	void SetCheckBoxColumns(int iColumnIndex, bool bSet = true);
	...
	// tigger OnBeginLabelEdit
	HWND BeginEditLabel( int iRowIndex, int iColumnIndex );

	// Returns the row & column index of the column on which mouse click event has occured
	bool SubItemHitTestEx(CPoint& rHitPoint, int* pRowIndex, int* pColumnIndex);

// Overrides
	// ClassWizard generated virtual function overrides
	//{{AFX_VIRTUAL(CComboListCtrl)
	//}}AFX_VIRTUAL
	void HideAllInPlaceCtrl();
...
// Implementation
	...
	CInPlaceCheckBox* ShowInPlaceCheckBox(int iRowIndex, int iColumnIndex, CString& strCurSelecetion);
	...
	bool IsCheckBox( int iColumnIndex);
...
// Attributes
	...
	CList<int, int>	m_CheckBoxColumnsList;
