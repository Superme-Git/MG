...
static void get_1555bit_row ( MEMFILE* mf, int iWidth, pixel32* pPixel )
{
	...
	for (col = iWidth; col > 0; col--)
	{
		...
		*/
		pPixel->blue = (unsigned char) c5to8bits[t & 0x1F]; // B
		t >>= 5;
		pPixel->green = (unsigned char) c5to8bits[t & 0x1F];// G
		t >>= 5;
		pPixel->red = (unsigned char) c5to8bits[t & 0x1F]; // R
		t >>= 5;
...
BITMAPINFO* texReadB0X( LPCTSTR fileName, HBITMAP* pBitmap, char** ppPixels)
{
	...
	// 格式校验
	if ( phead.format < 0 || phead.format > PIX_FMT_X1R5G5B5 || phead.format == PIX_FMT_A8 )
	{
	...
	long bufferSize = phead.width * phead.height * sizeof(pixel32);
	...
	*pBitmap = qt_hBITMAP;
	...
	for ( int i = 0; i < phead.blockNum; ++i)
	{
		...
		char* blockPtr = bitmapBuffer + (bufferHeight-desc.offsetY-1)* picLineSize + desc.offsetX * bypp;
		for(long y = 0; y < desc.imageHeight; y++)
		{
			pixel32* pPixel = ((pixel32*)blockPtr) + (-y) * bufferWidth;
...
BITMAPINFO* texReadPIC2( LPCTSTR filename, HBITMAP* pBitmap, char** ppPixels)
{
...

	{
		...
		*pBitmap = qt_hBITMAP;
		...
		pEOB = ((pixel32*)qt_pPixels) + bufferWidth * bufferHeight;
		...
		for(y = 0; y < bufferHeight; y++)
		{
			for(c = 0; c < i + 1; c++)
			{
				...
				pPixel = ((pixel32*)qt_pPixels) + (bufferHeight - y - 1) * bufferWidth;
...
#if 0
	{
		...
		{
			pixel32* pPixel = ((pixel32*)qt_pPixels) + yy * qt_pBitmapInfo->bmiHeader.biWidth;
