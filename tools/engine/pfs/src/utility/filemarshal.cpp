...
CFileStream::CFileStream( PFS::CBaseFile& file) : m_file( file) { }
CFileStream::~CFileStream() {}

fsize_t CFileStream::Seek( foff_t pos, PFS::FILE_SEEK_METHOD fsm) const
{
...
void CFileStream::pop_byte( void* x, size_t len) const
{
	...
	if( PFS::FILE_SIZE_ERROR == valid || valid < len)
		throw CFileStreamException();
}
...
size_t CFileStream::pop_size() const
{
	...
	if( PFS::FILE_SIZE_ERROR == valid || valid < 1 )
		throw CFileStreamException();
	switch ( x & 0xf0 )
	{
