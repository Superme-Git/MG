...
CLogScoped::CLogScoped( const wchar_t* funcname, const wchar_t* filename, PFS::CImageData& data) : m_funcname( funcname), m_filename( filename), m_data( data)
{
	m_outlog = PFS::CEnv::GetOutLogInstance();
	m_dwTimeStart = timeGetTime();
}

CLogScoped::CLogScoped( const wchar_t* funcname, const wchar_t* filename) : m_funcname( funcname), m_filename( filename), m_data( PFS::CImageData())
{
	m_outlog = PFS::CEnv::GetOutLogInstance();
	m_dwTimeStart = timeGetTime();
}
