...
public:
	...
	// 返回out的大小
	//
	size_t Inflate( const void* in, size_t insize, PFS::CDataBlock& out)
	{
		out.Clear();
	...
	// 返回out的大小
	//
	size_t Inflate( const PFS::CDataBlock& in, PFS::CDataBlock& out)
	{
...
public:
	...
	size_t Deflate( const PFS::CDataBlock& in, PFS::CDataBlock& out)
	{
		return Deflate( in.GetData(), in.GetSize(), out);
	}
	size_t Deflate( const void* in, size_t insize, PFS::CDataBlock& out)
	{
		out.Clear();
		...
		// BUGFIX: 
		// 已经在PackTool.exe中证实，当编辑skillicon.pak后保存时，导致程序僵死。
		// 最后跟踪到这里。分析如下：
		// 当insize为2，m_strm.next_in[0] == 0xFF, m_strm.next_in[1] == 0xFF时，
		// buffersize 仅仅为4,下面的循环无法退出。 
		// 针对这样的情况，放大buffersize到16临时解决之。
		// 关于buffersize的最小值问题，待进一步研究并确定。
		// 刘东学 2010-08-04
		if( buffersize < 16 )
			buffersize = 16;
...
public:
	...
	void Update( const PFS::CDataBlock& in)
	{
