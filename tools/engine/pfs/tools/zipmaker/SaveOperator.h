...
class CSaveOperator : public CNeedProjectOperator
{
	...
	std::wstring				m_file;
	bool						m_bResource;
	bool						m_bSaveFilesMeta; // 是否生成.files.meta
...
public:
	CSaveOperator( CZipTreeItem& parent, CProject& project, const std::wstring file, 
		bool bResource, bool bSaveFilesMeta, const std::wstring& lastDataPfs)
		: CNeedProjectOperator( parent, project), m_file( file), 
		m_bResource( bResource), 
		m_bSaveFilesMeta( bSaveFilesMeta),
		m_lastDataPfs( lastDataPfs)
	{ 
