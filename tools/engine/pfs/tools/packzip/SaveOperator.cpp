...
bool CSaveOperator::SaveOneFileData( CZipTreeItem& item, fsize_t& sizeSrc, fsize_t& sizeDes, PFS::CMetaInfo& meta)
{
	...
	if( m_bResource)
	{
		if( item.GetZipLevel() == 0)
			fc.m_usMethod = 0;
		m_writer->SetZipLevel( item.GetZipLevel());
	}
	else
	{
		fc.m_usMethod = 0;
		m_writer->SetZipLevel(0);
		//fc.m_usMethod = item.GetAttributes();
		//m_writer->SetZipLevel(item.GetZipLevel());
	}
	if( !m_writer->WriteNextCentral( fc))
	{
		return false;
	}
...
void CSaveOperator::SaveToZip( PFS::CBaseFile& file)
{
	...
	while( true)
	{
		...
		m_writer->WriteFileData( m_buffer, bFinish);
		if( bFinish)
		{
			break;
		}
...
bool CSaveOperator::MergeChildPackage( CZipTreeItem& item, fsize_t& size)
{
	...
	fcRoot.m_name = name;
	m_writer->WriteNextCentral( fcRoot);
		...
		else
		{
			...
			if( iFind != tempMetaInfos.end())
			{
				...
				if( m_writer->WriteNextCentral( fc))
				{
					...
					{
						m_writer->WriteDeflateData( m_buffer, false);
					}
...
{
	m_writer->SetZipLevel( CGlobalInfo::NO);
	if( m_listener)
...
bool CSaveOperator::MakeMD5()
{
	...
	if( !m_writer->MakeOutZipFileMD5( result))
	{
		if( m_listener)
		{
	...
	std::wstring md5file = m_writer->GetOutputFileName();
	md5file += L".md5";
	PFSX::CNativeFile nf;
...
bool CSaveOperator::SaveDataToZip( const std::wstring& filename, PFS::CDataBlock& data)
{
	...
	if( m_writer->WriteNextCentral( fc))
	{
		return m_writer->WriteFileData( data, true);
	}
