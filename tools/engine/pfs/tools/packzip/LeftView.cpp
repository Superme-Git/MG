...
bool CLeftView::AddPath( const std::wstring& path)
{
	...
	COperator::COperatorVector ov;
	CAddOperator* adder = new CAddPathOperator( *item, *m_project, path);
	ov.push_back( adder);
...
BOOL CLeftView::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	default:
		{
			int nLevel = wParam - IDR_FILECOMPRESSION;
			if( (nLevel >= 0) && (nLevel <= CGlobalInfo::DecodeCount))
...
bool CLeftView::SavePath( const std::wstring& rootpath)
{
	CSaveOperator* saver = new CSaveOperator( m_root, *m_project, rootpath);
	COperator::COperatorVector ov;
	ov.push_back( saver);
	...
	wss << std::endl;
	wss << L"类    型:  更新目录" << std::endl;;
	wss << L"保存路径:  " << rootpath << std::endl;
	wss << L"统计信息:  " << std::endl;
...
bool CLeftView::SaveFile(const std::wstring& file, bool bResource, const std::wstring& lastDataPfs)
{
	CSaveOperator* saver = new CSaveOperator( m_root, *m_project, file, bResource, lastDataPfs);
	COperator::COperatorVector ov;
	ov.push_back( saver);
