...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_COMMAND(ID_FILE_OUTPUT_DIR, &CMainFrame::OnFileOutputDir)
END_MESSAGE_MAP()
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if( cmdInfo.m_nShellCommand == CCommandLineInfo::FileOpen )
	{
		...
		{
			AfxMessageBox( L"不是有效的pkproj格式文件！");
		}
	}
...
void CMainFrame::SaveFile( bool bResource, const std::wstring& lastDataPfs)
{
	std::wstring pfsname;
	if( lastDataPfs.empty())
	{
		...
		{
			size_t pos ;
			if ( (pos = m_projectfilename.find( L"release.pkproj" ) ) != m_projectfilename.npos )
			{
				pfsname.replace( pos, pfsname.npos, L"data.pfs" );
			...
			{
				pfsname.replace( pos, pfsname.npos, L".pfs" );
			}
		}
	}
	else
	{
		pfsname = L"data_update.pfs";	// 临时更新包的名称
		// 临时更新包提醒编辑版本信息
		OnEditVersionmeta();
	}
	
	wchar_t szFilter[] = L"pfs file|*.pfs|all files|*.*|";
	CFileDialog dlgFile( FALSE, L"pfs", pfsname.c_str(), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter);
	...
	CLeftView* pLeftView = (CLeftView*)m_wndSplitter.GetPane(0, 0);
	if( !pLeftView->SaveFile( strFile.GetString(), bResource, lastDataPfs))
	{
	}
}
...
{
	OnFileSave();
	wchar_t szFilter[] = L"pkproj file|*.pkproj|all files|*.*|";
	CFileDialog dlgFile( TRUE, L"pkproj", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter);
	if( IDCANCEL == dlgFile.DoModal())
		return;
...
void CMainFrame::OnFileSave()
{
	...
	{
		wchar_t szFilter[] = L"pkproj file|*.pkproj|all files|*.*|";
		CFileDialog dlgFile( FALSE, L"pkproj", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter);
		if( IDCANCEL == dlgFile.DoModal())
			return;
...
void CMainFrame::OnFileOutputDir()
{
	CBrowsePath browsepath( m_hWnd);
	browsepath.SetIniPath( m_lastsavepath);
	if( browsepath.DoModal())
	{
		OnEditVersionmeta();
		m_lastsavepath = browsepath.GetSelectPath();
		CLeftView* pLeftView = (CLeftView*)m_wndSplitter.GetPane(0, 0);
		pLeftView->SavePath( m_lastsavepath);
	}
}
