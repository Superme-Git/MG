...
namespace PFS
{
	static const DWORD MAX_RAW_HEADER_BUFFER_LENGTH = 1024;

	class CInetIOException
	{
	protected:
		DWORD m_dwErrorCode;
		DWORD m_dwErrorCodeEx;

		TCHAR m_szDetail[MAX_RAW_HEADER_BUFFER_LENGTH + 1];
	...
	public:
		CInetIOException( DWORD dwErrorCode )
			...
			, m_dwErrorCodeEx( 0 )
		{
			ZeroMemory( m_szDetail, sizeof(m_szDetail) );
		}

		CInetIOException( DWORD dwErrorCode, DWORD dwErrorCodeEx, LPCTSTR lpszDetail )
			: m_dwErrorCode( dwErrorCode )
			, m_dwErrorCodeEx( dwErrorCodeEx )
		{
			if( lpszDetail != NULL )
				lstrcpyn( m_szDetail, lpszDetail, MAX_RAW_HEADER_BUFFER_LENGTH ) ;
			else
				ZeroMemory( m_szDetail, sizeof(m_szDetail) );
		}
		...
		// Available when GetErrorCode() returns ERROR_INTERNET_EXTENDED_ERROR
		DWORD GetErrorCodeEx() const
		{
			_ASSERT( m_dwErrorCode == ERROR_INTERNET_EXTENDED_ERROR );
			return m_dwErrorCodeEx;
		}

		LPCTSTR GetDetail() const
		{
			return m_szDetail;
		}
	};
