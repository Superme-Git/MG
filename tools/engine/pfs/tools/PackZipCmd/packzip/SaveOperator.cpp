...
void CSaveOperator::SaveAllFile( CZipTreeItem& item)
{
	...
	CZipTreeItem::CZipTreeItemList::size_type count = map.size(), cur = 0;
...
public:
	wstring errinfo;
	CSaveOperatorError(){}
	CSaveOperatorError(const wstring err):errinfo(err){}
	~CSaveOperatorError(){}
};
void CSaveOperator::SaveOneFile( CZipTreeItem& item)
{
	...
	catch( CSaveOperatorError& ex)
	{
		if( m_listener )
		{
			...
			if(!ex.errinfo.empty())
				strWarning.append(L" - " + ex.errinfo);
			m_listener->OnNotify( (WPARAM)strWarning.c_str(), COperator::PT_FILEERROR);
		}
...
bool CSaveOperator::MergeChildPackage( CZipTreeItem& item, fsize_t& size)
{
	...
	if( !file.Open( item.GetMount(), PFS::FM_EXCL, PFS::FA_RDONLY))
	{
		throw CSaveOperatorError(L"无法打开文件！");
	}
	size = file.GetSize();
	...
	{
		throw CSaveOperatorError(L"文件格式不正确！");
	}
	...
	for( PFSX::SetupMetaVector::iterator i=paths.begin(), e=paths.end(); i!=e; i++)
	{
		...
		{
			throw CSaveOperatorError(L"读取子包中的文件失败: "+ filename);
		}
		...
		{
			throw CSaveOperatorError(L"无法读取文件内容: "+ filename);
		}
		if( tempName.empty())
		{
	...
	for( ZIPFILE::CFileReader::CFileCentralMap::iterator itor = map.begin(); itor != map.end(); ++itor)
	{
		if( COperator::m_bStop)
		{
			throw CSaveOperatorError(L"线程被终止！");
		}
