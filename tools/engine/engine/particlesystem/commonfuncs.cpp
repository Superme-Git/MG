...
namespace Nuclear
{
	...
	float PointToLineMinDis(const CPOINT& pt, const LINE& line)
	{
		//点pt到线段line的最短距离
		POINT pta, ptb, ptc;
		pta.x = line.pt0.x;
	...
	bool PtInLine(const CPOINT& pt, const LINE& line) 
	{
	...
	//	return (pt.x - line.pt0.x)*(line.pt1.y - line.pt0.y) == (pt.y - line.pt0.y)*(line.pt1.x - line.pt0.x);
		//数学上的斜率相等
	...
	****************************************************************************/
	...
	void PluckFirstField(LPWSTR str, LPWSTR dest, int maxlen, LPCWSTR delim)					
	{
		...
		if( !wcslen(delim) )
		{
			wcscpy_s(dest, maxlen/sizeof(WCHAR), str);
			return;
		}
		...
		memcpy(dest, str, i*sizeof(WCHAR));

		wcscpy_s( str, maxlen/sizeof(WCHAR), &str[i+wcslen(delim)] );
	}
	...
	void WriteBlendModeProperty(FILE *file, LPCWSTR strName, int value)
	{
		for(int i=0; i<NUMBLENDINGMODES; i++)
		{
			...
			{
				_ftprintf(file, L"%s = %s\n", strName, g_BlendingModes[i].name);
				return;
			}
		}
		fclose(file);
		XPTRACE(L"WriteBlendModeProperty()--没有找到正确的参数\n");
	}

	void WriteProperty(FILE *file, LPCWSTR strName, PATHPOINT value)
	{
		TCHAR buf[41]; //5字符 * 8参数
		TCHAR cell[6];
		int index = 0;
		int i=0;
		memset(buf, 0, 41*sizeof(TCHAR));
		memset(cell, 0, 6*sizeof(TCHAR));

		_itow_s(value.pt.x, cell, 6, 10);
		ResetCell5Byte(cell);
		memcpy(buf+5*index, cell, 5*sizeof(TCHAR));
		index++;

		_itow_s(value.pt.y, cell, 6, 10);
		ResetCell5Byte(cell);
		memcpy(buf+5*index, cell, 5*sizeof(TCHAR));
		index++;

		_itow_s(value.m_bCurve, cell, 6, 10);
		ResetCell5Byte(cell);
		memcpy(buf+5*index, cell, 5*sizeof(TCHAR));
		index++;

		_itow_s(value.m_bLink, cell, 6, 10);
		ResetCell5Byte(cell);
		memcpy(buf+5*index, cell, 5*sizeof(TCHAR));
		index++;

		_itow_s(value.pre_ControlPt.x, cell, 6, 10);
		ResetCell5Byte(cell);
		memcpy(buf+5*index, cell, 5*sizeof(TCHAR));
		index++;

		_itow_s(value.pre_ControlPt.y, cell, 6, 10);
		ResetCell5Byte(cell);
		memcpy(buf+5*index, cell, 5*sizeof(TCHAR));
		index++;

		_itow_s(value.next_ControlPt.x, cell, 6, 10);
		ResetCell5Byte(cell);
		memcpy(buf+5*index, cell, 5*sizeof(TCHAR));
		index++;

		_itow_s(value.next_ControlPt.y, cell, 6, 10);
		ResetCell5Byte(cell);
		memcpy(buf+5*index, cell, 5*sizeof(TCHAR));
		index++;
		...
		LPCWSTR strName, LPWSTR dest, int destsize)
	{
		...
		// 		errorstr = std::wstring(L"Missing property \"") + std::wstring(strName) + std::wstring(L"\"!");
		// 		throw(errorstr);
		// 	}
		// 	wcsncpy_s(dest, destsize/sizeof(TCHAR), valuemap[strName].c_str(), destsize/sizeof(TCHAR));
...
//		else
//			XPTRACE(L"GetRightSideFromValueMap()--在粒子中有参数没有找到，已用默认值代替\n");
		return false;
	}
	...
	void ReadBlendModeProperty(std::map<std::wstring, std::wstring> &valuemap, LPCWSTR strName, int &value)
	{
		...
		for(int i=0; i<NUMBLENDINGMODES; i++)
		{
			if( !_wcsicmp(g_BlendingModes[i].name, buf) )
			{
	...
	void ReadProperty(std::map<std::wstring, std::wstring> &valuemap, LPCWSTR strName, PATHPOINT &value)
	{
		...
		memset(cell, 0, 6*sizeof(TCHAR));
		int index = 0;
		memcpy(cell, buf+5*index, 5*sizeof(TCHAR));
	...
	std::string ws2s(std::wstring WStr)
	{
		...
		std::string Result(szBuffer, nCount - 1);
		delete[] szBuffer;
		return Result;
	}
	std::wstring s2ws(std::string Str)
	{
		...
		MultiByteToWideChar(CP_ACP, 0, (LPCSTR)Str.c_str(), nCount, (LPWSTR)Result.c_str(), nCount);
		return Result;
	}
}