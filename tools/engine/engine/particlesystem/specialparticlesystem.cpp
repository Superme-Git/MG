...
namespace Nuclear
{
	...
	//	assert(m_ppsl->m_nAbsrtactPsl == 1);
		...
		//第一帧就产生出所有的粒子...然后等都死完了之后再产生出所有的粒子...
	//	m_ppsl->m_nSysMaxParticles = (int)m_shapeList.size(); //设置shapeList的时候就赋值好了..
		...
		for(int q=0; q<iNumNewParts && cnt<m_ppsl->m_nSysMaxParticles; q++)
		{
			...
			else
			{
				...
				if(nNum == 2) //todo; 应该在psl里增加一个参数来存储这种特殊的效果类型
				{
					if(m_bDir)
					{
						m_bDir = false;
		...
		for(int q=0; q<m_particleArray.GetTotalElements(); q++)
		{
			if( m_particleArray.IsAlive(q) )
			{
				...
				if( !part->Update(fdeltatime) )
				{
					...
					if(m_psParameter.cycle && (m_ppsl->m_nEmitterType == 1) && fabs(m_psParameter.m_psModifyParam.m_fEmitterSpinAngle)<XP_EPSILON ) //循环并且是连续发射的粒子才能复活..
					...
					{//全屏效果...
						...
						{ //改为对称移动效果会更好..
							...
							if(part->m_vPos.y > m_curViewPort.Height())
							{
								...
								//2009-06-17 导致粒子永远死不了...全屏特效的自动循环bug...暂时不用修改。
		...
		for(int q=0; q<m_particleArray.GetTotalElements(); q++) //NUMPARTICLES
		{
			...
			{	
				...
				if( (particlePosy > m_curViewPort.Height() +64) || (particlePosy < -256) 
					...
					continue;
				//计算该粒子的纹理坐标
				//	float fangleuv = part->m_fAngle; //自转角度..
			...
			//	int todo;//现在是一个一个粒子的绘制,可以改为一次绘制所有粒子,并且vertices预先分配好
	...
	bool CSpecialParticleSystem::SetShapePsTexture(PictureHandle pShapesTexture, float fu, float fv)
	{
		...
		//	SAFE_RELEASE(m_pD3dTextureShapes);//梦诛每帧设置...
	...
	{
		//暂时为空..以后如果这种特殊特效也改为批绘制了，并且还允许修改顶点格式再补充。
	}
