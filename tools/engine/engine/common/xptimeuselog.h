...
namespace Nuclear
{
	...
	public:
		...
		~XPTimeUseLog() 
		{
			...
			fwprintf(f, L"Begin......\n");
			fwprintf(f, L"SoundMan, EffectMan, AppTick, WorldTick, DelEffect, EngineEffectUpdate, AppBeforeRender, EngBeforeRender, WorldUpdateEffect, PreSort, ");
			fwprintf(f, L"FullSort, LocalSort, MovingBG, RenderCacheRenderTarget, RenderLowLayer, RenderBeforeMin, RenderMid, RenderAfterMid, RenderHeight, RenderMask, ");
			fwprintf(f, L"REnderWorldEnd, RenderUI, RenderBugInfo, RenderEnd, AniManGC, SoundManGC, ProcessMsg, MainLoopRunTask, ExecutorUpdate, FileIOUpdate, MsgBeginTask, ");
	...
	public:
		enum TUL_TYPES {
			...
			TUL_APPBEFORERENDER,
			TUL_ENGBEFORERENDER,//8
			TUL_WORLD_UPDATEEFFECT,//9
			TUL_PERPSORT,//×¼±¸ÅÅÐò 10
			TUL_FULLSORT,//11
			TUL_LOCALSORT,//12
			TUL_MOVINGBACKGROUND,//13
			TUL_RENDERCACHERENDERTARGET, //14
			TUL_RENDERLOWLAYER,	//15
			TUL_RENDERBEFOREMID,	//16
			TUL_RENDERMID,	//17
			TUL_RENDERAFTERMID, //18
			TUL_RENDERHIGHT,	//19
			TUL_RENDERMASK,	//20
			TUL_RENDERWORLDEND,	//21
			TUL_RENDERUI,//22
			TUL_RENDERDEBUGINFO,//23
			TUL_RENDEREND,//24
			TUL_ANIMANAGERGC, //25
			TUL_SOUNDMANAGERGC,
			TUL_PROCESSMESSAGE,
	...
	private:
		void LogAll( float v, bool isBigFrame)
		{
			...
			if (isBigFrame)
			{
				...
				XPLOG_BIG_FRAME_STATE(L", %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %d, %d\n", 
					...
					times[40], times[41], times[42], m_fileIOCount, m_fileIOSize);
			}
		}
#endif
	public:
		...
		inline bool Log( __int64 d, float v)
		{
#ifdef  XPDBG_PRINT_FRAME_STATE
			//if( d /v > 50)
			LogAll(v, d/v > 50);
