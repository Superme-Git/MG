...
namespace Nuclear
{
	...
	bool Write32BitWebPNGWithPitch(XBuffer &buffer, unsigned char* pBits, bool bNeedAlpha,float fQuality, int nWidth, int nHeight, int nPitch)
	{
		try 
		{
			...
			int sz = 0;
			if(bNeedAlpha)
				sz = WebPEncodeRGBA((const uint8_t*)pBits,nWidth,nHeight,nPitch,fQuality,&pOut);
			else
				sz = WebPEncodeRGB((const uint8_t*)pBits,nWidth,nHeight,nPitch,fQuality,&pOut);
			buffer.resize(sz);
			memcpy_s(buffer.normalbegin(), buffer.size(), pOut, sz);
		}
		catch(bool b)
		{
			return b;
