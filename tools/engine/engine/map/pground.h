#ifndef __Nuclear_REGIONMAP_PGROUND_H
#define __Nuclear_REGIONMAP_PGROUND_H
...
namespace Nuclear
{
	namespace SubMap
	{
		...
		union BiggerTileType
		{
			...
			BiggerTileType(unsigned short _d = 0) : data(_d) {}
		};
		typedef BiggerTileType* PtrBiggerTileType;
		union WaterTileType
		{
			unsigned char data;
			struct 
		...
		public:
			...
			BiggerTileType static const BIGGER_TILE_NULL;
			enum {	//Tile的种类			  ABCD	 本类型  	  空白Alpha		   A--------B
		...
		private:
			...
			int m_nWidth;
			int m_nHeight;
			PtrTileType m_pTiles[TILES_SMALL_LAYER_COUNT];
			//每两个字节表示一格，其中，第一个字节(MSK:FF 00)为地表类型，
			//第二个字节的高4位(MSK:00 F0)为Tile的种类，第二个字节的低4位(MSK:00 0F)为具体的TILE号
			unsigned int* m_pGroundColor;
			WaterTileType* m_pWaters;
			PtrBiggerTileType m_pMidTiles[TILES_MID_LAYER_COUNT];
			PtrBiggerTileType m_pBigTiles;
			unsigned char m_TileOrder[TILES_ALL_LAYER_COUNT];
		...
		public:
			PGround(void);
			virtual ~PGround(void);

			virtual bool Init(int width, int height);

			PGround& operator= (const PGround &x);
			...
			const XIStream& unmarshal(const XIStream &os, unsigned int version, int colorWidth, int colorHeight);
			...
			int GetMidTileSize() const { return GetMidTileCount() * sizeof(BiggerTileType); }
			int GetBigTileSize() const { return GetBigTileCount() * sizeof(BiggerTileType); }
			...
			static unsigned char GetGroundType(TileType tile)
			{
				return (tile & TILE_MSK_GROUND_TYPE) >> TILE_ADJ_GROUND_TYPE;
			}

			static TileType SetTileType(TileType tile, unsigned char type)
			{
				tile &= ~TILE_MSK_TILE_TYPE;
				tile |= type << TILE_ADJ_TILE_TYPE;
				return tile;
			}

			static unsigned char GetTileType(TileType tile)
			{
				return (tile & TILE_MSK_TILE_TYPE) >> TILE_ADJ_TILE_TYPE;
			}

			static TileType SetTileNumber(TileType tile, unsigned char number)
			{
				tile &= ~TILE_MSK_TILE_NUMBER;
				tile |= number;
				return tile;
			}

			static unsigned char GetTileNumber(TileType tile)
			{
				return tile & TILE_MSK_TILE_NUMBER;
			}
			...
			WaterTileType *GetWaterData() const { return m_pWaters; }

			TileType *GetSmlTileData(int layer) const 
			{ 
				if (layer<0 || layer >= TILES_SMALL_LAYER_COUNT)
					return NULL;
			...
			BiggerTileType *GetMidTileData(int layer) const 
			{ 
				if (layer<0 || layer >= TILES_MID_LAYER_COUNT)
					return NULL;
				return m_pMidTiles[layer]; 
			}
			BiggerTileType GetMidTileData(int layer, int x, int y) const
			{
				if (!IsPointInMapForMidTiles(x,y))
			...
			bool SetMidTileData(int layer, int x, int y, BiggerTileType data)
			{
				if (!IsPointInMapForMidTiles(x,y))
					return false;
				if (layer<0 || layer >= TILES_MID_LAYER_COUNT)
			...
			bool SetMidTileData(int layer, int pos, BiggerTileType data)
			{
				if (layer<0 || layer >= TILES_MID_LAYER_COUNT)
					return false;
			...
			BiggerTileType *GetBigTileData() const { return m_pBigTiles; }
			BiggerTileType GetBigTileData(int x, int y) const
			{
				if (!IsPointInMapForBigTiles(x,y))
					return BIGGER_TILE_NULL;
			...
			bool SetBigTileData(int x, int y, BiggerTileType data)
			{
				if (!IsPointInMapForBigTiles(x,y))
					return false;
			...
			bool SetBigTileData(int pos, BiggerTileType data)
			{
			...
			bool GetOrders(int i, GroundLayers &layer) const 
			{ 
				if (i>=TILES_ALL_LAYER_COUNT || i<0)
					return false;
				layer = static_cast<GroundLayers>(m_TileOrder[i]);
				return true;
			}
			bool SetOrders(int i, GroundLayers layer)
			{
				if (i>=TILES_ALL_LAYER_COUNT || i<0)
					return false;
				m_TileOrder[i] = layer;
