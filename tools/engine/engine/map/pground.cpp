...
namespace Nuclear
{
	...
	namespace SubMap
	{
		const BiggerTileType PGround::BIGGER_TILE_NULL = 0;
		PGround::PGround(void)
		{
			int layer;
			for (layer=0;layer<TILES_SMALL_LAYER_COUNT;++layer)
			{
				m_pTiles[layer] = NULL;
			}
			for (layer=0;layer<TILES_MID_LAYER_COUNT;++layer)
			{
			...
			for (layer=0;layer<TILES_ALL_LAYER_COUNT;++layer)
			{
		...
		XOStream& PGround::marshal(XOStream &os) const
		{
			//水的信息，不压缩了
			for (int i=0;i<WATER_EFFECT_COUNT;++i)
			{
		...
		const XIStream& PGround::unmarshal(const XIStream &os, unsigned int version, int colorGridWidth, int colorGridHeight)
		{
			//水的信息
			for (int i=0;i<WATER_EFFECT_COUNT;++i)
			{
			...
			/////////////////////水End
			if (version <= PMap::MAP_OLD_GROUND_COLOR)
			{
				...
				for (int oldy = 0; oldy < oldColorHeight; ++oldy)
				{
					for (int oldx = 0; oldx < oldColorWidth; ++oldx, ++pGroundColor)
					{
						...
						newylimit = min(newylimit, smlHeight);
						int newx = oldx * colorGridWidth;
						int newxlimit = newx + colorGridWidth;
						newxlimit = min(newxlimit, smlWidth);
						//都可以等于
		...
		bool PGround::Init(int width, int height)
		{
			if (width == 0 || height == 0)
				return false;
		...
		bool PGround::AllocMaps()
		{
			...
			size = GetWaterSize();
			m_pWaters = static_cast<WaterTileType*>(XAlloc(size));
			if (m_pWaters == NULL)
				return false;
		...
		PGround& PGround::operator= (const PGround &x)
		{
			if (&x == this)
				return *this;
			...
			memcpy(m_TileOrder, x.m_TileOrder, TILES_ALL_LAYER_COUNT);
			int layer;
			int size = GetSmlTileSize();
			FreeMaps();
			for (layer=0;layer<TILES_SMALL_LAYER_COUNT;++layer)
