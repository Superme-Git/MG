...
namespace Nuclear
{
	...
	typedef LinerInter<FPOINT> Vec2LinerInter;
	typedef LinerInter<XPVECTOR4> Vec4LinerInter;

	typedef CKeyValue<FPOINT, Vec2LinerInter>		CPosKeyValue;		//位置
	typedef CKeyValue<FPOINT, Vec2LinerInter>		CScaleKeyValue;		//缩放
	...
	typedef CKeyValue<XPVECTOR4, Vec4LinerInter>	CColorKeyValue;		//RGB颜色（不包括alpha）
	...
	public:
		...
		{
			// 找出第一个大于值的位置
			KEYS::iterator it = std::upper_bound( m_keys.begin(), m_keys.end(), key);
	...
	public:
		...
		{
			assert( !m_keys.empty());
			KEYS::size_type size = m_keys.size();
			if ( size == 1			// 为1作的优化
				|| keyPos >= size)			
			{
				T& back = m_keys.back();
