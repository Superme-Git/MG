#ifndef __Nuclear_SPINEEFFECT_H
#define __Nuclear_SPINEEFFECT_H
...
#include "effect.h"
#include "libspine/spine-dx/include/spine/spine-dx.h"

namespace Nuclear
{
	class SpineEffect : public Effect, public AniLoadingNotify
	{
	//自定义类型、友元、常量
	private:
	...
	public:
		...
		bool LoadEffect(const std::wstring &name);
		spine::SkeletonAnimation* m_pSkelAnim;
		std::string m_defaultActName;
		int				m_totoalTimes;	//! 播放时间
		float			m_alpha;		//! 透明度
		PictureHandle	m_texture;
		float* worldVertices;
