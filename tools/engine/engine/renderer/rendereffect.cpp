	...
	bool RenderEffect::InitShader(IDirect3DDevice9* ifDevice)
	{
		...
		for(int i=0; i<XPTOP_TYPE_COUNT; ++i)
		{
			...
			//	fclose(fp);
				...
				pErrorMsgs->Release();	
				MessageBox(NULL,errStr.c_str(),L"",MB_OK);
				return false;
			}
...
// 					a=2;
	...
	{
		param.BlurCent = FPOINT(0.2f, 0.3f);
	...
	DyeRenderEffect::DyeRenderEffect()
	{
	}

	void DyeRenderEffect::GetShaderStr(int colorOpType, std::string& code)
	{
		FXCode::GetDyeShaderStr(code);
	}

	void DyeRenderEffect::SetParameter(IDirect3DDevice9* ifDevice)
	{
		D3DXHANDLE fparamHandle = m_pConstTablePS[s_currentColorBlendType]->GetConstantByName(NULL, "v_PartParam0");
		m_pConstTablePS[s_currentColorBlendType]->SetFloatArray(ifDevice, fparamHandle, param.fParam0, 4);

		D3DXHANDLE fparamHandle2 = m_pConstTablePS[s_currentColorBlendType]->GetConstantByName(NULL, "v_PartParam1");
		m_pConstTablePS[s_currentColorBlendType]->SetFloatArray(ifDevice, fparamHandle2, param.fParam1, 4);

		D3DXHANDLE fparamHandle3 = m_pConstTablePS[s_currentColorBlendType]->GetConstantByName(NULL, "v_PartParam2");
		m_pConstTablePS[s_currentColorBlendType]->SetFloatArray(ifDevice, fparamHandle3, param.fParam2, 4);
	}
	void DyeRenderEffect::SetTexture(IDirect3DDevice9* ifDevice, const char* name, const void* pParam)
	{
		D3DXHANDLE ftexHandle = m_pConstTablePS[s_currentColorBlendType]->GetConstantByName(NULL, name);
		if(ftexHandle == NULL)	return;
		D3DXCONSTANT_DESC TexDesc;
		UINT count;
		m_pConstTablePS[s_currentColorBlendType]->GetConstantDesc(ftexHandle, &TexDesc, &count);
		ifDevice->SetTexture(TexDesc.RegisterIndex, (LPDIRECT3DTEXTURE9)pParam);
	}
