#ifndef __Nuclear_FONTTEXTURE_H
#define __Nuclear_FONTTEXTURE_H
...
protected:
	...
	struct TextureRes
	{
		...
		TextureRes()
			: allocFlag( 0)
		{}
		TextureRes( PictureHandle _hPic)
			: hPicture( _hPic), allocFlag( 0)
		{}
...
private:
	...
	std::map<int, CFontTexture*>	m_fonttexture;
...
private:
	...
	// segmentNum 纹理片段数, 对于申请长度超过最大级(当前为256)的, 需要拆分到多个纹理片段中存储
	// blocks 由字体高度决定, 获取字体高度所需的行数(一行高度是 MIN_FONT_TEX_HEIGHT 个像素)
