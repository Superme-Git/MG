...
namespace Nuclear
{
	...
	bool PSpriteMaze::Init(int width, int height, const CPOINT& ltpos)
	{
		if (width == 0 || height == 0)
			return false;
		if (m_pMaze)
	...
	XOStream& PSpriteMaze::marshal(XOStream &os) const
	{
		XPASSERT(m_pMaze != NULL);
	...
	const XIStream& PSpriteMaze::unmarshal(const XIStream &os)
	{
