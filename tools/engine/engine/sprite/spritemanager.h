...
public:
	...
	bool Save(CFileIOManager* pFileIOMan, const std::wstring &modelname) const; //编辑器专用
...
class Action : public PObject
{
	struct Frame
	{
		...
		__int64 m_equipOrder[XPDIR_COUNT]; // 衣服，武器，坐骑的先后顺序
...
class Component: public AniLoadingNotify
{
	...
	//typedef std::vector<DyeInfo> DyeInfoVec;//染色方案列表定义
...
public:
	...
	void SetName(const std::wstring &name); // 编辑器使用
	void SetLinkName(const std::wstring &linkname); // 编辑器使用
	...
	//void AddDyeInfo(int index, int h, int s, int l); // 编辑器使用
	//void DelDyeInfo(int index);// 编辑器使用
...
public:
	...
	int GetDyeEnable(const std::wstring &modelname) const;		//编辑器使用
	void SetDyeEnable(const std::wstring &modelname,int i);		//编辑器使用
...
public:
	...
	bool SetEffectPos(const std::wstring &modelname, const CPOINT& pos); //  编辑器使用
	bool SetHorseHeight(const std::wstring &modelname, const std::wstring &horsename, int pos); //  编辑器使用
	bool SetWaterSurfaceType(const std::wstring &modelname, WATER_SURFACE_TYPE type); //  编辑器使用
...
public:
	struct XModel
	{
		...
		std::vector<PartInfo> m_DyeInfos;			//染色方案队列
		int			m_CurDyeIndex;
		...
		void SetEffectPos(const CPOINT& pos) { m_effectPos = pos; }//  编辑器使用
		void SetSurfaceType(WATER_SURFACE_TYPE type) { m_surfaceType = type; } //  编辑器使用
		bool SaveBase(); //  编辑器使用
		...
		void AddDyePart(); // 编辑器使用
		void DelDyePart(int part); // 编辑器使用
		void AddDyeProj(int part); // 编辑器使用
		void DelDyeProj(int part,int proj); // 编辑器使用
		DyeInfo* GetDyeInfo(int part,int proj);
		int getDyePartCount();	// 编辑器使用
		int getDyeProjCount(int part);	// 编辑器使用
	};
