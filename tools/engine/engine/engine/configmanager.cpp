...
namespace Nuclear
{
	...
	void ConfigManager::Init()
	{
		...
		m_nControlFPS[1]			= 60;
		m_nControlFPS[2]			= 30;
		WorldLogicCoord::SetN(m_fLogicToWorldScaleX);
		//
		Update();
	}
	...
	// 创建纹理的池类型
	// @250ms更新
	XPPOOL_TYPE ConfigManager::GetCreateTexturePoolType() const
	{
	...
	// 是否绘制半透明人物
	bool ConfigManager::IsRenderAlphaSprite() const
	{
		return m_bRenderAlphaSprite;
	}

	void ConfigManager::SetRenderAlphaSprite(bool b)
	{
		m_bRenderAlphaSprite = b;
	}
	...
	void ConfigManager::SetSystemLevel(XPCOMPUTERCONFIGTYPE l)
	{
		m_sysLevel = l;
	...
	// 屏幕调试信息开关
	void ConfigManager::SetConsoleInfo(XPCONSOLE_INFO eInfo, bool bOn)
	{
		if( bOn )
	...
	bool ConfigManager::TestConsoleInfo(XPCONSOLE_INFO eInfo) const
	{
		return 0 != (eInfo & m_maskConsoleInfo);
	}

	// 帧状态调试信息开关
	void ConfigManager::SetFrameStateInfo(XPFRAMESTAT_TYPE type, bool bOn)
	{
		if ( bOn )
	...
	bool ConfigManager::TestFrameStateInfo(XPFRAMESTAT_TYPE type) const
	{
		return 0 != ((1 << type) & m_maskFramestat);
	}
	// 帧状态调试信息颜色
	void ConfigManager::SetFrameStateColor(XPFRAMESTAT_TYPE type, XPCOLOR color)
	{
		m_framestatColor[type] = color;
	}
	XPCOLOR ConfigManager::GetFrameStateColor(XPFRAMESTAT_TYPE type) const
	{
	...
	void ConfigManager::SetControlFPS(XPWINDOW_STATE type, int fps)//控制的FPS帧率，默认是60 60 30
	{
		m_nControlFPS[type] = fps;
	}

	int ConfigManager::GetControlFPS(XPWINDOW_STATE type) const
	{
