...
namespace Nuclear
{
	...
	void Smooth::SetSmoothTime( float fSeconds )
	{
		assert(fSeconds > 0);
		m_smoothTime = fSeconds;
	}

	float Smooth::SmoothCD(float &vel, float elapse)
	{
		float to = m_to;
	...
	void Smooth::SetTarget( float from, float to, float velocity, bool bResetVolocity  )
	{
		if( bResetVolocity )
			m_lastVelocity = 0;
		else
	...
	float Smooth::GetPostion( float elapsed )
	{
	...
	void CCameraSmoother::SetViewScale( float fScale )
	{
		if( fScale > 0 )
		{
			m_fViewScale = fScale;
			...
			if( m_bArrival )
				_SetFocus(m_focusX, m_focusY);
			else
			{
				// 如果处于运动过程中, 通过FocusTo完成缓冲式调整
				_FocusTo(m_focusX, m_focusY);
			}
		}
	}

	void CCameraSmoother::_SetFocus(coord_type x, coord_type y)
	{
	...
	void CCameraSmoother::SetFocus(coord_type x, coord_type y)
	{
		m_focusX = x;
		m_focusY = y;
		_SetFocus(x, y);
	...
	void CCameraSmoother::_FocusTo(coord_type x, coord_type y)
	{
		// 相机系统
		m_vxTarget = x - m_viewWidth / 2;
		m_vyTarget = y - m_viewHeight / 2;
		...
		//float vdistance = sqrtf( (float) ( m_vdx*m_vdx + m_vdy*m_vdy ) );	
		//float vfdx = m_vdx / vdistance; // x方向速度单位因子
		//float vfdy = m_vdy / vdistance; // y方向速度单位因子
		float vfdx = 0; // 暂时没有使用
		float vfdy = 0; // 暂时没有使用
	...
	void CCameraSmoother::FocusTo(coord_type x, coord_type y)
	{
		m_focusX = x;
		m_focusY = y;
	...
	void CCameraSmoother::FocusTo(coord_type xRole, coord_type yRole, coord_type xRoleTo, coord_type yRoleTo)
	{
	...
	bool CCameraSmoother::Update(float fCurrent)
	{
	...
	void CCameraSmoother::ScreenToWorld( coord_type& x, coord_type &y ) const
	{
		x += m_vx;
		y += m_vy;
	}

	void CCameraSmoother::WorldToScreen( coord_type& x, coord_type &y ) const
	{
		x -= m_vx;
		y -= m_vy;
	}
	...
	void CCameraSmoother::DrawPath( ) const
	{
		...
		if( pRender != NULL )
		{
			...
			//pRender->DrawLine( Nuclear::FRECT(/*m_vxFrom+cx, m_vyFrom+cy*/0,0, m_vxTarget-cp.left+cx, m_vyTarget-cp.top+cy), 0xff00ffff);
			//pRender->DrawLine( Nuclear::FRECT(m_vx + cx, m_vy + cy, m_vxTarget+cx, m_vyTarget+cy), 0xffff00ff);
			//pRender->DrawLine( Nuclear::FRECT(m_vxFrom-cp.left+cx, m_vyFrom-cp.top+cy, m_vx-cp.left+cx, m_vy-cp.top+cy), 0xff00ffff);
			//pRender->DrawLine( Nuclear::FRECT(0,0,m_vx-cp.left+ cx, m_vy-cp.top+ cy/*, m_vxTarget-cp.left+cx, m_vyTarget-cp.top+cy*/), 0xffff00ff);
		}
	}
