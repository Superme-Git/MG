...
namespace Nuclear
{
	//struct findTree : public std::binary_function<LkoTreeNode*, LkoTreeNode*, bool>
	//{
	//	bool operator ()(const LkoTreeNode* left, const LkoTreeNode* right) const 
	//	{
	...
	void LkoTreeNodeList::RandomBrightTime()
	{
		...
		{
			if ((*iter)->LinkedObjectMoveParam.m_dwFlag & XP_LINKEDOBJ_LIGHT)
			{
				(*iter)->nBeginBrightTime = XP_NIGHT_START_TIME + (rand() % (XP_LIGNT_RANGE * 2)) - XP_LIGNT_RANGE;
				(*iter)->nEndBrightTime = XP_DAY_START_TIME + (rand() % (XP_LIGNT_RANGE * 2)) - XP_LIGNT_RANGE;
				if ((*iter)->nBeginBrightTime >= XP_A_DAY_TIME)
				{
					(*iter)->nBeginBrightTime -= XP_A_DAY_TIME;
				}
				if ((*iter)->nEndBrightTime >= XP_A_DAY_TIME)
				{
					(*iter)->nEndBrightTime -= XP_A_DAY_TIME;
				}
	...
	void LkoTreeNodeList::SimpleUpdate(int timeDelta, int viewPortLeft, int viewPortTop, float fRatio)
	{
		...
		//生成最终画图的矩阵
		(*iter)->mTransAll.Translation((*iter)->mTransResult, -static_cast<float>((*iter)->rotateCent.x), 
			-static_cast<float>((*iter)->rotateCent.y));
		}
	}
	...
	void LkoTreeNodeList::Update(int timeDelta, int viewPortLeft, int viewPortTop, float fRatio)
	{
		...
		for(; iter!=iterEnd; ++iter)
		{
			...
			if ((*iter)->LinkedObjectMoveParam.m_nDistortionType == XPPLANTROTATE)	//旋转
			{
				...
				{
					(*iter)->fAngle += timeDelta * (*iter)->LinkedObjectMoveParam.m_fSpeed *fZoom *0.001f; // / 1000
				...
				XPMatrixRotationOrigin(&(*iter)->mTransAll, (*iter)->fAngle *0.01745f); // / 57.3f
			} 
			...
			if ((*iter)->pParent)//有父节点，自己不是根节点
			{
				...
				case XPMOVEFOLLOWRIGID:	//硬链接
					/*XPMatrixTranslation(&(*iter)->mTransfer, &(*iter)->fromMainRotateCent);
					(*iter)->mTransfer *= (*iter)->pParent->mTransAll;*/
			...
			//生成最终画图的矩阵
			//XPMatrixTranslation(&(*iter)->mTransResult, -static_cast<float>((*iter)->rotateCent.x), 
			//	-static_cast<float>((*iter)->rotateCent.y));
			//(*iter)->mTransResult *= (*iter)->mTransAll;
			(*iter)->mTransAll.Translation((*iter)->mTransResult, -static_cast<float>((*iter)->rotateCent.x), 
				-static_cast<float>((*iter)->rotateCent.y));
		}
