...
#ifdef _ASTAR_POINT_OPERATOR_EQUALS
	// 为了防止用错，必须显示使用上面的比较方法。这个重载用于astar测试，用来比较寻路结果是一样的。
	inline bool operator==(const Point & p2) const throw () { return this->finder_equals(p2); }
#endif
};
...
// 3D支持，在Direction,Point中都加上z轴。
//    修改相应的比较方法。当前寻路只能在2d上工作，忽略z轴。
//    修改apathio.hpp中系列化的。
