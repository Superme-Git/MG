...
public:
	...
	inline void open(ANode * parent, int cost, ANode * target)
	{
		...
		/*this->fcost  = this->gcost + 5 * (abs(target->x - this->x) + abs(target->y - this->y));*/
	...
	inline bool find(const AMap & amap, int startX, int startY, int targetX, int targetY, APath & path /* out */)
	{
		...
		while (!openList.empty())
		{
			...
			if (scanNodes.isOpen(target))
			{
				...
				//path.insert(path.begin(), Grid(startX, startY)); // insert start.
				return true;
			}
		}
		return false;
...
class CMemPoolNode
{
	class CMemPool
	{
		...
		private:
			SBlock& operator=(const SBlock&); // suppress VC2005 warning: "assignment operator could not be generated"
		};
		SBlock* m_block_head;
		SNode*  m_node_head;
	public:
