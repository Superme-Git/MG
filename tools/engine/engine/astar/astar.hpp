...
typedef std::list<Grid> APath; // astar 寻路返回路径。路径点是格子。
...
public:
	...
	*/
	...
	// removeme Point grid2world(float x, float y) const   { return Point(x * eGridSize + eGridSize / 2, y * eGridSize + eGridSize / 2); }
	// removeme Point grid2world(const Point & grid) const { return this->grid2world(grid.x, grid.y); }
