...
HRESULT CFileStream::OpenForWrite(LPCWSTR wzFilePath)
{
	...
	_hFile = ::CreateFileW(wzFilePath, GENERIC_WRITE, 0, NULL,
...
STDAPI CreateFileStreamForRead(LPCWSTR lpszPathName, IStream** ppstm)
{
	...
	HRESULT hr = pStream->OpenForRead( lpszPathName );
	if( FAILED(  hr ) )
	{
		delete pStream;
		return hr;
...
STDAPI CreateFileStreamForWrite(LPCWSTR lpszPathName, IStream** ppstm)
{
	if( lpszPathName == NULL) 
	{
		return E_INVALIDARG;
	...
	HRESULT hr = pStream->OpenForWrite( lpszPathName );
	if( FAILED(  hr ) )
	{
		delete pStream;
		return hr;
