...
CComboListCtrl::~CComboListCtrl()
{
	...
	CInPlaceListBox::DeleteInstance();
}
...
CInPlaceListBox* CComboListCtrl::ShowInPlaceListBox(int iRowIndex, int iColumnIndex, CStringList& rComboItemsList, CString& strCurSelecetion)
{
	CInPlaceListBox* pInPlaceCheckBox = CInPlaceListBox::GetInstance();
	...
	CRect rc;
	GetClientRect( &rc);
	if( obCellRect.bottom > rc.bottom)
	{
		obCellRect.bottom = rc.bottom;
	}

	pInPlaceCheckBox->ShowListBox( m_dwCheckBoxCtrlStyle, obCellRect, this, 0, 
		iRowIndex, iColumnIndex, rComboItemsList, strCurSelecetion);
	...
	if (-1 != iRowIndex)
	{
		...
		if ((GetItemState(iRowIndex, flag ) & flag) == flag)
		{
			...
			if (GetWindowLong(m_hWnd, GWL_STYLE) & LVS_EDITLABELS)
			{
				...
				else if( IsListBox(iColumnIndex))
				{
					CStringList obComboItemsList;

					GetParent()->SendMessage(WM_SET_ITEMS, (WPARAM)iColumnIndex, (LPARAM)&obComboItemsList);

					CInPlaceListBox* pInPlaceCheckBox = ShowInPlaceListBox(iRowIndex, iColumnIndex, 
						obComboItemsList, strCurSelection);
				}
...
void CComboListCtrl::SetListBoxColumns(int iColumnIndex, bool bSet /*= true*/)
{
	// If the Column Index is not present && Set flag is false
	// Then do nothing 
	// If the Column Index is present && Set flag is true
	// Then do nothing
	POSITION Pos = m_ListBoxColumnsList.Find( iColumnIndex);

	// If the Column Index is not present && Set flag is true
	// Then Add to list
	if ((NULL == Pos) && bSet) 
	{
		m_ListBoxColumnsList.AddTail(iColumnIndex); 
	}
	...
	if ((NULL != Pos) && !bSet) 
	{
		m_ListBoxColumnsList.RemoveAt(Pos); 
	}
}
...
bool CComboListCtrl::IsListBox( int iColumnIndex)
{
	if (m_ListBoxColumnsList.Find(iColumnIndex))
	{
		return true;
	}
...
void CComboListCtrl::HideAllInPlaceCtrl()
{
	...
	{
		pInPlaceCombo->PostMessage( WM_CLOSE);
	}
	CInPlaceEdit* pInPlaceEdit = CInPlaceEdit::GetInstance();
	if( pInPlaceEdit && ::IsWindow( pInPlaceEdit->GetSafeHwnd()))
	{
		pInPlaceEdit->PostMessage( WM_CLOSE);
	}
	CInPlaceListBox* pInPlaceListBox = CInPlaceListBox::GetInstance();
	if( pInPlaceListBox && ::IsWindow( pInPlaceListBox->GetSafeHwnd()))
	{
		pInPlaceListBox->PostMessage( WM_CLOSE);
	}
}
