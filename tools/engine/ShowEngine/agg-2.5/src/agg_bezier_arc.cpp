...
namespace agg
{
    ...
    void bezier_arc::init(double x,  double y, 
                          ...
                          double sweep_angle)
    {
        start_angle = fmod(start_angle, pi2 );
        if(sweep_angle >=  pi2 ) sweep_angle =  pi2;
        if(sweep_angle <= -pi2 ) sweep_angle = -pi2;
		...
		// fix the error for last vertex of the circle
		if ( m_num_vertices == 26 &&
			calc_distance( m_vertices[0],m_vertices[1], m_vertices[24], m_vertices[25]) < 0.01 )
		{
			m_vertices[25] += 0.5;
		}
    }
