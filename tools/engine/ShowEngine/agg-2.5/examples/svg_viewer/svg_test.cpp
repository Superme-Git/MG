...
public:
    ...
    virtual void on_draw()
    {
        ...
        sprintf_s(buf, sizeof(buf), "Vertices=%d Time=%.3f ms", vertex_count, tm);
    ...
    virtual void on_key(int x, int y, unsigned key, unsigned flags)
    {
        if(key == ' ')
        {
            ...
            char buf[128];
            sprintf_s(buf, sizeof(buf), "%3.3f, %3.3f, %3.3f, %3.3f, %3.3f, %3.3f",
...
int agg_main(int argc, char* argv[])
{
    ...
    {
        FILE* fd = NULL;
		errno_t err = fopen_s(&fd, app.full_file_name(fname), "r");
        if(err != 0 || fd == 0)
        {
    ...
    if(app.init(1200, 600, agg::window_resize))
    {
        return app.run();
    }
    return 1;
