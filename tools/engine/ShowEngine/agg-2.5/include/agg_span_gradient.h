...
namespace agg
{
	...
	public:
		...
		gradient_ellipse( double rx, double ry, double fx, double fy )
			: m_rx( rx )
			, m_ry( ry )
			, m_fx( fx )
			, m_fy (fy )
		...
		void init(double rx, double ry, double fx, double fy  )
		{
			m_rx = rx;
			m_ry = ry;
			m_fx =  fx;
		...
		int calculate(int x, int y, int) const
		{
			if ( m_rx > m_ry )
			{
				//return int(fast_sqrt( x*x + m_e2 * y*y));
				return int(sqrt( x*x + m_e2 * y*y));
			}
			else
			{
				return int(sqrt( m_e2 * x*x + y*y));
			}
	...
	private:
		...
		// e -  eccentricity
		double    m_e2; // 1-e^2
		double    m_fx; // reserved
		double    m_fy; // reserved
	};
