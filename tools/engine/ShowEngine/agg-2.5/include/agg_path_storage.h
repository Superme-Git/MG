...
namespace agg
{
    ...
    public:
		...
		//yanghaibo
		void ell_arc_to(double x, double y);
		...
		void join_path(VertexSource& vs, unsigned path_id = 0)
		{
			...
			if(!is_stop(cmd))
			{
				if(is_vertex(cmd))
				{
					...
					if(is_vertex(cmd0))
					{
						// 因为round_rect的实现问题，导致使用vertex_dist_epsilon会出现不必要的line_to
						// 参见ell_arc_to()
						// [10/17/2008 yanghaibo]
						if(calc_distance(x, y, x0, y0) > 0.01 /*vertex_dist_epsilon*/)
						{
	...
	template<class VC> 
	inline void path_base<VC>::ell_arc_to(double x, double y)
	{
		//////////////////////////////////////////////////////////////////////////
		//
		// Draw an pi/2.0 clockwise ellipse arc from current position(x0,y0)
		// to the destination(x, y). 
		// It is used to draw the corner of a rounded-rectangle.
		//
		// 1. top-right arc
		// start angle : pi/-2.0, cx,cy : (x0,y)
		//
		//                 (x0,y0)
		//     /-----------------\
		//    /                   \
		...
		//                     (x,y)
		// 3. left-bottom arc:
		// start angle : pi/2.0: 0.0, cx,cy : (x,y0)
		//
		//      /-----------------\
		//     /                   \
		//     |                   | 
		...
		bezier_arc a(cx, cy, fabs(rx), fabs(ry), start_angle, pi/2.0 );
		join_path(a,0);
	}
