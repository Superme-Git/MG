...
public:
	...
	public:
		CSingleLock( CCriticalSection& m) : m_mutex( m) { m_mutex.Lock(); }
		~CSingleLock() { m_mutex.Unlock(); }
	};
public:
...
#define AUTO_LOCK()	CCriticalSection::CSingleLock __autoLock(g_csLogFile);
#define TRACE __noop
...
// 格式化系统API产生的错误消息
//
static void FormatErrorMessage( DWORD dwErrorCode, LPWSTR pMessageBuffer, UINT nBufferSize, ... )
{
	va_list vl;
	va_start(vl, nBufferSize);
	...
	if( lpBuffer != NULL )
	{
		wcscpy_s( pMessageBuffer, nBufferSize, lpBuffer );
		LocalFree(lpBuffer);
	}
...
__RETRY__:
		if( g_logFile.Open(
			g_lpszLogFileName,
			CFile::FM_CREAT, 
			CFile::FA_WRONLY, &fileException ) )
		{		
			if( g_logFile.GetSize() == 0 )
			{
			...
			{
				g_logFile.Seek(0, CFile::FSM_END);
			}
		}
		else
		{
			int nError = fileException.GetLastError();
			if ( !bRetried && (
				nError == ERROR_ACCESS_DENIED ||
				nError == ERROR_SHARING_VIOLATION ||
				nError == ERROR_LOCK_VIOLATION ) )
			{
				// 根据进程ID重新确定文件名
			...
			else
			{
				...
				FormatErrorMessage( nError, ErrorMessage, 256 );
				if( ErrorMessage[0] != 0 )
				{
	...
	if( g_logFile.GetSize() >= MAX_LOG_FILE_SIZE )
	{
		g_logFile.Flush();
		// 关闭当前的日志文件
		g_logFile.Close();
		...
		std::wstring lpszLogFileName_backup = g_lpszLogFileName;
		size_t validSize = lpszLogFileName_backup.size()-(int)_tcslen(g_lpszSuffix);
		lpszLogFileName_backup.resize(validSize*2);
		
		StringCchPrintfW( (wchar_t*)(lpszLogFileName_backup.c_str() + validSize), lpszLogFileName_backup.size(), 
			_T("_[%04hu-%02hu-%02hu][%-06d]%s"), 
			systemTime.wYear,systemTime.wMonth,systemTime.wDay, lMilliSeconds, g_lpszSuffix );
		validSize = wcslen( lpszLogFileName_backup.c_str() );
		lpszLogFileName_backup.resize(validSize);

		// 重命名
		if( !MoveFileW(g_lpszLogFileName,  lpszLogFileName_backup.c_str()) )
		{
			DWORD dwError = GetLastError();
			TCHAR ErrorMessage[256] = {0};
			FormatErrorMessage( dwError, ErrorMessage, 256 );
			if( ErrorMessage[0] != 0 )
			{
				TRACE(ErrorMessage);TRACE(_T("\n"));
			}
