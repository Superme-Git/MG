...
: m_mode( UPDATE_MODE_DEFAULT)
{
	...
	m_serverVersion.Close(); // 显示标记版本无效
}
...
BOOL CUpdateThread::_DoCheckVersion( UPDATE_MODE mode, 
									BOOL& bHasNewVersion, BOOL& bLocalValid, BOOL& bAutoPatch, BOOL& bMergeCache )
{
	m_bMerging = FALSE;

	// 设置主界面检查版本进度条，最大值30，步长10
	if( m_pMainWnd != NULL )
		m_pMainWnd->PostMessage( WM_CHECK_START, 10, 30 );

	CUpdateManager& rUpdater = GetUpdateManager( mode);
	rUpdater.SetUpdateServerType( m_updateServerType);	// 电信 or 网通
	rUpdater.SetPatchPath( m_patchpath);	// 手动更新包路径
	
	bool bRet = rUpdater.HasNewVersion( GetGameClientDir(), 
		(bool&)bHasNewVersion, (bool&)bLocalValid, (bool&)bAutoPatch, (bool&)bMergeCache );
	DWORD wParam = 0;
	if( bHasNewVersion)
	{
	...
	if( bRet )
	{
		...
		{
			// 对于手动补丁包更新，从服务器获取在线的最新版本信息。
			GetOnlineServerVersionInfo( rUpdater.GetLocalVersion(), m_serverVersion );
		}
...
BOOL CUpdateThread::_DoUpdate( UPDATE_MODE mode )
{
	...
	if( m_pMainWnd != NULL )
	{
		...
		if( bRet )
		{
			...
			{
				if( mode == UPDATE_MODE_MERGECACHE )
				{
					// 如果合并后台更新完成，且进行过重启，关闭此标志。
					SetRebooted(FALSE);
				}
		...
		else
		{
			if( mode == UPDATE_MODE_MERGECACHE )
			{
				...
				// 缓存合并更新失败之后，自动启动新一轮版本检查过程，更换其他模式进行自动更新。
				m_pMainWnd->PostMessage(WM_COMMAND, IDC_CHECK_UPDATE, 0 );
			}
		}
	}
		...
		case WAIT_OBJECT_0 + 1:
			{
				...
				BOOL bMergeCache = FALSE;
				BOOL bRet = _DoCheckVersion( m_mode, bHasNewVersion, bLocalValid, bAutoPatch, bMergeCache );
				...
				if( bMergeCache )
				{
					// 无论原来是什么模式的版本检查，此时如果需要首先合并cache，那么就切换更新方式，直接进行更新。
					m_mode = UPDATE_MODE_MERGECACHE;
				}
				else
				{
					// 如果不需要首先合并后台缓存
		...
		case WAIT_OBJECT_0 + 3:
			{
				...
				BOOL bAutoPatch = FALSE;
				BOOL bMergeCache = FALSE;
				BOOL bRet = _DoCheckVersion( UPDATE_MODE_BACKGROUND, bHasNewVersion, bLocalValid, bAutoPatch, bMergeCache );
				m_bBKUpdatingInProgress = FALSE;
