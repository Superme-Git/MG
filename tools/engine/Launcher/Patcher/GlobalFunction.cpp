								   ...
								   HANDLE hStopEvent, const PFSX::SetupMetaVector& setupMetas)
{
	...
	// Mount PFS 子包
	bOK = _MountForSetupMetas(strRunDir, bReadOnly, hStopEvent, setupMetas);
	if( !bOK )
		UnmountPFS();
	else
		m_bNeedUnmount = true;
...
bool CUpdateMountManager::_MountForSetupMetas( const std::wstring& strRunDir, bool bReadOnly, 
											  HANDLE hStopEvent, const PFSX::SetupMetaVector& setupMetas)
{
	bool bRet = true;
	...
	for( PFSX::SetupMetaVector::const_iterator i=setupMetas.begin(); i != setupMetas.end(); ++i )
	{
		...
		do 
		{
			...
			{
				int nLastError = PFS::CEnv::GetLastError();
				if( nLastError == PFS::EC_DEVICE_ALREADY_MOUNT )
				{
					KHMsgLog_Warning(L"Warning - try to duplicate mount device (%s) (%d)\n", pfsFile.c_str(), nLastError );
					continue;
				}
...
bool CUpdateMountManager::MountPFSForNewAdded( const std::wstring& strRunDir,
											  HANDLE hStopEvent, const PFSX::SetupMetaVector& newSetupMetas )
{
	// 肯定不是只读的Mount
	return _MountForSetupMetas(strRunDir, false, hStopEvent, newSetupMetas);
}
...
bool CDownloadFiles::Download( HANDLE hBreakEvent )
{
	...
	std::wstring cacheDir = m_strDir + L".cache";
	...
	if( !PFS::CEnv::SetOption( PFS::PFS_OPTION_BASE_FILESMETAVERSION, RES_ROOT, L"1.1" ) )
	{
		// 暂时忽略，只记录日志
		KHMsgLog_Warning(L"Warning - CEnv::SetOption(PFS_OPTION_BASE_FILESMETAVERSION, %s, %s) failed(%d)\n", cacheDir.c_str(), L"1.1", PFS::CEnv::GetLastError() );
	}
					  ...
					  bool bPatch)
{
	...
	for(PFS::CMetaInfoMap::const_iterator itr = rBegin; itr != rEnd; ++ itr )
	{
		...
		//-----------------------------------------------------
		// PFS实现PFS_OPTION_BASE_FILESMETAVERSION之后，RefreshMetaFile()可以根据
		// PFS::CEnv::SetOption()函数设置的FilesMetaVersion记录指定版本的MetaInfo。
		// 上述问题可以解决，但由于“没有必要再根据文件名获取一次metainfo”，所以暂不修改。
		// 2011-10-24 
		PFS::CFile theFile;
...
bool WriteMetaInfoFile2( const std::wstring& dir, const PFS::CMetaInfoMap& mimFile )
{
	bool bRet = PFSX::WriteMetaInfoFile( dir + PFSX::FILES2_META_NAME, mimFile );
	if( !bRet )
	{
		upd::SetLastError( PFS::CEnv::GetLastError(), 
			GlobalFormatText( ERR_WRITE_FILE, dir.c_str(), PFSX::FILES2_META_NAME.c_str()) );
	}
...
//
PFS::fsize_t SumMetaInfoFileSize2( const PFS::CMetaInfoMap& rMetaInfoMap )
{
	PFS::fsize_t total = 0;
	for(PFS::CMetaInfoMap::const_iterator itr = rMetaInfoMap.begin(), rEnd = rMetaInfoMap.end();
		itr != rEnd; ++ itr )
	{
		if( itr->second.IsV10() )
			total += itr->second.GetFileSize();
		else
			total += itr->second.GetFileSize2();
	}

	return total;
}

void EraseMetaInfosByIgnoringConfig( PFS::CMetaInfoMap& rMetaInfoMap )
{
	CStringArray filePathArray;
	BOOL bConfigResult = GetUpdateIgnoringFilePathArray(filePathArray);
	if( bConfigResult )
	{
		for( INT_PTR i = filePathArray.GetSize() - 1; i >= 0; i -- )
		{
			const CString& strFileName = filePathArray.GetAt(i); // 自动trim
			KHMsgLog_DebugW(L"%d:%s\r\n", i, strFileName.GetString() );
			rMetaInfoMap.erase(std::wstring(strFileName));
		}
	}
}
...
//
bool GetOnlineServerVersionInfo( const PFSX::CVersionInfo& rLocalVersionInfo, 
								 PFSX::CVersionInfo& rServerVersionInfo )
{
	...
	// 现在所支持的获取服务器最新版本信息的更新类型（不考虑后台更新）
	int versionTypes[] = 
	{
		PFSX::VERSION_DEFAULT,	// 缺省更新
		PFSX::VERSION_SERVER,	// 版本服务器
		PFSX::VERSION_PATCH,	// 补丁包更新
	};

	// 遍历各种服务器、各种更新模式的URL。
	for( int server = 0; server < _countof(serverTypes); server ++ )
	{
		int serverType = serverTypes[server];
		for( int version = 0; version < _countof(versionTypes); version ++ )
		{
			int versionType = versionTypes[version];
			int type = serverType*PFSX::VERSION_COUNT + versionType;
			const PFSX::VersionUpdateInfo *pUpdateInfo = rLocalVersionInfo.GetUpdateInfoByType(type);

			if ( pUpdateInfo != NULL && !pUpdateInfo->Url.empty() )
			{
				if( PFSX::GetRemoteVersion( pUpdateInfo->Url, rServerVersionInfo) )
				{
					KHMsgLog_InfoW(
						L"GetOnlineServerVersionInfo ok( by url: %s, serverType: %d, versionType: %d)\r\n",
					pUpdateInfo->Url.c_str(), serverType, versionType );
	...
	rLocalVersionInfo.WriteTo(dataVersionInfo);
	const wchar_t ends = 0;
	dataVersionInfo.Append( &ends, 2 );

	KHMsgLog_ErrorW(
		L"GetOnlineServerVersionInfo failed by version:\n %s", dataVersionInfo.GetData() );

	rServerVersionInfo.Close();
