...
BOOL CreateRgnByAlphaBits(const CBitmap &rAlphaDIBmp, const CRect& rRect, CRgn& rWndRgn)
{
	...
	//取得图像的长和宽
	int nW = min(BitMap.bmWidth, rRect.right);
	int nH = min(BitMap.bmHeight, rRect.bottom);
	...
	BYTE* pLine = NULL;
	BYTE* pBit = NULL;
	for(int y = rRect.top; y < nH; y++)
	{
		//pLine = lpBits + BitMap.bmWidthBytes * (BitMap.bmHeight-y-1);
		pLine = lpBits + BitMap.bmWidthBytes * (y-1);
...
//////////////////////////////////////////////////////////////////////////
/// 抽取Bitmap的子图。如果rBitmapFull是DIB，抽取的rBitmapResult 也是DIB。
//////////////////////////////////////////////////////////////////////////
void ExtractBitmap( const CBitmap& rBitmapFull, const CRect& rClipBox, CBitmap& rBitmapResult )
{
	CDC dcTemp;
	dcTemp.CreateCompatibleDC( NULL );
	CGdiObject* pOldBitmap = dcTemp.SelectObject( (CBitmap*)&rBitmapFull );

	CDC rDC;
	rDC.CreateCompatibleDC( NULL );
	rBitmapResult.DeleteObject();
	rBitmapResult.CreateCompatibleBitmap(&dcTemp, rClipBox.Width(), rClipBox.Height() );
	CGdiObject* pOldBitmap2 = rDC.SelectObject(&rBitmapResult);

	rDC.BitBlt( 0, 0, rClipBox.Width(), rClipBox.Height(), &dcTemp, rClipBox.left, rClipBox.top, SRCCOPY );

	rDC.SelectObject(pOldBitmap2);
