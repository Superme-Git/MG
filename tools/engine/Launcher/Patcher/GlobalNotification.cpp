...
void CGlobalNotification::NotifyStep(const Message& rMessage, int stepValue )
{
	if ( rMessage.dwStatusCode != 0 )
	{
		// 只记录错误状态的信息
	...
	if( m_watcher != NULL )
	{
		m_watcher->OnProgressStep(m_nextMid, stepValue);
	}
...
void GlobalNotifyStep( const std::wstring& rstrItem, DWORD dwStatusCode, int stepValue)
{
	Message message;
	message.dwStatusCode = dwStatusCode;
	...
	AUTO_LOCK();
	return CGlobalNotification::Instance().NotifyStep(message, stepValue);
}
