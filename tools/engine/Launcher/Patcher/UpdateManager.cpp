...
bool EvalSetupMetaDifference(
	PFSX::SetupMetaVector smvServer,
	PFSX::SetupMetaVector smvLocal,
	PFSX::SetupMetaVector& rsmvDifference,
	bool localVersionValid )
{
	// 计算 SetupMeta 的差异列表
	if( smvServer.empty() )
	{
		rsmvDifference = smvLocal;
	}
	else
	{
		if ( smvLocal.IsOriginal() || // 跨版本处理.setup.meta的问题
			!localVersionValid )	// 本地版本无效时，处理所有pfs包
		{
			rsmvDifference = smvServer; // 使用服务器.setup.meta
			rsmvDifference.SetOriginal(false);
		}
		else
		{
			...
			// 本地和服务器均有效，计算差异。
			PFSX::SubtractSetupMeta( smvServer, smvLocal, rsmvDifference );
		}
	}
...

{
	...
	// 将根目录下的MetaInfoMap保存到缓存目录中的.files2.meta文件中（版本1.1）。
	WriteMetaInfoFile2(cachedir, rMimServer);
...
bool EvalDownloadItemsForRoot(const std::wstring& rundir,		// 运行目录
							  const std::wstring& cachedir,		// 缓存目录
							  PFS::CMetaInfoMap& rMimDifference	// 服务器根目录和本地根目录的差异列表
							  )
{
	PFS::CMetaInfoMap rMimServer, rMimLocal;  
	rMimDifference.clear();

	// 读本地 mount 目录的 .files.meta 文件
	if( !ReadMetaInfoFile( rundir, rMimLocal))
	{
		// .file.meta 有可能不存在
		int nErrorCode = PFS::CEnv::GetLastError();
		if( nErrorCode != ERROR_PATH_NOT_FOUND &&
	...
	// 读服务器 mount 目录的 .files2.meta 文件	
	if( !ReadMetaInfoFile2(cachedir, rMimServer) )
	{
		return false;
	}
					 ...
					 )
{
	...
	KHMsgLog_DebugW( L"正在读取 %s 中的文件列表.\n", cacheDir.c_str() );
	...
	if( !EvalDownloadItemsForRoot( rundir, cacheDir, rMimDownload) )
	{
		return false;
	}
	...
	for( PFSX::SetupMetaVector::const_iterator iV = smvDiff.begin(); iV != smvDiff.end(); ++ iV)
	{
		...
		if( !ReadMetaInfoFile2( cacheDir + mount, mimServer ) )
		{
			continue;
		}
		...
		{
			// 读本地.file.meta 失败，本地旧版本可能尚不存在此pfs包，此种情况，假定.files2.meta中记录的文件都需要下载。
		}

		PFS::CMetaInfoMap mimDownload;
		// 计算2个MetaInfoMap的差集
		SubtractMetaInfoMap( mimServer, mimLocal, mimDownload );
					 ...
					 )
{
	...
	if( !EvalUpdateItemsForRoot( url, rundir, cacheDir, mimRootServer, mimRootLocal, rMimDownload) )
	{
		return false;
	}
	...
	if( !rbNeedUpdateLauncher)
	{
		...
		for( PFSX::SetupMetaVector::const_iterator iV=smvDiff.begin(); iV!=smvDiff.end(); ++iV)
		{
			...
			// 读cache目录mount子目录的 .files2.meta 文件（服务器最新的子包文件清单，用于计算需要下载的项目）
			GlobalNotify( GlobalGetText( TIP_ANALYZE_CHILD_META_FILE).c_str(), RES_ROOT, mount.c_str());
			...
			if( mimLocal.size() != mimServer.size() )
			{
				KHMsgLog_DebugW( L"%s 中本地有 %lu 项，服务器有 %lu 项。\n", mount.c_str(),
					mimLocal.size(), mimServer.size() );
			}

			PFS::CMetaInfoMap mimDelete;
			// 统计需要删除的文件
			SubtractMetaInfoMapEx( mimLocal, mimServer, mimDelete );
...
public:
	...
	virtual bool HasNewVersion( LPCTSTR, bool& rHas, bool&, bool&, bool& )
	{
		rHas = false;
		return true;
	}
...
CUpdateManager& GetUpdateManager(UPDATE_MODE eUpdateMode)
{
	...
	private:
		CUpdateManager* _CreateUpdater( int mode )
		{
			...
			case UPDATE_MODE_MERGECACHE:
				return new CUpdateManagerMergeCache();
...
bool CUpdateManager::HasNewVersion( LPCTSTR szRunDir, bool& rHas, bool& rLocalValid, bool& rAutoPatch, bool& rMergeCache )
{
	...
	if ( rHas )
	{
		...
		if( PFSX::GetLocalVersion( m_cacheDir, cacheVersion) && cacheVersion.IsValid() )
		{
			bCacheFullyReady = PFSX::CompareVersion( cacheVersion, m_newVersionInfo );
			if( !IsBackground() )
			{
				if( !bCacheFullyReady )
				{
					bCachePartlyReady = !PFSX::CompareVersion( m_localVersion, cacheVersion) &&
						m_localVersion.m_Version < cacheVersion.m_Version &&  // 这里增加版本大小判断(允许相同是因为有可能仅仅变更了发布日期)
						m_localVersion.m_ReleaseDate <= cacheVersion.m_ReleaseDate; 

					std::wstring cacheVersionString;
					PFSX::Version2String( cacheVersion.m_Version, cacheVersionString );
		...
		if( bCacheFullyReady && IsBackground() )
		{
	...
	m_smvServer.clear();
	bool bSetupMetaReady = PFSX::CSetupMetaFile::DoRead( m_cacheDir + PFSX::SETUP_META_NAME, m_smvServer);
	if( !bSetupMetaReady )
	{
		KHMsgLog_WarningW( L"Read SetupMetaFile failed: %s(%d)\n",
			(m_cacheDir + PFSX::SETUP_META_NAME).c_str(), PFS::CEnv::GetLastError() );

		// 缓存没有就绪或者从缓存读取失败，改从服务器下载 .setup.meta 文件
		if( !ReadRemoteSetupMeta( m_url, m_smvServer))
		{
			KHMsgLog_WarningW( L"Download SetupMetaFile failed: %s(%d)\n",
				m_url.c_str(), PFS::CEnv::GetLastError() );
		}
		else
		{
			bSetupMetaReady = true;
		}
	}
		...
		m_localVersion.m_Version < m_newVersionInfo.m_luv )
	{
		do
		{
			...
			// 下面计算补丁包大小的逻辑，没有考虑已经下载成功的补丁包，也没有考虑已经下载了部分数据的补丁包。
			// 为了统计是否有必要启用补丁包更新，暂时没有必要做更多的优化计算。
			PFS::fsize_t patchesTotalSize = 0;
			for( std::vector< PFSX::PatchInfo >::const_iterator i = piVector.begin(), e = piVector.end(); i != e; ++ i )
				patchesTotalSize += i->size;

			KHMsgLog_InfoW( L"patchesTotalSize: %I64u\n", patchesTotalSize);
			
			// 读本地 .setup.meta 文件,  .setup.meta 不存在的情况下也允许更新
			m_smvLocal.clear();
			if( PFSX::CSetupMetaFile::DoRead( m_runDir + PFSX::SETUP_META_NAME, m_smvLocal))
			{
				// 检查本地 pfs 文件是否存在
			...
			PFS::fsize_t totalNeedDownload = 0, totalCached = 0;
			
			if( bSetupMetaReady )
			{
				PFSX::SetupMetaVector smvDiff;
				EvalSetupMetaDifference(m_smvServer, m_smvLocal, smvDiff, rLocalValid );
				
				PFS::CMetaInfoMap mimDownload, mimCache;
				if( EvalDownloadItems(m_runDir, m_cacheDir, smvDiff, mimDownload, mimCache ) )
				{
					totalNeedDownload = SumMetaInfoFileSize2( mimDownload );
					totalCached = SumMetaInfoFileSize2( mimCache );

					KHMsgLog_InfoW( L"totalNeedDownload: %I64u\n", totalNeedDownload);
					KHMsgLog_InfoW( L"totalCached: %I64u\n", totalCached);
				}
			}

			if( totalNeedDownload == 0 || 
				patchesTotalSize <= totalNeedDownload )
			{
				// 为补丁包自动更新模式准备数据
	...
	{
		if( bSetupMetaReady && bCachePartlyReady )
		{
			rMergeCache = true;
		}
	}
	...
	else
	{
		if( upd::GetLastError().ErrorCode != SC_USER_CANCELED )
		{
				...
				m_newVersionInfo, PFSX::VERSION_ALL_TYPES ) )
			{
				...
				KHMsgLog_ErrorW( L"由于缓存无效，标记版本文件无效失败，删除之 - %s(%d)",
