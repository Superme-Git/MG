...
bool CUpdateManagerRepair::HasNewVersion( LPCTSTR szRunDir, bool& rHas, bool& rLocalValid, bool& rAutoPatch, bool& rMergeCache )
{
	ASSERT(FALSE);
	rHas = true;
	rAutoPatch = false;
...
/// </summary>
bool CUpdateManagerRepair::BeginLoop( )
{
	// 不下载
	m_mimDownload.clear();
...
bool CUpdateManagerRepair::CheckDifference( size_t& rDownload, size_t& rMerge, size_t& rDelete )
{
	rDownload = 0;
	rMerge = 0;
	rDelete = 0; // 用手动更新包对版本进行修复的时候，不处理删除项目。
...
void CUpdateManagerRepair::EndLoop( bool bLoopSucceeded )
{
