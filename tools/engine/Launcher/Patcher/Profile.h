...
const TCHAR DOWNLOAD[] = L"download";
const TCHAR DSPEED[] = L"dspeed";
const TCHAR USPEED[] = L"uspeed";
...
enum UPDATE_MODE
{
	...
	UPDATE_MODE_MERGECACHE	= 5, // 从已经就绪的后台缓存中合并更新
...
public:
	...
	int GetFileDownloadType()
	{
		return GetPrivateProfileInt( SECTION, DOWNLOAD, 0, m_file.c_str());
	}

	void SetFileDownloadType( int type)
	{
		std::wostringstream woss;
		woss << type;
		WritePrivateProfileString( SECTION, DOWNLOAD, woss.str().c_str(), m_file.c_str());
	}

	int GetDownloadSpeed()
	{
		return GetPrivateProfileInt( SECTION, DSPEED, 0, m_file.c_str());
	}

	void SetDownloadSpeed(int s)
	{
		std::wostringstream woss;
		woss << s;
		WritePrivateProfileString( SECTION, DSPEED, woss.str().c_str(), m_file.c_str());
	}

	int GetUploadSpeed()
	{
		return GetPrivateProfileInt( SECTION, USPEED, 0, m_file.c_str());
	}

	void SetUploadSpeed(int s)
	{
		std::wostringstream woss;
		woss << s;
		WritePrivateProfileString( SECTION, USPEED, woss.str().c_str(), m_file.c_str());
	}
