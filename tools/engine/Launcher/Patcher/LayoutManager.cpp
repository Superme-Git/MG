...
BOOL CLayoutManager::SetPanel( CWnd* pPanel, BOOL bDoRanging/* = TRUE*/ )
{
	ASSERT( pPanel != NULL );
	m_wndPanel = pPanel;
...
// 初始化面板窗口
BOOL CLayoutManager::_InitPanel(BOOL bDoRanging)
{
	CString strQueryString;
	strQueryString.Format(L"/Desktop/Content/Frame[@n=\"%s\"]/Image1", m_wndName.c_str());

	// 使用XPath接口
	CComBSTR bstr = strQueryString.AllocSysString();

	CComPtr<IXMLDOMNode> spImageNode;
	HRESULT hr = m_rootNode->selectSingleNode(bstr, &spImageNode );
	if ( hr != 0 )
	{
		return FALSE;
	}
	...
	CRect rClipRect;
	HBITMAP hBmpFull;
	if( !_ParseImageNode(spImageNode, hBmpFull, TRUE, rClipRect, FALSE) )
	{
		// No resource
		return FALSE;
	}
	...
	CBitmap _bitmapFull;
	if( !_bitmapFull.Attach(hBmpFull) )
	{
		return FALSE;
	}

	_bitmapFull.GetBitmap(&m_bitmapInfo);

	CRect rcBitmap(0,0,m_bitmapInfo.bmWidth,m_bitmapInfo.bmHeight);
	if( rClipRect.EqualRect(&rcBitmap) )
	{
		// 无需剪裁，直接使用原图。
		m_bitmap.Attach( _bitmapFull.Detach() );
	}
	else
	{
		ExtractBitmap(_bitmapFull, rClipRect, m_bitmap);
		// 注意，hBmpFull已经被缓存，这里必须Detach这个缓存的Bitmap。
		_bitmapFull.Detach();
	...
	if( bDoRanging )
	{
		...
		{
			//下面两种方式可以得到等价的结果： 
			//CreateRgnByAlphaBits(_bitmapFull, rClipRect, wndRgn);
			// 但是显然下面的方式效率更高。
			CRgn wndRgn;
			BOOL bRangeOK = CreateRgnByAlphaBits(m_bitmap, wndRgn);
			if( bRangeOK )
			{
		...
		m_wndPanel->MoveWindow(&rClipRect);
...
// 加载DIB位图资源
HBITMAP CLayoutManager::_LoadDIBitmap( LPCTSTR lpszFileName, BOOL bPremultiply )
{
	void* pVoid = NULL;
	if( bPremultiply )
	{
		if( m_dibMapPremultiplied.Lookup(lpszFileName, pVoid ) )
			return (HBITMAP)pVoid;
	}
	else if( m_dibMap.Lookup(lpszFileName, pVoid ) )
	{
		return (HBITMAP)pVoid;
	}
	...
	if ( pFile != NULL )
	{
		// 当初这里执行Premultiply操作，现在看似乎没有任何必要。
		hBmp = LoadDIBitmap( pFile, bPremultiply );
	...
	if( bPremultiply )
	{
		m_dibMapPremultiplied.SetAt( lpszFileName, (void*)hBmp );
	}
	else
	{
...
BOOL CLayoutManager::_ParseImageNode(CComPtr<IXMLDOMNode> spImageNode,
									 HBITMAP& rBitmap, BOOL bDIB, CRect& rRect, BOOL bPremultiply)
{
	...
	{
		// Load DIB
		rBitmap = _LoadDIBitmap( imageFileName.c_str(), bPremultiply );
	}
	else
	{
		// Load DDB
	...
	_ParseNodeRect(spImageNode, rRect);
...
// 输入控件
BOOL CLayoutManager::ApplyEdit( UINT nID, CDIYEditCtrl& rButton )
{
	CString strName;
	strName.LoadString( nID );
	strName = strName.SpanExcluding( _T("\n\t") );
	...
	CString strQueryString;
	strQueryString.Format(
		_T("/Desktop/Content/Frame[@n=\"%s\"]/Content/EditImage[@n=\"%s\"]"), m_wndName.c_str(),
		strName.GetString());
	...
	hr = ctrlNode.SelectSingleNode( L"Image", spImageNode);
	if ( hr != S_OK )
	{
		return FALSE;
	}
	...
	// 但是，必须保证这些状态图在一张底图上。
	bRet = rButton.InitializeFromBitmap(imageBitmap, rcRect);
...
//
BOOL CLayoutManager::GetBitmap( LPCTSTR lpszControlName, LPCTSTR lpszImageElementName, HBITMAP& rBitmap, BOOL bDIB, CRect& rRect)
{
	CString strQueryString;
	strQueryString.Format(
		_T("/Desktop/Content//*[@n=\"%s\"]"), lpszControlName);
	...
	CComPtr<IXMLDOMNode> spImageNode;
	CComBSTR bstrElementName(lpszImageElementName);
	hr = ctrlNode.SelectSingleNode( bstrElementName, spImageNode);
	if ( hr != S_OK )
	{
		return FALSE;
	}
