...
{
	public class CComponentButtonText : CComponent
	{
		private readonly CFormMain m_mainform;
		private readonly CFormButtonText m_form;
		...
		public CComponentButtonText(CComponentDesktop desktop, CFormMain mainform, string name) : base(desktop)
		{
			...
			m_form = new CFormButtonText(m_mainform, this);
		...
		{
			CComponentButtonText one = new CComponentButtonText(Desktop, m_mainform, Name);
			one.Parent = Parent;
            Rectangle rect = Rect;
            rect.Offset(5, 10);
            one.Rect = rect;
			...
			if(m_form.T.Length > 0)
			{
				...
				if(fontformat != null)
				{
					...
					if(font != null)
					{
						...
						Render.DrawTextEx(AbsRect, m_form.T, font, fontformat.GetEffect(m_state), fontformat.state[m_state].effcolor);
					}
				}
			}
		}
		...
		{
			SetBrush(1);
			XmlElement sub = doc.CreateElement("ButtonText");
