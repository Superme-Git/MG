...
{
	public class CComponentButtonRadio : CComponent
	{
		private readonly CFormMain m_mainform;
		private readonly CFormButtonRadio m_form;
		...
		public CComponentButtonRadio(CComponentDesktop desktop, CFormMain mainform, string name) : base(desktop)
		{
			...
			m_form = new CFormButtonRadio(m_mainform, this);
		...
		{
			CComponentButtonRadio one = new CComponentButtonRadio(Desktop, m_mainform, Name);
			one.Parent = Parent;
            Rectangle rect = Rect;
            rect.Offset(5, 10);
            one.Rect = rect;
			...
			one.Form.GroupID = m_form.GroupID;
		...
		public override void LoadXML(XmlElement elem)
		{
			...
			m_form.GroupID = GetAttributeInt(elem, "groupid");
