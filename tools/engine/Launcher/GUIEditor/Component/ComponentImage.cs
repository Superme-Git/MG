...
namespace GUIEditor2
{
	public class CComponentImage : CComponent
	{
		...
		public CComponentImage(CComponentDesktop desktop, CFormMain mainform, string name) : base(desktop)
		{
			Brush.SetColor(Color.FromArgb(192, 192, 0, 192));
			Name = name;
			m_mainform = mainform;
			m_brush0 = Render.CreateBrush();
			m_brush1 = Brush;
			m_brush2 = Render.CreateBrush();
			m_form = new CFormImage(m_mainform, this);
		...
		{
			CComponentImage one = new CComponentImage(Desktop, m_mainform, Name);
			one.Parent = Parent;
            Rectangle rect = Rect;
            rect.Offset(5, 10);
            one.Rect = rect;            
		...
		{
			SetBrush(1);
			XmlElement sub = doc.CreateElement("Image");
