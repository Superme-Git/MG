    ...
    public class DefineManager
    {
        ...
        public bool loadDefFromFile(string xmlDefFileName)
        {
            ...
            if(spaceName=="fire"&&xn.ChildNodes.Count>0)
            {
                ...
                if(spaceName=="gsp"&&xn.ChildNodes.Count>0)
                {
                    xn = xn.ChildNodes[0];
                    spaceName = ((XmlElement)xn).GetAttribute("name");
                }
                ...
                { 
                    ...
                    for (int i = 0; i < childXn.ChildNodes.Count; i++)
                    {
                        ...
                        if (nodeType.Name == "XmlElement")
                        {
                            ...
                            {
                                List<string> subCols = MainWindow.stovs(beanCol.fromCol, ',');
                    ...
                    else if (beanDef.genXml == "client")
                    {
                        ...
                        {
                            MainWindow.Error("在解析BEAN<" + beanDef.baseClass + ">时发现base class=" + beanDef.baseClass + ",目前不支持客户端BEAN定义基类！");
                            return false;
                        }
                    }
                    ...
                    {
                        MainWindow.Error("BEAN格式错误 " + beanDef.name + "的genXml " + beanDef.genXml + " 不支持！");
                        return false;
                    }
                    try
                    {
                    ...
                    {
                        MainWindow.Error("已经读入名字为" + beanDef.name + "的Bean,请检查相关定义 异常信息=" + e.Message);
                        return false;
                    }

                    List<string> xlsFileNames = MainWindow.stovs(beanDef.from, ',');
                    for (int i = 0; i < xlsFileNames.Count; i++)
                    {
                        string xlsFileName = MainWindow.ConverPath(xlsFileNames[i]);
        ...
        public bool GetBeanFromBeanName(out BeanDef beanDef,string type)
        {
            ...
            List<string> vsType = MainWindow.stovs(type,'.');
            if (vsType.Count > 1)
            {
