...
public:
    ...
    * creating the frame.*/
	...
	// 
	bool isShowPixelUnit(){ return m_windowPixelUnit;  }

	// by bayaer, 移动只修改offset
	bool isMoveOnlyOffset(){ return m_moveOnlyOffset; }
private:
    ...
    enum {
		...
		ID_VIEW_1280x720, // by bayaer,  1280 * 720
		ID_VIEW_1136x640, // by bayaer,  
		ID_VIEW_1334x750, // by bayaer,  
		ID_VIEW_960x640, // by bayaer,  
		ID_VIEW_1080x720, // by bayaer,  
        ...
        ID_VIEW_SET_GRID,
        ID_VIEW_SET_FONT,
		ID_VIEW_SET_WINDOWUNIT,
		ID_VIEW_SET_MOVE_ONLY_OFFSET,		// by bayaer,
        ID_SKINS,
        ID_LOCK_SELECTION,
		ID_SAVE_LUA_CODE	// by liugeng
    };
	...
	// 窗口单位是否以像素形式显示.默认是缩放方式.
	bool					m_windowPixelUnit;

	// by bayaer, 是否移动窗口只改变offset
	bool					m_moveOnlyOffset;
    ...
    */
	...
	void On1280x720(wxCommandEvent& event);		// by bayaer
	void On1136x640(wxCommandEvent& event);		// by bayaer
	void On1334x750(wxCommandEvent& event);		// by bayaer
	void On960x640(wxCommandEvent& event);		// by bayaer
	void On1080x720(wxCommandEvent& event);		// by bayaer
	//
	...
	void OnSetWindowUnit(wxCommandEvent& event);
	// by bayaer, 移动只改offset
	void OnSetMoveOnlyOffset(wxCommandEvent& event);
	void OnUpdateSetMoveOnlyOffset(wxUpdateUIEvent& event);
	...
	void OnSaveLuaCode(wxCommandEvent& event);
};
