...
public:
	...
	// 设置以像素为显示单位.
	void SetWindowShowUnit(bool bPixelUnit);
...
private:
    ...
    */
	...
	//
	wxString getFakePropertyPixelValue(const wxString& propertyName, int volume);

	//
	wxString getFakePropertyScaleValue(const wxString& property, int pixelSize);

	bool checkPropertiesForLua(const CEGUI::String &propertyName);
};
