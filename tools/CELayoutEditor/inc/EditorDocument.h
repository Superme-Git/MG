...
public:
	struct UndoSegment
	{
		...
		UndoSegment(int type, Selection* sel, const CEGUI::String& name, const CEGUI::String& property)
		{
			nType = type;
			pSelect = sel;
			aName = name;
		...
		UndoSegment(int type, Selection* sel, float l, float t, float r, float b)
		{
			nType = type;
			pSelect = sel;
			left = l;
    ...
    */
	...
	/** @Returns container with undo action.*/
	std::list<UndoSegment*> GetUndoList() const
	{
		return m_UndoList;
	}

	/** @Returns container with redo action.*/
	std::list<UndoSegment*> GetRedoList() const
	{
		return m_RedoList;
	}
	 ...
	 * @param addUndoList is need add to undo list.
	 * @param allowEmpty denotes whether an empty values is allowed.
     */
	bool SetProperty(Selection* selection, const CEGUI::String& aName, const CEGUI::String& newValue, bool addUndoList = true, const bool allowEmpty = false);
	...
	/** Undo the currently action from the GUI system.*/
	void Undo();

	/** Redo the currently action from the GUI system.*/
	void Redo();

	void AddUndoSeg(UndoSegment* pSeg);
	void AddRedoSeg(UndoSegment* pSeg);
	...
	/** Generate lua code .by liugeng
	*/
	void SaveLuaCode(const char * filePath);
...
private:
    ...
    * @todo needs to be moved to the VIEW*/
	...
	/** Generate lua code .by liugeng
	 */
	void WriteLuaMember(CEGUI::Window *win, std::ofstream &out, std::vector<std::pair<CEGUI::String, CEGUI::String>> &withEventClicked, bool isDialog);
	void GenDialogLuaCode(std::ofstream &out);
	void GenWidgetLuaCode(std::ofstream &out);
};
