...
// by bayaer
EVT_MENU(ID_VIEW_1280x720, EditorFrame::On1280x720)
EVT_MENU(ID_VIEW_1136x640, EditorFrame::On1136x640)
EVT_MENU(ID_VIEW_1334x750, EditorFrame::On1334x750)
EVT_MENU(ID_VIEW_960x640, EditorFrame::On960x640)
EVT_MENU(ID_VIEW_1080x720, EditorFrame::On1080x720)
//
...
//
EVT_UPDATE_UI(wxID_UNDO, EditorFrame::OnUpdateUndo)
EVT_MENU(wxID_UNDO, EditorFrame::OnUndo)
//
EVT_UPDATE_UI(wxID_REDO, EditorFrame::OnUpdateRedo)
...
EVT_MENU(ID_VIEW_SET_WINDOWUNIT, EditorFrame::OnSetWindowUnit)
// by bayaer, 
//刷新一下状态 EVT_UPDATE_UI(wxID_CUT, EditorFrame::OnUpdateEditCut)
EVT_MENU(ID_VIEW_SET_MOVE_ONLY_OFFSET, EditorFrame::OnSetMoveOnlyOffset)
EVT_UPDATE_UI(ID_VIEW_SET_MOVE_ONLY_OFFSET, EditorFrame::OnUpdateSetMoveOnlyOffset)
...
EVT_MENU(ID_SAVE_LUA_CODE, EditorFrame::OnSaveLuaCode)
...
m_windowPixelUnit(true),
m_moveOnlyOffset(true),
...
void EditorFrame::AttachMenubar()
{
    ...
    // Edit items
	wxClassInfo * pClass =  mEditMenu->GetClassInfo();
    ...
    (void)mEditMenu->Append(wxID_REDO, wxT("&Redo\tCtrl+Y"), wxT("Redo the last action."));
	...
	(void)viewMenu->Append(ID_VIEW_1280x720, wxT("Make exactly 1280 x 720"), wxT("Convenience resizer for 1280 x 720 (wide)."));	//by bayaer, 
	(void)viewMenu->Append(ID_VIEW_1136x640, wxT("Make exactly 1136 x 640"), wxT("Convenience resizer for 1136 x 640 (wide)."));	//by bayaer, 
	(void)viewMenu->Append(ID_VIEW_1334x750, wxT("Make exactly 1334 x 750"), wxT("Convenience resizer for 1334 x 750 (wide)."));	//by bayaer, 
	(void)viewMenu->Append(ID_VIEW_960x640, wxT("Make exactly 960 x 640"), wxT("Convenience resizer for 960 x 640 (wide)."));	//by bayaer, 
	(void)viewMenu->Append(ID_VIEW_1080x720, wxT("Make exactly 1080 x 720"), wxT("Convenience resizer for 1080 x 720 (wide)."));	//by bayaer, 	
	...
	(void)viewMenu->Append(ID_VIEW_SET_WINDOWUNIT, wxT("Set Pixel Unit..."));
	// by bayaer
	(void)viewMenu->Append(ID_VIEW_SET_MOVE_ONLY_OFFSET, wxT("Set Move Only Offset..."));
...
void EditorFrame::AttachToolbar()
{
	...
	(void)toolBar->AddTool(wxID_UNDO, wxBitmap(copy_xpm), wxT("undo the last action"));
	(void)toolBar->AddTool(wxID_REDO, wxBitmap(paste_xpm), wxT("redo the last action"));
	(void)toolBar->AddSeparator();

    (void)toolBar->AddTool(wxID_ABOUT, wxBitmap (help_xpm), wxT("About"));

	(void)toolBar->AddTool(ID_SAVE_LUA_CODE, wxBitmap(save_xpm), wxT("Save Lua Code"));
...
{
	GetClientSize(&mClientWidth, &mClientHeight);
    ...
    //canvas->changeSize(mClientWidth + BORDER_SIZE, mClientHeight + BORDER_SIZE);
	canvas->changeSize(mClientWidth, mClientHeight);
...
void EditorFrame::InitializeCEGUI()
{
...
#ifdef _DEBUG
    CEGUI::System::setDefaultImageCodecName("SILLYImageCodec_d");
#else
	...
	CEGUI::ResourceProvider *pResourceProvider = new CEGUI::PFSResourceProvider;
	m_GUISystem = &CEGUI::System::create(*m_GUIRenderer, pResourceProvider);
	...
	while (!it.isAtEnd())
	{
		std::string textureName = (*it)->getTextureName().c_str();
		char sDrive[MAX_PATH];
		char sDir[MAX_PATH];
		char sFileName[MAX_PATH];
		char sExt[MAX_PATH];
		_splitpath_s((const char*)textureName.c_str(), sDrive, MAX_PATH, sDir, MAX_PATH, sFileName, MAX_PATH, sExt, MAX_PATH);
		textureName = sFileName;
		textureName += sExt;
		CEGUI::Texture& texture = CEGUI::System::getSingleton().getRenderer()->createTexture(textureName, "imagesets", 0, true);
...
void EditorFrame::InitializePaths()
{
	...
	CEGUI::PFSResourceProvider* const rp =
		wx_static_cast(CEGUI::PFSResourceProvider*, CEGUI::System::getSingleton().getResourceProvider());
...
bool EditorFrame::GetFileNames(const wxString& path, const wxString& wildcard, wxArrayString& filenames) const
{
	...
	if (!path.IsEmpty())
	{	// Find all layouts
		wxString cwd = wxGetCwd() + path;
		wxDir::GetAllFiles(cwd, &filenames, wildcard, wxDIR_FILES);
	}
...
void EditorFrame::LoadData() const
{
	...
	if (GetFileNames(path, wxT("*.scheme"), filenames))
	{
		...
		size_t nCnt = filenames.GetCount();
...
void EditorFrame::OnResize(wxSizeEvent& WXUNUSED(event)) 
{
	...
	if (mClientWidth == clientWidth && mClientHeight == clientHeight)
		return;
    ...
    if (m_renderCanvas)
    {
		...
		m_renderCanvas->changeSize(mClientWidth, mClientHeight);
    }
    ...
    if (m_GUISystem)
    {
        ...
        m_GUISystem->notifyDisplaySizeChanged(
			CEGUI::Size(wx_static_cast(float, mClientWidth),
			wx_static_cast(float, mClientHeight)));
		Options::GetInstancePtr()->SetCurrentResolution(mClientWidth, mClientHeight);
    }
...
void EditorFrame::OnOpenCustom(wxCommandEvent& event)
{
	...
	GetFileNames(path, wxT("*.layout"), filenames);
	for (index = 0; index < filenames.GetCount(); index++)
	{
		//filenames[index]
	...
	{	// Sort, because we appended from two arrays
		...
		//
		//// by bayaer, 设置到当前位置
		//chooser.SetSelection(currentID);
		...
		//}
		DialogSelect *addWindow = new DialogSelect(this, choices);
		//wxAnyChoiceDialog chooer(
		...
		if (addWindow->ShowModal() == wxID_OK)
		{
			...
			wxGetApp().GetDocManager()->CreateDocument(addWindow->GetLayout(), wxDOC_SILENT);
		}
...
//------------------------------------------------------------------------
// by bayaer, 
void EditorFrame::On1280x720(wxCommandEvent& WXUNUSED(event))
{
	Resize(1280, 720); // Widescreen
}
void EditorFrame::On1136x640(wxCommandEvent& WXUNUSED(event))
{
	Resize(1136, 640); // Widescreen
}
void EditorFrame::On1334x750(wxCommandEvent& WXUNUSED(event))
{
	Resize(1334, 750); // Widescreen
}
void EditorFrame::On960x640(wxCommandEvent& WXUNUSED(event))
{
	Resize(960, 640); // Widescreen
}
void EditorFrame::On1080x720(wxCommandEvent& event)		// by bayaer
{
	Resize(1080, 720); // Widescreen
}
...
//设置窗口单位.
void EditorFrame::OnSetWindowUnit(wxCommandEvent& event)
{
	m_windowPixelUnit = !m_windowPixelUnit;
	m_dialogMain->SetWindowShowUnit(m_windowPixelUnit);
}


// by bayaer, 移动窗口只移动 offset 
void EditorFrame::OnSetMoveOnlyOffset(wxCommandEvent& event)
{
	m_moveOnlyOffset = !m_moveOnlyOffset;
	// 这里不用设置 dialogMain, 直接通过单键获取
}

void EditorFrame::OnUpdateSetMoveOnlyOffset(wxUpdateUIEvent& event)
{
	//event.Check(m_moveOnlyOffset);
	event.Enable(m_document && m_document->GetCopiedSelection()->Size() > 0);
}
...
{
	event.Enable(m_document && m_document->GetUndoList().size() > 0);
}
...
{
	event.Enable(m_document && m_document->GetRedoList().size() > 0);
}
...
//------------------------------------------------------------------------
void EditorFrame::OnSaveLuaCode(wxCommandEvent& event)
{
	CEGUI::Window* window = m_document->GetActiveLayout();
	if (window->GetLuaClassName().empty())
	{
		LogDebug(wxT("Lua class name is empty"));
		//CEGUI::WindowManager::getSingletonPtr()->DEBUG_dumpWindowNames("");
#if USE_MODAL_MESSAGE_BOXES
		(void)ModalMessageBox(wxT("Lua class name is empty, please set the property \"LuaClassName\" to root window and try again"), wxT("Not Complete"), wxOK);
#endif
		return;
	}

	wxString currentDirectory = wxGetCwd();
	currentDirectory = currentDirectory.SubString(0, currentDirectory.find_last_of('\\')) + L"res\\script\\logic";
	wxFileDialog dialog(NULL, wxT("Save a Lua file"), currentDirectory, wxT(""), wxT("Lua files (*.lua;*.*)|*.lua;*.*"), wxSAVE);
	if (dialog.ShowModal() != wxID_OK)
	{
		return;
	}

	currentDirectory = dialog.GetPath();
	m_document->SaveLuaCode(currentDirectory.char_str());
}
