...
void DialogMain::CreateSupportedPropertyMap()
{
    ...
    while(semicolonTokenizer.HasMoreTokens())
    {
        ...
        } mnemonics[] =
        {
			...
			{ wxT("itemcell_style")		,  ITEMCELL_STYLE_TYPE   }
        };
...
wxString DialogMain::getFakePropertyPixelValue(const wxString& propertyName,int pixelSize)
{
	// by yang
	if (wxGetApp().GetMainFrame()->isShowPixelUnit())
	{
		wxString value = GetFakePropertyCommonValue(propertyName);
		float pixelPos = ceil(_wtof(value.c_str())*pixelSize);
		wchar_t text[32];
		swprintf_s(text, 32, L"%d", (int)pixelPos);
		return text;
	}
	else
		return GetFakePropertyCommonValue(propertyName);
}
...
void DialogMain::AddFakeProperties()
{
	...
	wxGetApp().GetMainFrame()->GetClientSize(&clientWidth, &clientHeight);
	// by bayaer, 使用父窗口坐标
	int clientPixelWidth = m_currentWindow->getParentPixelWidth();
	int clientPixelHeight = m_currentWindow->getParentPixelWidth();

    AddPropertyRow(new wxGridCellTextEditor(), NAME, GetFakePropertyCommonValue(NAME), multiSelect, FAKE_PROPERTY_COLOR); 
	AddPropertyRow(new wxGridCellFloatEditor(), LEFT_SCALE, getFakePropertyPixelValue(LEFT_SCALE, clientPixelWidth), multiSelect || locked, FAKE_PROPERTY_COLOR);
	...
	AddPropertyRow(new wxGridCellFloatEditor(), TOP_SCALE, getFakePropertyPixelValue(TOP_SCALE, clientPixelHeight), multiSelect || locked, FAKE_PROPERTY_COLOR);
	...
	AddPropertyRow(new wxGridCellFloatEditor(), RIGHT_SCALE, getFakePropertyPixelValue(RIGHT_SCALE, clientPixelWidth), multiSelect || locked, FAKE_PROPERTY_COLOR);
	...
	AddPropertyRow(new wxGridCellFloatEditor(), BOTTOM_SCALE, getFakePropertyPixelValue(BOTTOM_SCALE, clientPixelHeight), multiSelect || locked, FAKE_PROPERTY_COLOR);
...
//------------------------------------------------------------------------
bool DialogMain::checkPropertiesForLua(const CEGUI::String &propertyName)
{
	if (propertyName == "LuaClass" ||
		propertyName == "LuaForDialog" ||
		propertyName == "LuaMemberName" || 
		propertyName == "LuaEventOnClicked"||
		propertyName == "CloseIsHide")
	{
		return false;
	}
	return true;
...
void DialogMain::AddRealProperties(const bool mapSkins)
{
	...
	// by liugeng. add some properties for generate lua code
	Selection::Boxes &boxes = m_document->GetSelection()->GetAllBoxes();
	if (!boxes.empty())
	{
		CEGUI::Window *wnd = boxes.front().GetWindow();
		// by liugeng. If the boxed window is root layout, add "Class" property
		if (wnd == m_document->GetActiveLayout())
		{
			AddPropertyRow("LuaClass", wnd->getProperty("LuaClass"));
			AddPropertyRow("LuaForDialog", wnd->getProperty("LuaForDialog"));
			AddPropertyRow("LuaMemberName", wnd->getProperty("LuaMemberName"));
			AddPropertyRow("CloseIsHide", wnd->getProperty("CloseIsHide"));
		}
		else
		{
			AddPropertyRow("LuaMemberName", wnd->getProperty("LuaMemberName"));
			if (wnd->getType().find("Button") != CEGUI::String::npos)
			{
				AddPropertyRow("LuaEventOnClicked", wnd->getProperty("LuaEventOnClicked"));
			}
		}
	}
    ...
    for(; current != commonProperties.end(); ++current)
    {
		...
		// by liugeng. if it's lua used property, don't auto add it
		if (checkPropertiesForLua((*current).first))
		{
...
void DialogMain::AddPropertyRow(const CEGUI::String& propertyName, const CEGUI::String& propertyValue, const wxColour& textColor/* =*wxBLACK */, const wxColour& backgroundColor /* =*wxWHITE */, const wxColour& lockedBackgroundColor /* =*wxLIGHT_GREY */)
{
    ...
    if (cellEditor)
	{
		AddPropertyRow(cellEditor, StringHelper::ToWXString(propertyName), StringHelper::ToWXString(propertyValue.c_str()), false, textColor, backgroundColor, lockedBackgroundColor);
    }
}
...
wxGridCellEditor* DialogMain::GetPropertyEditor(const CEGUI::String& propertyName)
{
	...
	case DialogMain::ITEMCELL_STYLE_TYPE:
		{
			static const int ITEMCELL_STYLE_CHOICE_COUNT = 2;
			const wxString itemcell_styleChoices[ITEMCELL_STYLE_CHOICE_COUNT] = { wxT("IconInside"), wxT("IconExtend")};
			return new wxGridCellChoiceEditor(ITEMCELL_STYLE_CHOICE_COUNT, itemcell_styleChoices);
		}
    default:
        // Unsupported
...
wxString DialogMain::getFakePropertyScaleValue(const wxString& property, int pixelSize)
{
	if (wxGetApp().GetMainFrame()->isShowPixelUnit())
	{
		float pixelPos = _wtof(property.c_str());
		float scale = (float)pixelPos / (float)pixelSize;
		char text[32];
		sprintf(text, "%f", scale);
		wchar_t wtext[32];
		MultiByteToWideChar(CP_ACP, 0, text, 32, wtext, 32);
		return wtext;
	}
	else
		return property;
...
// 		newWindowArea.d_min.d_x.d_scale = StringHelper::WXStringToFloat(getFakePropertyScaleValue(m_grid->GetCellValue(1, COLUMN_VALUE),clientWidth));
// 		newWindowArea.d_min.d_x.d_offset = StringHelper::WXStringToFloat(m_grid->GetCellValue(2, COLUMN_VALUE));
// 		newWindowArea.d_min.d_y.d_scale = StringHelper::WXStringToFloat(getFakePropertyScaleValue(m_grid->GetCellValue(3, COLUMN_VALUE),clientHeight));
// 		newWindowArea.d_min.d_y.d_offset = StringHelper::WXStringToFloat(m_grid->GetCellValue(4, COLUMN_VALUE));
// 		newWindowArea.d_max.d_x.d_scale = StringHelper::WXStringToFloat(getFakePropertyScaleValue(m_grid->GetCellValue(5, COLUMN_VALUE),clientWidth));
// 		newWindowArea.d_max.d_x.d_offset = StringHelper::WXStringToFloat(m_grid->GetCellValue(6, COLUMN_VALUE));
// 		newWindowArea.d_max.d_y.d_scale = StringHelper::WXStringToFloat(getFakePropertyScaleValue(m_grid->GetCellValue(7, COLUMN_VALUE),clientHeight));
// 		newWindowArea.d_max.d_y.d_offset = StringHelper::WXStringToFloat(m_grid->GetCellValue(8, COLUMN_VALUE));
		
		// by bayaer, 上面注掉， 根据 父窗口坐标计算
		int clientPixelWidth = m_currentWindow->getParentPixelWidth();
		int clientPixelHeight = m_currentWindow->getParentPixelWidth();
		newWindowArea.d_min.d_x.d_scale = StringHelper::WXStringToFloat(getFakePropertyScaleValue(m_grid->GetCellValue(1, COLUMN_VALUE), clientPixelWidth));
 		newWindowArea.d_min.d_x.d_offset = StringHelper::WXStringToFloat(m_grid->GetCellValue(2, COLUMN_VALUE));
		newWindowArea.d_min.d_y.d_scale = StringHelper::WXStringToFloat(getFakePropertyScaleValue(m_grid->GetCellValue(3, COLUMN_VALUE), clientPixelHeight));
 		newWindowArea.d_min.d_y.d_offset = StringHelper::WXStringToFloat(m_grid->GetCellValue(4, COLUMN_VALUE));
		newWindowArea.d_max.d_x.d_scale = StringHelper::WXStringToFloat(getFakePropertyScaleValue(m_grid->GetCellValue(5, COLUMN_VALUE), clientPixelWidth));
		newWindowArea.d_max.d_x.d_offset = StringHelper::WXStringToFloat(m_grid->GetCellValue(6, COLUMN_VALUE));
		newWindowArea.d_max.d_y.d_scale = StringHelper::WXStringToFloat(getFakePropertyScaleValue(m_grid->GetCellValue(7, COLUMN_VALUE), clientPixelHeight));
		newWindowArea.d_max.d_y.d_offset = StringHelper::WXStringToFloat(m_grid->GetCellValue(8, COLUMN_VALUE));
...
void DialogMain::OnChangeText(wxCommandEvent& WXUNUSED(event))
{
    ...
    if (m_document->GetSelection()->Size() == 1)
	{
		...
		{	// http://www.cegui.org.uk/mantis/view.php?id=278 
			// Allow empty string!
			(void)m_document->SetProperty(m_document->GetSelection(), "Text", StringHelper::ToCEGUIString(m_editText->GetValue()), true);
		}
...
void DialogMain::SaveProperty(int row)
{
	...
	if (m_lastSelectedWindow != NULL && m_lastSelectedRow >= 0)
	{
		...
		else
		{
			...
			(void)m_document->SetProperty(m_document->GetSelection(), StringHelper::ToCEGUIString(propertyName), StringHelper::ToCEGUIString(propertyValue));
...
void DialogMain::SetWindowShowUnit(bool bPixelUnit)
{
	//wxGridCellEditor* cellPosX = GetPropertyEditor("");
}
