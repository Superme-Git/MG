...
float GetAbsOffset(float number, int pixelSize)
{
	return number* pixelSize;
}

void SetLayoutAbsOffset(CEGUI::Window* layout)
{
	CEGUI::URect rect=layout->getArea();
	CEGUI::Size size=layout->getPixelSize();
	rect.d_min.d_x.d_offset += GetAbsOffset(rect.d_min.d_x.d_scale,size.d_width);
	rect.d_min.d_y.d_offset += GetAbsOffset(rect.d_min.d_y.d_scale,size.d_height);
	rect.d_max.d_x.d_offset += GetAbsOffset(rect.d_max.d_x.d_scale,size.d_width);
	rect.d_max.d_y.d_offset += GetAbsOffset(rect.d_max.d_x.d_scale,size.d_height);
	rect.d_min.d_x.d_scale = rect.d_min.d_y.d_scale = rect.d_max.d_x.d_scale = rect.d_max.d_y.d_scale = 0.0f;
	layout->setArea(rect);
	for (int i = 0; i < layout->getChildCount(); i++)
	{
		SetLayoutAbsOffset(layout->getChildAtIdx(i));
	}
}
...
bool EditorDocument::OnOpenDocument(const wxString& fileName)
{
	...
	wxString path = wxGetCwd() + options->GetPathToLayouts();
...
CEGUI::Window* EditorDocument::AddWindow (CEGUI::Window* aParentWindow, const CEGUI::String& aType, CEGUI::String aName) 
{
    ...
    while ( CEGUI::WindowManager::getSingleton().isWindowPresent(newName) )
    {
		...
		newName = aName + StringHelper::ToCEGUIString((const wxString)p_chNamePostfix);
    }
...
void EditorDocument::InitNewWindow (CEGUI::Window* aNewWindow, CEGUI::Window* aParentWindow)
{
	...
	{	// Menus need some specific handling...
		...
		{   // Set initial (relative) position in the usual top-left to bottom-right apprauch
			...
			aNewWindow->setPosition(CEGUI::UVector2(CEGUI::UDim(0.0f, newStartXY), CEGUI::UDim(0.0f, newStartXY)));
			aNewWindow->setSize(CEGUI::UVector2(CEGUI::UDim(0.0f, 1.0f / ADD_WINDOW_HELPER), CEGUI::UDim(0.0f, 1.0f / ADD_WINDOW_HELPER)));
		}
	}
	else
...
void EditorDocument::InitTabControl (CEGUI::Window* aTabControl)
{
    ...
    if (aTabControl->testClassName("TabControl"))
    {
        ...
        for (int i=0; i<TAB_COUNT; i++)
        {
			...
			// Create the new tab
			newTab = CEGUI::WindowManager::getSingleton().createWindow("DefaultWindow", controlName + StringHelper::ToCEGUIString((const wxString)tabIndex));
			InitNewWindow(newTab, aTabControl);
			newTab->setText(StringHelper::ToCEGUIString((const wxString)tabCaptions[i]));    // Overwrite default caption.
...
//------------------------------------------------------------------------
bool EditorDocument::SetProperty(Selection* selection, const CEGUI::String& aName, const CEGUI::String& newValue, bool addUndoList, const bool allowEmpty)
{
    bool bChanges = false;
	CEGUI::String oldValue;
#if defined(WIN32) && defined(_DEBUG) && CHECK_STACK_TRACES
	std::wstring strName(aName.length(), 0);
	std::wstring strValue(newValue.length(), 0);
	CEGUI::String::size_type i = 0;
	for (i = 0; i != aName.length(); ++i){
		strName[i] = aName[i];
	}
	for (i = 0; i != newValue.length(); ++i){
		strValue[i] = newValue[i];
	}
#endif
	...
	Selection::Boxes& boxes = selection->GetAllBoxes();
    for (Selection::Boxes::iterator boxIt = boxes.begin(); boxIt != boxes.end(); ++boxIt)
    {
        ...
        if (boxIt->GetWindow()->isPropertyPresent(aName)) 
        {
            ...
            if (boxIt->GetWindow()->getProperty(aName) != newValue)
            {
				...
				oldValue = boxIt->GetWindow()->getProperty(aName);
...
#if defined(WIN32) && defined(_DEBUG) && CHECK_STACK_TRACES
		std::wstring strOldValue(oldValue.length(), 0);
		for (i = 0; i != oldValue.length(); ++i){
			strOldValue[i] = oldValue[i];
		}
#endif
		Selection*	pSelect = new Selection;
		*pSelect = *selection;
		UndoSegment* undoSeg = new UndoSegment(1, pSelect, aName, oldValue);
		if (addUndoList)
		{
			AddUndoSeg(undoSeg);
		}
...
//------------------------------------------------------------------------
void EditorDocument::WriteLuaMember(CEGUI::Window *win, std::ofstream &out, std::vector<std::pair<CEGUI::String, CEGUI::String>> &withEventClicked, bool isDialog)
{
	//write member to lua file by parent-and-child relationship
	if (!win->GetLuaMemberName().empty())
	{
		if (win->getType() == "DefaultWindow")
		{
			if (isDialog)
			{
				out << "\tself." << win->GetLuaMemberName().c_str() << " = winMgr:getWindow(\"" << win->getName().c_str() << "\")\n";
			}
			else
			{
				out << "\tself." << win->GetLuaMemberName().c_str() << " = winMgr:getWindow(prefixstr .. \"" << win->getName().c_str() << "\")\n";
			}
		}
		else
		{
			CEGUI::String typeName = CEGUI::WindowFactoryManager::getSingletonPtr()->getFalagardMappingForType(win->getType()).d_baseType;
			if (typeName == "DefaultWindow")
			{
				if (isDialog)
				{
					out << "\tself." << win->GetLuaMemberName().c_str() << " = winMgr:getWindow(\"" << win->getName().c_str() << "\")\n";
			...
			{
				//OutputDebugStringA(typeName.c_str());
				int pos = typeName.find_last_of('/');
				if (pos != CEGUI::String::npos)
				{
					typeName = typeName.substr(pos + 1, typeName.length());
				}
				if (isDialog)
				{
					out << "\tself." << win->GetLuaMemberName().c_str() << " = CEGUI.to" << typeName.c_str() << "(winMgr:getWindow(\"" << win->getName().c_str() << "\"))\n";
				}
				else
				{
					out << "\tself." << win->GetLuaMemberName().c_str() << " = CEGUI.to" << typeName.c_str() << "(winMgr:getWindow(prefixstr .. \"" << win->getName().c_str() << "\"))\n";
				}
			}
		}


		if (!win->GetLuaEventOnClicked().empty())
		{
			withEventClicked.push_back(std::make_pair(win->GetLuaMemberName(), win->GetLuaEventOnClicked()));
		}
	}

	if (win->getChildCount())
	{
		for (int i = 0; i < win->getChildCount(); i++)
		{
			CEGUI::Window *child = win->getChildAtIdx(i);
			WriteLuaMember(child, out, withEventClicked, isDialog);
		}
	}
}

//------------------------------------------------------------------------
void EditorDocument::GenDialogLuaCode(std::ofstream &out)
{
	CEGUI::Window *win = GetActiveLayout();
	CEGUI::String name = win->GetLuaClassName();

	out << "require \"logic.dialog\"\n\n";

	out << name.c_str() << " = {}\n"
		"setmetatable(" << name.c_str() << ", Dialog)\n"
		<< name.c_str() << ".__index = " << name.c_str() << "\n\n";

	out << "local _instance\n";
	out << "function " << name.c_str() << ".getInstance()\n"
		"\tif not _instance then\n"
		"\t\t_instance = " << name.c_str() << ":new()\n"
			"\t\t_instance:OnCreate()\n"
		"\tend\n"
		"\treturn _instance\n"
	"end\n\n";

	out << "function " << name.c_str() << ".getInstanceAndShow()\n"
		"\tif not _instance then\n"
		"\t\t_instance = " << name.c_str() << ":new()\n"
			"\t\t_instance:OnCreate()\n"
		"\telse\n"
	...
	out << "function " << name.c_str() << ".getInstanceNotCreate()\n"
		"\treturn _instance\n"
	"end\n\n";

	out << "function " << name.c_str() << ".DestroyDialog()\n"
	"\tif _instance then \n"
	...
	out << "function " << name.c_str() << ".ToggleOpenClose()\n"
		"\tif not _instance then\n"
		"\t\t_instance = " << name.c_str() << ":new()\n"
			"\t\t_instance:OnCreate()\n"
		"\telse\n"
	...
	out << "function " << name.c_str() << ".GetLayoutFileName()\n"
		"\treturn \"" << Options::GetInstancePtr()->GetCurrentLayout().char_str() << "\"\n"
	"end\n\n";

	out << "function " << name.c_str() << ":new()\n"
		"\tlocal self = {}\n"
		"\tself = Dialog:new()\n"
		"\tsetmetatable(self, " << name.c_str() << ")\n"
		"\treturn self\n"
	"end\n\n";

	out << "function " << name.c_str() << ":OnCreate()\n"
		"\tDialog.OnCreate(self)\n"
		"\tlocal winMgr = CEGUI.WindowManager:getSingleton()\n\n";
	
	std::vector<std::pair<CEGUI::String,CEGUI::String>> withEventClicked;
	//assign members
	WriteLuaMember(win, out, withEventClicked, true);
	out << "\n";

	//assign events
	std::vector<std::pair<CEGUI::String, CEGUI::String>>::iterator it = withEventClicked.begin();
	while (it != withEventClicked.end())
	{
		out << "\tself." << (*it).first.c_str() << ":subscribeEvent(\"Clicked\", " <<
			name.c_str() << "." << (*it).second.c_str() << ", self)\n";
		it++;
	}
	out << "\n";
	out << "end\n\n";

	it = withEventClicked.begin();
	while (it != withEventClicked.end())
	{
		out << "function " << name.c_str() << ":" << (*it).second.c_str() << "(args)\n\n"
			"end\n\n";
		it++;
	}

	out << "return " << name.c_str();
}

//------------------------------------------------------------------------
void EditorDocument::GenWidgetLuaCode(std::ofstream &out)
{
	CEGUI::Window *win = GetActiveLayout();
	CEGUI::String name = win->GetLuaClassName();
	...
	out << "setmetatable(" << name.c_str() << ", Dialog)\n"
		<< name.c_str() << ".__index = " << name.c_str() << "\n"
		<< "local prefix = 0\n\n";

	out << "function " << name.c_str() << ".CreateNewDlg(parent)\n"
		"\tlocal newDlg = " << name.c_str() << ":new()\n"
	...
	out << "function " << name.c_str() << ":OnCreate(parent)\n"
		"\tprefix = prefix + 1\n"
	...
	std::vector<std::pair<CEGUI::String, CEGUI::String>> withEventClicked;
	//assign members
	WriteLuaMember(win, out, withEventClicked, false);
	out << "\n";
...
//------------------------------------------------------------------------
void EditorDocument::SaveLuaCode(const char * filePath)
{
	std::ofstream luaFile;
	luaFile.open(filePath, std::ios::binary);
	if (luaFile.is_open())
	{
		CEGUI::Window *window = GetActiveLayout();
		if (window->GetLuaForDialog())
		{
			GenDialogLuaCode(luaFile);
		}
		else
		{
			GenWidgetLuaCode(luaFile);
		}
	...
	{
		LogError(wxT("EditorDocument::OnSaveDocument - Error: could not open a lua file for saving. Is it read-only?"));
	}
}

//------------------------------------------------------------------------
void EditorDocument::OnPreSave()
{
...
{
	UndoSegment* undoSeg = m_UndoList.back();
	if (undoSeg->nType == 0)
	{
		SelectionMover mover(undoSeg->pSelect);
		mover.UpdateWindowAreas(undoSeg->left, undoSeg->top, undoSeg->right, undoSeg->bottom, false);
	}
	else if (undoSeg->nType == 1)
	{
		SetProperty(undoSeg->pSelect, undoSeg->aName, undoSeg->aProperty, false);
	}
	m_UndoList.pop_back();
	...
	EditorDocument::Modify(true);
	gDialogMain->UpdateProperties();
}

//------------------------------------------------------------------------
void EditorDocument::Redo()
{
	UndoSegment* redoSeg = m_RedoList.back();
	if (redoSeg->nType == 0)
	{
		SelectionMover mover(redoSeg->pSelect);
		mover.UpdateWindowAreas(redoSeg->left, redoSeg->top, redoSeg->right, redoSeg->bottom, true);
	}
	else if (redoSeg->nType == 1)
	{
		SetProperty(redoSeg->pSelect, redoSeg->aName, redoSeg->aProperty, true);
	}
	m_RedoList.pop_back();
...
//------------------------------------------------------------------------
void EditorDocument::AddUndoSeg(UndoSegment* pSeg)
{
	if (m_UndoList.size() < Max_Undo_Num)
	{
		m_UndoList.push_back(pSeg);
	}
	else
	{
...
//------------------------------------------------------------------------
void EditorDocument::AddRedoSeg(UndoSegment* pSeg)
{
	if (m_RedoList.size() < Max_Undo_Num)
	{
		m_RedoList.push_back(pSeg);
	}
	else
	{
...
//------------------------------------------------------------------------
void EditorDocument::ClearUndoRedoList()
{
	std::list<UndoSegment*>::iterator it;
	for (it = m_UndoList.begin(); it != m_UndoList.end(); ++it)
	{
		delete *it;
	}
	m_UndoList.clear();
	for (it = m_RedoList.begin(); it != m_RedoList.end(); ++it)
	{
		delete *it;
	}
	m_RedoList.clear();
