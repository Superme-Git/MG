...
void Options::UseSettings()
{
	...
	// by bayaer, 获取绝对路径, 这里的代码有待确认
	char addressBuffer[512];
	memset( addressBuffer, 0, sizeof(addressBuffer) );
	getcwd(addressBuffer, sizeof(addressBuffer));

	char addressBufferA[512];

	//sscanf( addressBuffer,  );

	//std::string strAddress(addressBuffer);
	std::string strAddress("");

	// 删除  tools 5个字符串
	//std::string  strAddressA = strAddress.substr(0,  strAddress.length() - 5 );
    ...
    if (value != "")
    {
		...
		tempStr = strAddress + value;
		m_pathConfigs = wxConvLibc.cMB2WX(tempStr.c_str());
    ...
    if (value != "")
    {
		...
		tempStr = strAddress + value;
		m_pathFonts = wxConvLibc.cMB2WX(tempStr.c_str());
    ...
    if (value != "")
    {
		...
		tempStr = strAddress + value;
		m_pathImagesets = wxConvLibc.cMB2WX(tempStr.c_str());
    ...
    if (value != "")
    {
		...
		tempStr = strAddress + value;
		m_pathLookNFeels = wxConvLibc.cMB2WX(tempStr.c_str());
    ...
    if (value != "")
    {
		...
		tempStr = strAddress + value;
		m_pathScripts = wxConvLibc.cMB2WX(tempStr.c_str());
    ...
    if (value != "")
    {
		...
		tempStr = strAddress + value;
		m_pathSchemes = wxConvLibc.cMB2WX(tempStr.c_str());
    ...
    if (value != "")
    {
		...
		tempStr = strAddress + value;
		m_pathLayouts = wxConvLibc.cMB2WX(tempStr.c_str());
