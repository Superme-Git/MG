...
class Validator implements Visitor {
	...
	{
		ps.println(prefix + "public final boolean _validator_() {");
	...
	{
		return str.equals("com.locojoy.base.Octets") ||
			   str.equals("java.lang.String");
	}

	private static String UnboxingName(String type)
	{
		if(type.equals("Byte"))
			return "byte";
...
*/
	...
	//  octets   capacity=max
	//  string   capacity=max
	private String ParseString(String type)
	{
		String result = "";
		for(String node : validator.split(";"))
		{
			if(node.startsWith(PREFIX_CAPACITY))
			{
				...
				if(type == "java.lang.String")
					result = prefix + "if (" + varname + ".length() > " + valuestr + ") return false;" + LN;
				else
	...
	private String ParseArray(String valuetype)
	{
		...
		for(String node : validator.split(";"))
		{
			...
			else if(node.startsWith(PREFIX_VALUE))
			{
				...
				str_value = ParseRange("_v_." + UnboxingName(valuetype) + "Value()", valuetype, node.substring(PREFIX_VALUE.length()));
			}
			else if(!node.isEmpty())
				throw new RuntimeException("unknown prefix in '" + validator + "'");
		}
		...
		{
			result += prefix + "for (" + valuetype + " _v_ : " + varname + ")" + LN;
			if(str_value != null)
				result += prefix + "	if (" + str_value + ") return false;" + LN;
			else
				result += prefix + "	if (!_v_._validator_()) return false;" + LN;
		}
		return result;
	}

	//	map      capacity=max *key=[min,max] *value=[min,max]
	private String ParseMap(String keytype, String valuetype)
	{
		String str_capacity = null;
		String str_key = null;
		String str_value = null;
		...
		for(String node : validator.split(";"))
		{
			...
			else if(node.startsWith(PREFIX_KEY))
			{
				...
				str_key = ParseRange("_e_.getKey()." + UnboxingName(keytype) + "Value()", keytype, node.substring(PREFIX_KEY.length()));
			}
			else if(node.startsWith(PREFIX_VALUE))
			{
				if(str_value != null)
				...
				if(!IsBaseType(valuetype))
					throw new RuntimeException("not base type '" + valuetype + "' for value range in '" + node + "'");
				str_value = ParseRange("_e_.getValue()." + UnboxingName(valuetype) + "Value()", valuetype, node.substring(PREFIX_VALUE.length()));
			}
			else if(!node.isEmpty())
				throw new RuntimeException("unknown prefix in '" + validator + "'");
		}
		...
		{
			result += prefix + "for (java.util.Map.Entry<" + keytype + ", " + valuetype + "> _e_ : " + varname + ".entrySet()) {" + LN;
			if(str_key != null)
				result += prefix + "	if (" + str_key + ") return false;" + LN;
			else if(keyisbean)
				result += prefix + "	if (!_e_.getKey()._validator_()) return false;" + LN;
			if(str_value != null)
				result += prefix + "	if (" + str_value + ") return false;" + LN;
			else if(valueisbean)
				result += prefix + "	if (!_e_.getValue()._validator_()) return false;" + LN;
			result += prefix + "}" + LN;
		}
		return result;
	}
	...
	{
		ps.print(ParseBaseType("Byte"));	// BoxingNameJava不能转换byte类型
	}

	@Override
	public void visit(TypeInt type)
	{
		String valuetype = BoxingName.getName(type);
		ps.print(ParseBaseType(valuetype));
	}
	...
	{
		String valuetype = BoxingName.getName(type);
		ps.print(ParseString(valuetype));
	}
	...
	{
		String valuetype = BoxingName.getName(type.getValuetype());
		ps.print(ParseArray(valuetype));
	}
	...
	{
		String keytype = BoxingName.getName(type.getKeytype());
		String valuetype = BoxingName.getName(type.getValuetype());
		ps.print(ParseMap(keytype, valuetype));
	}
