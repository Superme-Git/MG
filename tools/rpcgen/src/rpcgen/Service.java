...
public class Service extends Naming {
	...
	public void make() throws Exception {
		...
		if (Main.isClient){
			...
			if (Main.isLuaPkg) {
				// 收集需要注册进lua的协议
				Set<Protocol> temp_protocols = new HashSet<Protocol>();
				for (Protocol protocol : protocols)
					if(protocol.getTolua() > 0 && protocol.getTolua()<3)
						temp_protocols.add(protocol);
				...
				new rpcgen.luapkg.Rpcgen(this).make();
				return;
			}
			...
			if (Main.isLua) {
				...
				new rpcgen.lua.Rpcgen(this).make();
				return;
			}
