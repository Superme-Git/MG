...
public class WavRecorder {
	...
	private static WavRecorder result = null;
	private static boolean bCanRecord = true;
	private static boolean bRecordingCompressed = false;
	
	@SuppressWarnings("deprecation")
	public static WavRecorder getInstance() {
		if (result == null)
		{
			if (bRecordingCompressed) {
				result = new WavRecorder(RECORDING_COMPRESSED, AudioSource.MIC,
						sampleRates[3], AudioFormat.CHANNEL_CONFIGURATION_MONO,
						AudioFormat.ENCODING_PCM_16BIT);
			} else {
				int sampleRatesIndex = 3;
				do {
					result = new WavRecorder(RECORDING_UNCOMPRESSED, AudioSource.MIC,
							sampleRates[sampleRatesIndex], AudioFormat.CHANNEL_CONFIGURATION_MONO,
							AudioFormat.ENCODING_PCM_16BIT);
				} while ((++sampleRatesIndex < sampleRates.length)
						&& !(result.getState() == WavRecorder.State.INITIALIZING));
			}
			bCanRecord = (result.getState() == WavRecorder.State.INITIALIZING);
		}
		return result;
	}
	 ...
	 * record chat
	 */
	public static void recordChat(String argPath) {
		result.setOutputFile(argPath);
		result.prepare();
		result.start();
	}
	 ...
	 */
	@SuppressWarnings("deprecation")
	private WavRecorder(boolean uncompressed, int audioSource, int sampleRate, int channelConfig, int audioFormat) {
		try {
			...
			if (rUncompressed)
			{
				...
				if (bufferSize < AudioRecord.getMinBufferSize(sampleRate, channelConfig, audioFormat)) {
					...
					framePeriod = bufferSize / (2 * bSamples * nChannels / 8); // Set frame period and timer interval accordingly
				}
		...
		if (state == State.RECORDING) {
			if (rUncompressed) {
				...
				try {
					...
					randomAccessWriter.seek(40); // Write size to Subchunk2Sizefield
