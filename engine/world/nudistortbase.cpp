...
{
	//不存在超过512宽高的img文件	
	NuclearTCT1Vertex DistortBase::s_vertexs[1089]; //33*33
	...
	bool DistortBase::render(Canvas* canvas, bool isAlpha, bool realRender)
	{
		if(NULL == m_pEB || !realRender)
			return false;
		...
		//SystemResourceManager* pSystemResoureManager = m_pEB->GetSystemResourceManager();
		...
		int nGameTime = m_pEB->GetGameTime();
		bNightFlag = nGameTime<FIRE_DAY_START_TIME || nGameTime>FIRE_NIGHT_START_TIME;
		...
		{ //绘制一张静态图片
			for(int i=0; i<cnt; i++) 
			{
				...
				rct += NuclearFPoint((float)(m_rct.left), (float)(m_rct.top)); //得到该分块绘制的屏幕位置
				rct.left -= vp.left;
				rct.right -= vp.left;
				rct.top -= vp.top;
				rct.bottom -= vp.top;
				...
				m_pEB->GetRenderer()->DrawPicture(param);
				...
				//	m_pEB->GetRenderer()->SetRenderEffect(XPRE_NULL);
				//	m_pEB->GetRenderer()->DrawPicture(param);
				//	m_pEB->GetRenderer()->SetRenderEffect(oldEffect);
				//}
			}
			return false;
		}
		...
		for(int i=0; i<cnt; i++) 
		{
			...
			const std::vector<NuclearFPoint>& fAmplitudes = m_effectParam.fAmplitudes;
	...
	//			nCols += 1;
			...
			rct += NuclearPoint(m_rct.left, m_rct.top); //得到该分块绘制的屏幕位置
			rct -= NuclearPoint(vp.left, vp.top);
			...
			for(int j=0; j<nRows-1; j++)
			{
 			...
 			m_pEB->GetRenderer()->SetAlphaBlendSrcType(XPBLEND_SRCALPHA);
 			m_pEB->GetRenderer()->SetAlphaBlendDstType(XPBLEND_INVSRCALPHA);
 			m_pEB->GetRenderer()->SetTextureColorBlendType(XPTOP_MODULATE);

			m_pEB->GetRenderer()->DrawIndexVerticeBuffer(handle, s_vertexs, s_index, nRows * nCols, nIndex);
...
//				XPRENDER_EFFECT oldEffect = m_pEB->GetRenderer()->GetRenderEffect();
//				m_pEB->GetRenderer()->SetRenderEffect(XPRE_NULL);
//				m_pEB->GetRenderer()->DrawIndexVerticeBuffer(lightHandle, s_vertexs, s_index, nRows * nCols, nIndex);
//				m_pEB->GetRenderer()->SetRenderEffect(oldEffect);
			}
		}
	...
	{
		m_effectParam.fPos += fTime*2*Nuclear::FIRE_PI/m_effectParam.fSpeed;
		int count = (int)(m_effectParam.fPos/(2.0f*Nuclear::FIRE_PI));
		m_effectParam.fPos -= count*2.0f*Nuclear::FIRE_PI;//------控制函数移动
