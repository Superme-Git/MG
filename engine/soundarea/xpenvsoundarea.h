...
namespace Nuclear 
{
	...
	class XPEnvSoundArea : public XPSoundArea
	{
	//自定义类型们
	public:
		struct SoundParam
	...
	private:
		EngineBase *m_pEB;
		unsigned int m_lastSetVolumeTick;
		SoundVet m_sound;
	...
	protected:
		void LoadSound(LJXML::LJXML_Node<>& node);
		void SaveSound(LJXML::LJXML_Node<>& node);
	...
	public:
		...
		virtual bool Update(const NuclearPoint& pos);
		virtual float GetAreaVolume(const NuclearPoint& pos) const = 0;
