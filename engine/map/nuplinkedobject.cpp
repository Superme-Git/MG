#include "nuplinkedobject.h"

namespace Nuclear
{

NuclearLinkedObject::NuclearLinkedObject(void)
{
	m_ptCentPos = NuclearPoint(); //plant的重心,(相对于主节点的左上角..)
	for(int i=0; i<4; i++)
		m_ptBasePos[i] = NuclearPoint();//plant的底座,----位置关系同上	
	m_vectorLinkedObjectInfo.clear();

	m_MainNodeID = 0;

	m_rctMaxBox = NuclearRect(-500, -500, 500, 500);
...
NuclearOStream& NuclearLinkedObject::marshal(NuclearOStream &os) const
{ //save
	os << (int)CURVER;
	...
	{
		//从Version_NotMountElement开始，直接写std::wstring
		os<<myIterator->m_strPicName;
...
const INuclearStream& NuclearLinkedObject::unmarshal(const INuclearStream &os)
{ //Load
	int nVer;
	os >> nVer;
	...
	{
		sPLANTPICINFO plantInfo;

		if (nVer < Version_NotMountElement)
		{
			/*os >> str_size;
...
//			ZeroMemory(str, str_size+2);
			...
			XFree(str);
			plantInfo.m_strPicName = plantInfo.m_strPicName;*/
			os >> plantInfo.m_strPicName;
		} else {
			os >> plantInfo.m_strPicName;
		}
		...
		if(plantInfo.m_LinkedObjectMoveParam.m_nDistortionType == XPPLANTSKEW)
		{
			...
			{
				NuclearPoint pt = plantInfo.m_rotateCent;
				if(plantInfo.m_LinkedObjectMoveParam.m_nSkewType == 0) //上切
				{
					plantInfo.m_rotateCent = NuclearPoint(0, plantInfo.m_nHeight/2);
				}
				else //下切
				{
					plantInfo.m_rotateCent = NuclearPoint(0, -plantInfo.m_nHeight/2);
