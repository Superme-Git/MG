#ifndef __Nuclear_generalpaticlesystem_H__
#define __Nuclear_generalpaticlesystem_H__
...
namespace Nuclear
{
	...
	{
	public:
		CGeneralParticleSystem(const NuclearHardRef<PSL>& RefPsl, const PSParameter& psParameter, IBaseRenderer *pRenderer,
	...
	public://编辑器专用...
		NuclearHardRef<PSTEXTUREINFOSTRUCT> GetPSTextureInfo() { return m_pRefPSTextureInfo; } //返回之后修改..
        ...
        virtual bool IsTextureEtc(){return m_pRenderer->IsTextureEtc(m_pRefPSTextureInfo->PsTextureInfo.handle);}

	private:
		//出于效率考虑--把std::vector<VERTEX> vertices;移到类成员变量里,取消了上面的模板函数
		bool Render_TCT(PictureHandle handle, bool bEndScene = true); //NuclearTCT1Vertex
		bool Render_UTCT(PictureHandle handle); //NuclearUTCT1Vertex
		...
		bool LoadFullPathPoint(); //---不同路径采样精度下会要修改psl的一些速度参数
	...
	private:
		...
		std::vector<NuclearFPoint> m_vectorFullPathPoint; //路径点---矢量字符路径和bezier曲线路径---在构造函数里面再来初始化这个vector
		...
		std::vector<NuclearTCT1Vertex> m_vertices_TCT;
		std::vector<NuclearUTCT1Vertex> m_vertices_UTCT;
