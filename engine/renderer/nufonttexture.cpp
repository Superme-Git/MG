...
CFontTexPool::TextureRes* CFontTexPool::AllocRenderTex( Renderer& renderer, int texLevel)
{
	...
	int len = RENDER_TEX_SIZE[texLevel] * RENDER_TEX_SIZE[texLevel] * 4;
	void* pData = malloc(len);
	memset(pData, 0, len);
	renderer.SetPictureData(hPic, pData, len);
	free(pData);
...
{
	CFontTextureMap::iterator it = m_fonttexture.find( hFontTexture);
...
{
	for ( CFontTextureMap::iterator it = m_fonttexture.begin(); it != m_fonttexture.end(); ++ it)
	{
		CFontTexture* pft = it->second;
		pft->Release();
		delete pft;
...
bool CFontTexture::SetPictureData( Renderer& renderer, const void *data, int size, const RECT* pRect)
{
	...
	for ( CFontTextureInfoVector::iterator it = m_resInfos.begin(); it != m_resInfos.end() && src_x >= 0; ++ it)
	{
		...
		{
			NuclearRect rect(src_x, src_y, src_x + pRect->right - pRect->left, src_y + pRect->bottom - pRect->top);
