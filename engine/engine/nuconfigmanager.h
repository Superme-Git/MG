#ifndef __Nuclear_CONFIGMANAGER_H__
#define __Nuclear_CONFIGMANAGER_H__
...
namespace Nuclear
{
	...
	public:
		// 创建纹理的池类型
		// @缓存，OnTimer更新
		NuclearPoolType GetCreateTexturePoolType() const;
	...
	public:
		...
		NuclearComputerConfigType GetSystemLevel() const { return m_sysLevel; }
	...
	public:
		...
		void SetControlFPS(NuclearWindowState type, int fps);//控制的FPS帧率，默认是60 60 30
		...
		NuclearSpriteTranslucentType GetSpriteTranslucentType() const { return m_spriteTranslucentType; }
		void SetSpriteTranslucentType(NuclearSpriteTranslucentType t) { m_spriteTranslucentType = t; }

		// 如果是XPSTT_ELEMENT_TRANSLUCENT，就是遮挡物Alpha；
		// 如果是XPSTT_SPRITE_TRANSLUCENT，就是精灵Alpha
		float GetMaskAlpha() const { return m_fMaskAlpha; }
		void SetMaskAlpha(float f) { m_fMaskAlpha = f;}

		// 是否绘制遮挡物底图（如果是整图其实就不需要绘制了），只在XPSTT_ELEMENT_TRANSLUCENT半透明方案生效
		bool IsRenderSolidMask() const { return m_bIsRenderSolidMask; }
		void SetRenderSolidMask(bool b) { m_bIsRenderSolidMask = b; }
	...
	private:
		// auto setting
		NuclearPoolType m_eCreateTexturePoolType;
		...
		bool m_bIsRenderSolidMask;
		float m_fMaskAlpha;

		// debug setting
		unsigned int m_maskConsoleInfo;
		unsigned int m_maskFramestat;
		NuclearColor m_framestatColor[XPFRAMESTAT_TYPE_COUNT];
		...
		NuclearSpriteTranslucentType m_spriteTranslucentType;
		NuclearComputerConfigType m_sysLevel;	//计算机的配置
