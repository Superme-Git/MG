...
namespace Nuclear
{
	...
	{
		//m_pRenderer->DrawLine(pos.x - 100, pos.y, pos.x + 100, pos.y, 0xFFFFFFFF);
	...
	bool ToolsTitleManager::TestTitleAlpha(EntitativeTitleHandle title, const Location &testLoc)
	{
		...
		{
			ToolsTitle &title = iter->second;
			NuclearRect rect = m_pEngine->GetWorld()->GetViewport();
	...
	void ToolsTitleManager::SetTitle(EntitativeTitleHandle handle, int slot, const std::wstring &title, int fontType, uFireCOLOR color)
	{
