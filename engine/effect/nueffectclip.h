#ifndef __Nuclear_EFFECTCLIP_H__
#define __Nuclear_EFFECTCLIP_H__

#include "nugeffect.h"
#include "nueffectkeytable.h"

namespace Nuclear
{
	class ToolsKPVector : public AbstractEffectClip::KP_VECTOR
	{
	public:
		void Load(LJXML::LJXML_Node<LJXML::Char>* pNode);
	};


	class ToolsSPVector : public AbstractEffectClip::SP_VECTOR
	{
	public:
		void Load(LJXML::LJXML_Node<LJXML::Char>* pNode);
	};
	...
	class EffectClip : public AbstractEffectClip
	{
	...
	public:
		...
		int Load(LJXML::LJXML_Node<LJXML::Char>* pNode, EngineBase* pEB, int flag = 0);

		void Release();

	public:
		// 等到3D特效也能放进来的时候，这个就要加一个参数了，NuclearFPoint的dir，现在先不加；干脆搞个Param吧
		int		Update( float curFrame, DWORD tickTime, const NuclearFPoint& parentPos, const NuclearFPoint& parentScale, 
			const float& parentRotation, const NuclearVector4& parentColor, NuclearDirection dir);

		void	Render(Canvas* canvas, bool realRender);
	...
	public:
		...
		virtual bool	GetPosKeyValue( unsigned int index, int& frame, NuclearFPoint& v) const
		{
		...
		virtual bool	GetScaleKeyValue( unsigned int index, int& frame, NuclearFPoint& v) const
		{
		...
		virtual bool	GetColorKeyValue( unsigned int index, int& frame, NuclearColor& v) const
		{
		...
		virtual bool	SetPosKeyValue( unsigned int index, const NuclearFPoint& v)
		{
		...
		virtual bool	SetScaleKeyValue( unsigned int index, const NuclearFPoint& v)
		{
		...
		virtual bool	SetColorKeyValue( unsigned int index, const NuclearColor& v)
		{
		...
		virtual int		GetPosKeyValue( int frame, NuclearFPoint& v) const
		{
		...
		virtual int		GetScaleKeyValue( int frame, NuclearFPoint& v) const
		{
		...
		virtual int		GetColorKeyValue( int frame, NuclearColor& v) const
		{
		...
		virtual unsigned int	AddPosKey( int frame, const NuclearFPoint& v)
		{
			return m_posTable.Insert( CPosKeyValue( (float)frame, v));
		}

		virtual unsigned int	AddScaleKey( int frame, const NuclearFPoint& v)
		{
		...
		virtual unsigned int	AddColorKey( int frame, const NuclearColor& v)
		{
		...
		virtual const IEffectClipControl* GetControl() const { return m_control;}

		virtual void Clone(EffectClip* p, EngineBase* pEB, int flag);
