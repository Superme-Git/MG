...
#include "../../../../dependencies/LJFilePack/LJFP_SMS4.h"
#include "../../../../dependencies/LJFilePack/LJFP_MiniZ.h"
...
int LJZDataToFile(unsigned char* pSrcData, unsigned int uiSrcDataLen, std::wstring wsDstFileName)
{
	...
	return CFileUtil::PutDataToFileByFS(pDstData, uiDstDataLen, wsDstFileName);
}
...
int LJZFileToData(std::wstring wsSrcFileName, unsigned char*& pDstData, unsigned int* uiDstDataLen)
{
	...
	CFileUtil::GetDataFromFileByFS(wsSrcFileName, pSrcData, uiSrcDataLen);
...
{
	std::string sZipFileName = ws2s(wsZipFileName);
	std::string sDstFileName = ws2s(wsDstFileName);
	std::string sComment = ws2s(wsComment);
...
int LJAddFileToZipFile(std::wstring wsZipFileName, std::wstring wsDstFileName, std::wstring wsSrcFileName, std::wstring wsComment, unsigned int uiCompressLevel)
{
	...
	std::ifstream FSSrc(ws2s(wsSrcFileName).c_str(), std::ios_base::binary);
	if (!FSSrc.is_open())
	{
		return -1;
	}
