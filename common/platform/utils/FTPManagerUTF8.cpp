...
FTP_API CFTPManager::Put2(const std::string &strRemoteFile, const std::string &strLocalFile)
{
    std::string strCmdLine;
    const unsigned long dataLen = FTP_DEFAULT_BUFFER;
    char strBuf[dataLen] = { 0 };
    ...
    std::ifstream FSSrc(strLocalFile.c_str(), std::ios_base::binary);
    if (!FSSrc.is_open())
    {
        return -1;
    }
    FSSrc.seekg(0, std::ios_base::end);
    unsigned int nSize = FSSrc.tellg();
    FSSrc.seekg(0, std::ios_base::beg);
    
    unsigned int nBlockCount = nSize / dataLen;
    unsigned int nLastBlockSize = nSize - nBlockCount * dataLen;
    
    for (unsigned int i = 0; i < nBlockCount; i++)
    {
        FSSrc.read(strBuf, dataLen);
        if (Send(data_fd, strBuf, dataLen) < 0)
        {
            Close(data_fd);
            FSSrc.close();
            return -1;
    ...
    if (nLastBlockSize > 0)
    {
        char* pLastBlockBuf = new char[nLastBlockSize];
        FSSrc.read(pLastBlockBuf, nLastBlockSize);
        //for (int i = 0; i < nLastBlockSize; i++)
        {
            if (Send(data_fd, pLastBlockBuf, nLastBlockSize) < 0)
            {
                Close(data_fd);
                FSSrc.close();
                delete[] pLastBlockBuf;
                return -1;
            }
        }
