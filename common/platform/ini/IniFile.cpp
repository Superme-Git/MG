...
*/
int IniFile::read_profile_string(const char *section, const char *key, char *value, int size, const char *default_value, const char *file)
{
...
*@return profile int value,if read fail, return default value
*/
int IniFile::read_profile_float(const char *section, const char *key, float default_value, const char *file)
{
	char value[32] = { 0 };
	if (!read_profile_string(section, key, value, sizeof(value), NULL, file))
	{
...
std::string IniFile::getCfgFilename()
{
#if defined(_OS_IOS)
...
#if defined(_ANDROID)
	std::string clientIniPath = rootDir + "/cfg/clientsetting_android.ini";
#elif defined(_OS_IOS)
	std::string clientIniPath = rootDir + "/cfg/clientsetting_ios.ini";
#elif defined(_WINDOW)
	std::string clientIniPath = rootDir + "/cfg/clientsetting_win.ini";
#else
	std::string clientIniPath = rootDir + "/cfg/clientsetting_wp.ini";
#endif
