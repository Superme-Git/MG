...
namespace FireNet {
	...
	void Compress::putbits(unsigned char *&buf, unsigned int val, unsigned int n, unsigned int& l)
	{
		l += n;
		*(unsigned int *)buf = FireNet::BYTE32Order(val << (32 - l)) | *buf;
		buf += l >> 3;
		l &= 7;
	}

	void Compress::putlit(unsigned char *&buf, unsigned int c, unsigned int& l)
	{
		if (c < 0x80)
			putbits(buf, c, 8, l);
		else
	...
	void Compress::putoff(unsigned char *&buf, unsigned int off, unsigned int& l)
	{
	...
	unsigned char* Compress::compress_block( unsigned char *obuf, size_t isize )
	{
	...
	Compress::Compress()
		: histptr(history), legacy_in(0)
	{
		for( int i = 0; i < 256; i++ )
			hash[i] = history;
	}

	Compress::Compress( const Compress& rhs)
		: histptr(history + (rhs.histptr - rhs.history)), legacy_in(rhs.legacy_in)
	{
		memcpy ( history, rhs.history, sizeof(history) );
		for ( int i = 0; i < 256; i++ ) hash[i] = history + ( rhs.hash[i] - rhs.history );
	}

	Octets& Compress::UpdateSelf(Octets &in)
	{
		Octets out;
	...
	Octets& Compress::Final(Octets &in)
	{
		if ( in.size()==0 && legacy_in==0 ) return in;
		Octets& out = UpdateSelf(in);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	bool Decompress::passbits(const unsigned int n)
	{
		l    += n;
		blen += n;
		if ( blen < blen_total )
	...
	unsigned int Decompress::fetch()
	{
		rptr += l >> 3;
		l &= 7;
		return FireNet::BYTE32Order(*(unsigned int *)rptr) << l;
	}

	void Decompress::lamecopy(unsigned char *dst, unsigned char *src, unsigned int len)
	{
		if ( dst - src > 3 )
		{
...
#pragma novector
	...
	Decompress::Decompress()
		: histptr(history), l(0)
	{}
	Decompress::Decompress( const Decompress& rhs)
		: histptr(history + (rhs.histptr - rhs.history)), l(rhs.l),
	...
	Octets& Decompress::UpdateSelf(Octets &in)
	{
