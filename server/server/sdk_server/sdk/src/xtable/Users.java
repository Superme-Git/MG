...
public class Users {
	...
	public static mkdb.TTableCache<Long, xbean.UserInfo> getCache() {
		return _Tables_.getInstance().users.getCache();
	}

	public static mkdb.TTable<Long, xbean.UserInfo> getTable() {
		return _Tables_.getInstance().users;
	}

	public static xbean.UserInfo select(Long key) {
		return getTable().select(key, new mkdb.TField<xbean.UserInfo, xbean.UserInfo>() {
			public xbean.UserInfo get(xbean.UserInfo v) { return v.toData(); }
		});
	}

	public static String selectPlat(Long key) {
		return getTable().select(key, new mkdb.TField<xbean.UserInfo, String>() {
