...
class Status_model extends MY_Model
{
	...
	public function get_byusers($roleids, $status_id, $limit)
	{
		...
		$status_list = $this->Status_model->getList($where, $order, 0, $limit, null, 600, false);
		
		return $status_list;
	}
	
	//根据用户ID获取微博列表
	public function get_byuser($roleid, $offset, $limit)
	{
		$where = array();
		$where['roleid'] = $roleid;
		$where['status'] = 0;
		$order = array('create_time' => 'desc');
		$status_list = $this->Status_model->getList($where, $order, $offset, $limit, null, 600, false);
