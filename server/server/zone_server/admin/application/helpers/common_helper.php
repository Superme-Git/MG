...
{
    return !empty($_SERVER['HTTP_REFERER']) ? trim($_SERVER['HTTP_REFERER']) : $default;
}
...
//分页函数
function page($count, $offset, $limit, $otherRoute = null)
{
	$ci = _get_object();
	$ci->load->library("pagination");
	$pageConfig = array();
	$pageConfig["base_url"] = base_url($ci->router->fetch_directory(). $ci->router->fetch_class() . '/'.$ci->router->fetch_method() . ($otherRoute !== null ? '/'.trim($otherRoute, '/') : ''));
...
function setSite($siteKey_ = '')
{
    ...
    log_message('error', __CLASS__.' '.__FUNCTION__.' succ '.json_encode(array($siteKey)));
    return $siteKey;
}
