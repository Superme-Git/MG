...
class MY_Model extends CI_Model
{
    ...
    public function getInfoByAttribute($where, $timeout = 300, $mount = true)
    {
    	if ($timeout < 1)
    	{
    ...
    {
    	$result = $this->db()->where($this->getPrimaryKey(), $primaryVal)->delete($this->getTable());
    	log_message('debug', __CLASS__ . ' ' . __FUNCTION__ . ' last_query '.serialize($this->db->last_query()));
    ...
    {
    	$result = $this->db()->set($attributes)->where($this->getPrimaryKey(), $primaryVal)->update($this->getTable());
    	log_message('debug', __CLASS__ . ' ' . __FUNCTION__ . ' last_query '.serialize($this->db->last_query()));
    	...
    	if (cache_has($cache_key))
    	{
    		...
    		log_message('debug', __CLASS__ . ' ' . __FUNCTION__ . ' cache_set '.serialize(array($cache_key, $cacheInfo)));
    	}
    ...
    public function increment($primaryVal, $attributes)
    {
    	...
    	{
    		$this->db()->set($key, $key .'+'.$val, false);//第三个参数false意为不需要escape
    	}
    	
    	$result = $this->db()->where($this->getPrimaryKey(), $primaryVal)->update($this->getTable());
    	log_message('debug', __CLASS__ . ' ' . __FUNCTION__ . ' last_query '.serialize($this->db->last_query()));
    ...
    public function decrement($primaryVal, $attributes)
    {
    	...
    	if (cache_has($cache_key))
    	{
    		...
    		foreach ($attributes as $field => $val)
    		{
    			...
    			{
    				$cacheInfo[$field] = (int)$cacheInfo[$field] - $val;
    				$cacheInfo[$field] < 0 && ($cacheInfo[$field] = 0);
    			}
    			else
    			{
    				return false;
    ...
    public function connect($hostname, $port, $username, $password, $database)
    {
    	...
    	log_message('error', __CLASS__ . ' ' . __FUNCTION__ . ' database '.serialize(array($db, $this->db->conn_id)));
    
    	return $this->db->conn_id ? true : false;
    }
    
    public function connect_config($dbconfig)
    {
    	$this->db = $this->load->database($dbconfig, true);
    	log_message('error', __CLASS__ . ' ' . __FUNCTION__ . ' database '.serialize(array($dbconfig, $this->db->conn_id)));
    ...
    public function connect_group($group)
    {
    	...
    	log_message('error', __CLASS__ . ' ' . __FUNCTION__ . ' database '.serialize(array($group, $this->db->conn_id)));
    ...
    private function getWhere($where)
    {
    	...
    	if (!empty($where['exist']))
    	{
    		foreach ($where['exist'] as $key => $val)
    		{
    			...
    			$this->db()->where_not_in($key, array('', 0));
    		}
    		$where['exist'] = '';
    		unset($where['exist']);
    	}
