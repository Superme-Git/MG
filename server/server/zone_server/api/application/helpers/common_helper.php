...
function setSite($siteKey_ = '')
{
    ...
    log_message('debug', __CLASS__.' '.__FUNCTION__.' succ '.json_encode(array($siteKey)));
    return $siteKey;
}
...
function response($data, $errno = '', $message = '')
{
	...
	log_message('debug', ' ------------- response -------------- running info '.serialize(array('memory' => $memory, 'return' => $dataStr)));
	exit();
}
