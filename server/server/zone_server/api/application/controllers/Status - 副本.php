...
class Status extends MY_Controller
{
    	...
    	if ($status_list)
    	{
    		foreach ($status_list as $k => $item)
    		{
    			...
    			if ($item_attribute['favorite_count'] > 0)
    			{
    				...
    				$where['status'] = 0;
    				$order = array('create_time' => 'desc');
    				$favorite_list = $this->Status_favorite_model->getList($where, $order, 0, 5, null, 0, false);
    				if ($favorite_list)
    				{
    					foreach ($favorite_list as $k_ => $item_)
    					{
    			...
    			if ($item_attribute['comment_count'] > 0)
    			{
    				...
    				$where['status'] = 0;
    				$order = array('create_time' => 'desc');
    				$comment_list = $this->Status_comment_model->getList($where, $order, 0, 5, null, 0, false);
    				if ($comment_list)
    				{
    					foreach ($comment_list as $k_ => $item_)
    					{
    ...
    public function user_timeline()
    {
    	...
    	//根据好友IDs获取微博列表
    	$status_list = $this->Status_model->get_byuser($roleid, $status_id, $count);
    ...
    public function get_comment_list()
    {
        ...
        if ($comment_list)
        {
            foreach ($comment_list as $k => $item)
            {
                ...
                $role_info_ = $this->Role_model->getInfo($item['roleid']);
                $comment_list[$k]['name'] = $role_info_['name'];
                $comment_list[$k]['avatar'] = $role_info_['avatar'];
                $comment_list[$k]['level'] = $role_info_['level'];
        
                //被回复人信息
                $comment_list[$k]['reply_name'] = '';
                $comment_list[$k]['reply_avatar'] = '';
                $comment_list[$k]['reply_level'] = 0;
                if ($item['reply_roleid'])
                {
                    $reply_role_info = $this->Role_model->getInfo($item['reply_roleid']);
                    $comment_list[$k]['reply_name'] = $reply_role_info['name'];
                    $comment_list[$k]['reply_avatar'] = $reply_role_info['avatar'];
                    $comment_list[$k]['reply_level'] = $reply_role_info['level'];
                }
            }
        }
    ...
    public function get_favorite_list()
    {
        ...
        if ($favorite_list)
        {
            foreach ($favorite_list as $k => $item)
            {
                if ($item['status'])
                {
                    $favorite_list[$k] = null;
                    unset($favorite_list[$k]);
                    log_message('error', __CLASS__.' '.__FUNCTION__.' had deleted '.serialize(array($item, $this->log_data)));
                    continue;
                }
                
                $role_info_ = $this->Role_model->getInfo($item['roleid']);
                $favorite_list[$k]['name'] = $role_info_['name'];
                $favorite_list[$k]['avatar'] = $role_info_['avatar'];
                $favorite_list[$k]['level'] = $role_info_['level'];
            }
        }
