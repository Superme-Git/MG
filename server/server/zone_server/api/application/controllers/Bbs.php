...
class Bbs extends MY_Controller
{
    ...
    public function bbs_timeline()
    {
    	...
    	$bbs_list = $this->Bbs_model->getList($where, $order, ($page-1)*$count, $count);
    ...
    public function add_popularity()
    {
    	...
    	$data = array();
    	$data['popularity_count'] = 1;
    	$this->Role_attribute_model->increment($to_roleid, $data);
    	
    	
    	
    	log_message('debug', __CLASS__.' '.__FUNCTION__.' succ '.serialize(array($where, $this->log_data)));
    	return response(true);
    }
    ...
    public function popularity_timeline()
    {
    	...
    	$where['to_roleid'] = $roleid;
    	$where['status'] = 0;
    	$bbs_popularity_list = $this->Bbs_popularity_model->getList($where, array('create_time' => 'desc'), ($page-1)*$count, $count, null, 0, false);
    ...
    public function gift_timeline()
    {
    	...
    	$where['to_roleid'] = $roleid;
    	$where['status'] = 0;
    	$bbs_gift_list = $this->Bbs_gift_model->getList($where, array('create_time' => 'desc'), ($page-1)*$count, $count, null, 0, false);
