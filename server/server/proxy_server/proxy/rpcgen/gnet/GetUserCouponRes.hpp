...
public:
	...
	GetUserCouponRes(int _retcode_, int _remain_coupon_, int _today_exchange_coupon_, int _today_remain_exchange_coupon_, int _reserved1_, int _reserved2_)
	...
	{
		return new GetUserCouponRes(*this);
	}

	virtual GNET::Rpc::Data & operator=(const GNET::Rpc::Data & d)
	{
		if (&d == this) return *this;
		if (const GetUserCouponRes *o = dynamic_cast<const GetUserCouponRes*>(&d)) return *this = *o;
		return *this;
	}

	GetUserCouponRes& operator=(const GetUserCouponRes & d)
	{
		if (&d == this) return *this;
		retcode = d.retcode;
	...
	bool operator<(const GetUserCouponRes &_o_) const {
		return compareTo(_o_) < 0;
	}

	int compareTo(const GetUserCouponRes &_o_) const {
