...
public:
	...
	MobileChecker( unsigned int uid, unsigned int ip, int _algo, const GNET::Octets& data) 
		: MatrixChecker(uid,ip), algorithm(_algo&0x0000FFFF), random(0)
	{
		key = 0;
		try{
			GNET::Marshal::OctetsStream(data) >> key;
		}catch(...){ }
	}

	unsigned int Challenge()
	{
		time_t t = GNET::Timer::GetTime();
...
public:
	...
	MatrixChecker *Create(unsigned int uid, unsigned int ip, int algo, const GNET::Octets& ctx)
	{
