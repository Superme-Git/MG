...
namespace gdeliverd
{
	...
	Cert::Cert(unsigned char* cert, int len)
	{
		...
		const unsigned char *ca_cert = locojoyca_certificate;                                                                     
		x509_ca = d2i_X509(NULL, &ca_cert, sizeof(locojoyca_certificate));        
		x509_cert = d2i_X509(NULL, (const unsigned char**) &cert, len);                                               
#else 
		unsigned char *ca_cert = locojoyca_certificate; 
		x509_ca = d2i_X509(NULL, &ca_cert, sizeof(locojoyca_certificate));               
	...
	int Cert::VerifyCert()
	{
		...
		if (strstr(cn_name, "au.locojoy.com") == NULL ){
			return 0;
		}	
