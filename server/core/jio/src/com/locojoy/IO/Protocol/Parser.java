...
public class Parser
{
	...
	{
		String dirname = bl_gen_with_seperate_dir ? rpcdata_default_package : default_package;
	...
	{
		String dirname = bl_gen_with_seperate_dir ? rpc_default_package : default_package;
		String cname = dirname + "/" + name;
		File file = new File(cname + ".java");
	...
	private static void generate_import(PrintStream ps)
	{
		...
		ps.println("import com.locojoy.base.*;");
		ps.println("import com.locojoy.base.Marshal.*;");
		ps.println("import com.locojoy.base.Security.*;");
    ...
    private static void rpc_generate_import(PrintStream printstream)
    {
    	if(bl_gen_with_seperate_dir)
    		printstream.println("package " + rpc_default_package + ";\n");
    	else
    		printstream.println("package " + default_package + ";\n");
        printstream.println("import com.locojoy.base.*;");
        printstream.println("import com.locojoy.base.Marshal.*;");
        printstream.println("import com.locojoy.base.Security.*;");
        printstream.println("import com.locojoy.IO.Protocol.*;");
        printstream.println("");
    }
    ...
    {
    	if(bl_gen_with_seperate_dir)
    		printstream.println("package " + rpcdata_default_package + ";\n");
    	else
    		printstream.println("package " + default_package + ";\n");
        printstream.println("import com.locojoy.base.*;");
        printstream.println("import com.locojoy.base.Marshal.*;");
...
/*
	private static void generate_dataaccess(PrintStream ps, Vector<Variable> variables)
	{
		int size = variables.size();
		for (int i = 0; i < size; i ++)
		{
...
*/
			...
			new DefaultHandler()
			{
				...
				public void endElement(String uri, String localName, String qName)
				{
					if (parsing && qName.compareTo("rpcdata") == 0)
					{
						...
						{
							if(bl_gen_with_seperate_dir)
								typemap.put(rpcdataName, Class.forName(rpcdata_default_package + "." + rpcdataName).newInstance());
							else
				...
				public void startElement(String uri, String localName, String qName, Attributes attributes) 
				{
					if (qName.compareTo("rpc") == 0)
					{
						try
						{
							...
							{
								if(bl_gen_with_seperate_dir)
									rpc = (Rpc)Class.forName(rpc_default_package + "." + name).newInstance();
								else
	...
	protected static void ParseFlag(final Map map) throws Exception
	{
		sap.parse(config.openStream(),
			new DefaultHandler()
			{
