...
namespace GNET
{
	enum ErrCode {
		...
		ERR_ACCOUNTLOCKED          = 9,  // 多次重复登录，当前用户的一个登录正在被处理，处于锁定状态
		ERR_MULTILOGIN             = 10, // 多次重复登录，且用户选择自动下线
		// keyexchange
		...
		ERR_ENTERWORLD_FAIL        = 14, //登录游戏服务器失败
		...
		ERR_LOGOUT_BY_BLKICK       = 20, //因账号强制登录被踢下线（区别被GM或反外挂踢下线的32）
		// deliver use
		ERR_DELIVER_SEND           = 21, // 转发失败
		ERR_DELIVER_TIMEOUT        = 22, // 转发超时
		//account
		ERR_ACCOUNTEMPTY           = 23, //账户余额不足
		ERR_ACCOUNTFORBID          = 24, //账号被GameMaster锁定，不允许登录
		ERR_INVALIDCHAR            = 25, //文字中含有非法的字符
		// LoginCheck
		ERR_IP_LOCK		   = 26, //IP已被封禁，无法登录游戏
		ERR_ID_LOCK		   = 27, //您已连续三次使用密保产品登录失败，为了您的账号安全，系统已暂时封禁，30分钟后再次尝试登录。
		ERR_MATRIXFAILURE	   = 28, //密保卡错误
		ERR_PASSWD_EXCEED          = 29, //输错密码的频率超过了deliver中的配置
		ERR_MATRIX_EXCEED          = 30, //输错密保卡的频率超过了deliver中的配置
		
		//player login
		ERR_LOGINFAIL              = 31, //登录游戏失败
		ERR_KICKOUT                = 32, //被踢下线
		ERR_CREATEROLE             = 33, //创建角色失败
		ERR_DELETEROLE             = 34, //删除角色失败
		ERR_ROLELIST               = 35, //获得角色列表失败
		ERR_UNDODELROLE            = 36, //撤销删除角色失败
		ERR_SRVMAINTAIN            = 37, //服务器维护中，暂时不能登录
		ERR_ROLEFORBID             = 38, //角色被GM封杀，不允许登录游戏
		...
		ERR_NOT_ACTIVED            = 131, //本服务器需经激活方可登入，该账号未激活。
		ERR_ZONGHENG_ACCOUNT       = 132, //纵横中文网账号未经激活不能登录游戏。
		ERR_STOPPED_ACCOUNT        = 133, //为了优化服务器负载，因该账号长时间未登录游戏，已被封禁，请与客服联系。
		ERR_ELEC_NUMBER_USED	   = 134, //此电子码已用过(用于二代神盾和手机令牌)
		ERR_LOGIN_LIMIT		   = 135, //账号存在恶意登录的嫌疑，禁止登录，如果不是本人登录，请与客服联系。		
		ERR_ACCOUNT_BE_MANAGED	   = 136, //您的账号已被锁定，请稍候尝试登录。(被托管账号自己登录时)	
		ERR_PHONETOKEN_ID_LOCK	   = 137, //您已连续三次使用令牌密码登录失败，为了您的账号安全，系统已暂时禁止登录，请在10分钟后再次尝试登录。
		ERR_SSO_TICKET_EXPIRE	   = 138, //跳转时ticket过期，登录失败
		ERR_SSO_TICKET_AID_NOT_MATCH = 139, //跳转时应用ID信息校验失败，登录失败
		ERR_SSO_TICKET_ZONEID_NOT_MATCH	= 140, //跳转时服务器ID信息校验失败，登录失败
		ERR_IP_FORBID		   = 141, //因该ip上账号或密码错误次数超限，该ip被封，时间由配置文件配置(deliver)		
		...
		ERR_MS_ACCOUNTFROZEN    =   215,      //账号已经冻结
		...
		//snail (8000-8999)
		ERR_NOT_CONNECT_AU	   = 8001,  // deliver没有连上AU或gdeliverd到AU的发送缓冲堵塞				
		ERR_LINK_NOT_CONNECT_DELIVER    = 8002,  // link还未连上deliver或deliver进程未启动
		ERR_GS_NOT_CONNECT_LINK    = 8003,  // gs还未连上link，暂无可用的gs或client暂时没bind到任何provider
		ERR_GS_NOT_CONNECT_DELIVER = 8004,  // gs还未连上deliver,账号信息无法发给gs
		ERR_AU_DELIVER_TIMEOUT     = 8005,  // deliver和AU之间消息超时
		ERR_CLIENT_AUTO_LOGIN      = 8006,  // 适用于梦诛客户端自动登录频率过高时直接取deliver缓存登录
		ERR_PHONETOKEN_FAILURE     = 8007,  // 手机令牌电子码错误或已过期，请校准后再尝试输入
		ERR_PHONETOKEN_EXCEED      = 8008,  // 手机令牌电子码输错超限
		ERR_LOGIN_EXCEED_LIMIT     = 8009,  // 账号登录过于频繁
		ERR_AUANY_LOGIN_EXCEPTION  = 8010,  // 账号通过AuAny登录,参数异常或AuAny平台异常，登录失败
		ERR_AUANY_LOGIN_NET_ERROR  = 8011,  // 账号通过AuAny登录,deliver和AuAny server之间网络通信错误，登录失败
	};
