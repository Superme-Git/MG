...
# gcc call convention: pass first six integer/pointer parmameter use RDI,RSI,RDX,RCX,R8,R9

	.align 2
mppc256_transform:
	push	%rbx
	push	%RdX			#push parmameter *hispter
	mov	%RdX, %r8		#r8 = *histptr
	mov	%RsI, %r9		#r = r9 = rsi = isize
	mov	%RcX, %r10		#rcx = *hash
	xor	%r11, %r11		#l = r11 = 0
	mov	$64, %edx		
	mov	(%r8), %rsi		#s = rsi = histptr
	cmp	$2, %r9
	mov	%rsi, %r8		#r8 = histptr
	jbe	.Llast_bytes
	add	%rsi, %r9
.Lnext_bytes:
	movzb	(%rsi), %rax
...
.Lfinal_none:
	pop	%r9			#pop *histptr
	mov	%r8, (%r9)		#set histptr to newvalue
	pop	%rbx
	ret
