...
public:
	...
	*/
	...
	index_hdr* great_equal_key( const void *key, size_t key_len )
	{
		...
		// 刘文�? 当key不存在的时候，下面这个版本能正确工作。否则部分key得不到正确结果，会返回NULL�?		return (hdr && hdr==Layout2Page(hdr)->index_tail()) ? Layout2Page(hdr)->next(hdr-1) : hdr;
	}
...
public:
	...
	void* greater_key( const void* key, size_t *key_len )
	{
		return page_cache->greater_key( key, *key_len );
	}
