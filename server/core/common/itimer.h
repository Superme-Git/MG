...
public:
	static void Update()
	{
		std::vector<Observer*> tmp;
		{
			...
			Observers::iterator lower_bound = observers.lower_bound(tick_now);
			std::transform(observers.begin(), lower_bound, std::back_insert_iterator<std::vector<Observer*> >(tmp), std::_Select2nd<Observers::value_type>());
			observers.erase(observers.begin(), lower_bound);
		}

		for(std::vector<Observer*>::iterator it = tmp.begin(), ie = tmp.end(); it != ie; ++it)
		{
			if((*it)->Update())
				AttachObserver(*it);
		}
