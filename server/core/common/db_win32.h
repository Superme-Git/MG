...
#ifdef WIN32
inline size_t pread( HANDLE fd, void *buffer, size_t size, int64_t off )
{
	LARGE_INTEGER li;
	li.QuadPart = off;
	...
	DWORD readed;
	if (!ReadFile(fd, buffer, size, &readed, NULL))
	{
...
inline size_t pwrite( HANDLE fd, const void *buffer, size_t size, int64_t off )
{
	LARGE_INTEGER li;
	li.QuadPart = off;
	...
	SetFilePointer(fd, li.LowPart, &li.HighPart, FILE_BEGIN);
	DWORD written;
	if (!WriteFile(fd, buffer, size, &written, NULL))
	{
...
public:
	~PageFile() { CloseHandle(fd); free(ident); DeleteCriticalSection(&cs); }
	struct Exception { };
	...
	PageFile ( const char *path, int flags = O_CREAT|O_RDWR|O_BINARY, mode_t mode = 0600 )
	{
		...
		wchar_t wstr[plen + 1];
		int wlen = MultiByteToWideChar(CP_UTF8, 0, path, plen, wstr, plen);
		if (wlen >= 0)
			wstr[wlen] = 0;
		fd = CreateFileW(wstr, GENERIC_READ|GENERIC_WRITE, FILE_SHARE_READ|FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if (fd == INVALID_HANDLE_VALUE)
		{
			printf("open failed '%S' %d:%d\n", wstr, wlen, GetLastError()); fflush(stdout);
			throw Exception();
		}
	...
	void read( page_index_t idx, void *data )
	{
		...
		count_read++;
		if ( pread( fd, data, PAGESIZE, (int64_t)idx * PAGESIZE ) != PAGESIZE )
		{
	...
	void write( page_index_t idx, const void *data )
	{
		...
		count_write++;
		if ( pwrite( fd, data, PAGESIZE, (int64_t)idx * PAGESIZE ) != PAGESIZE )
		{
			LeaveCriticalSection(&cs);
			throw Exception();
		}
	...
	void truncate( page_index_t idx )
	{
		...
		LARGE_INTEGER li;
		li.QuadPart = (int64_t)idx * PAGESIZE;

		SetFilePointer(fd, li.LowPart, &li.HighPart, FILE_BEGIN);
		if (!SetEndOfFile(fd))
		{
			LeaveCriticalSection(&cs);
			printf("trucate failed %s\n", identity()); fflush(stdout);
			throw Exception();
		}
		else
			LeaveCriticalSection(&cs);
...
public:
	...
	void truncate( page_index_t idx )
	{
		if ( ftruncate( fd, (off64_t)idx * PAGESIZE ) == -1 )
			throw Exception();
	}
...
public:
	...
	Page( page_index_t index, PageCache *cache ) : idx(index), lru(0), 
		layout((PageLayout *)PageMemory::alloc()), page_cache(cache), dirty(false) { *(Page **)(layout + 1) = this; }
	~Page() { PageMemory::free(layout); }
	PageLayout *snapshot_create()
	{
		dirty = false;
		PageLayout *tmp = (PageLayout *)PageMemory::alloc();
		memcpy( tmp, layout, PAGEUSED );
...
public:
	...
	*/
	...
	index_hdr* great_equal_key( const void *key, size_t key_len )
	{
		...
		// 刘文伟. 当key不存在的时候，下面这个版本能正确工作。否则部分key得不到正确结果，会返回NULL。
	...
	index_hdr* greater_key( const void *key, size_t key_len )
	{
		...
		if (index_hdr *hdr = find_key( key, key_len, pos ))
		{
			if (pos == 0) // equals found.
				return Layout2Page(hdr)->next(hdr); // return NULL if the key is last. see next.
			// pos < 0 see next line.
			// greater found. maybe tail. see great_equal_key.
			return (hdr == Layout2Page(hdr)->index_tail()) ? Layout2Page(hdr)->next(hdr-1): hdr;
		}
		// eof
		return NULL;
	}
...
public:
	...
	PageLayout** snapshot_reference(size_t& size) { size = snapshot_size; return snapshot; }

	size_t snapshot_create( Page *magic )
	{
		if ( snapshot_capacity < _size + 1 )
		{
			allocator.free(snapshot, snapshot_capacity * sizeof(PageLayout *) );
			snapshot = (PageLayout **)allocator.alloc( (snapshot_capacity = _size + 1) * sizeof(PageLayout *) );
		}
		snapshot_size = 0;
		for ( Page **it = bucket, **ie = bucket + bucket_mask + 1; it != ie; ++it )
			for ( Page *page = *it ; page ; page = page->get_next_hash_equal() )
				if ( page->is_dirty() )
					snapshot[snapshot_size++] = page->snapshot_create();
		if ( snapshot_size || magic->is_dirty() )
			snapshot[snapshot_size++] = magic->snapshot_create();
...
public:
	...
	PageLayout** snapshot_reference(size_t &snapshot_size) { return hash.snapshot_reference(snapshot_size); }
...
public:
	...
	static void save_page( PageLayout *layout, PageFile *log_file, page_index_t page_index )
	{
		log_file->write( page_index, layout );
		PageMemory::free( layout );
	}

	static void save_page( PageLayout **it, PageLayout **ie, PageFile *log_file )
	{
		for ( ; it != ie; ++it )
			save_page( *it, log_file, *(page_index_t *)(*it + 1) );
	}

	static page_index_t save_page( PageLayout **it, PageLayout **ie, PageFile *log_file, page_index_t page_index )
	{
		for ( ; it != ie; ++it )
...
public:
	...
	State commit(time32_t timestamp )
	{
		...
		PageLayout** snapshot = page_cache->snapshot_reference( snapshot_size );
		try
		{
...
public:
	...
	std::vector<time32_t> version () { return logger->check_version(); }
	std::vector<time32_t> restore (time32_t timestamp) { return logger->restore( timestamp ); }
	std::vector<time32_t> remove_logs()
	{
		std::vector<time32_t> r;
		time32_t time_stamp = logger->get_logger_id();
		if ( DIR *dir = opendir( logdir ) )
		{
			while ( dirent *d = readdir( dir ) )
				if ( d->d_name[0] != '.' )
