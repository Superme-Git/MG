...
public:
    ...
    inline timeval tickbegin2()
    {
        timeval tv;
        gettimeofday( &tv, &m_tz ); 
        return tv;
    }

    inline u_int32_t tickend2( timeval begin )
    {
        ...
        if( 0 == gettimeofday( &tv_now, &m_tz ) )
        {
            // get values
            u_int32_t span = 0;
            span = ((long)tv_now.tv_usec - (long)begin.tv_usec);
            if( tv_now.tv_sec - begin.tv_sec > 0 )
                span += (tv_now.tv_sec - begin.tv_sec)*1000000;
