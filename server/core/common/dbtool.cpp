...
int collection_main( int argc, char *argv[], const char *datadir, const char *logdir )
{
	...
	if ( init )
	{
		...
		if ( version )
		{
			std::vector<time32_t> r = env->version();
			puts("Version:");
			for ( std::vector<time32_t>::iterator it = r.begin(), ie = r.end(); it != ie; ++it ){
                tm = *it;
				printf("\t<%lx> %s", *it, ctime(&tm) );
            }
		}
		else if ( timestamp )
		{
			std::vector<time32_t> r = env->restore( (size_t)atoi(timestamp) );
			puts("Restore:");
			for ( std::vector<time32_t>::iterator it = r.begin(), ie = r.end(); it != ie; ++it ){
                tm = *it;
				printf("\t<%lx> %s", *it, ctime(&tm) );
		...
		{
			std::vector<time32_t> r = env->remove_logs();
			puts("RemoveLogs:");
