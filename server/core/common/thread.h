...
namespace Thread
{
	...
	public:
		...
		public:
			...
			Policy() : m_max_queuesize(1048576), m_state(stateNormal), m_prior_strict(false)
			{  }
			...
			virtual bool OnAddTask(Thread::Runnable * pTask, size_t queuesize, bool bForced)
			{
				...
				if( stateNormal == m_state )
				{
					...
					Log::log( LOG_WARNING, "OnAddTask failed.(pTask=%u,QueueSize=%u,TaskPriority=%u)",
								pTask, queuesize, (pTask ? pTask->GetPriority() : 0) );
				}
				delete pTask;
				return false;
			}

			virtual void OnRunThread(int nThreadPrior) {}
	...
	private:
		...
		{
			Mutex::Scoped lock(s_mutex_tasks);
			if( s_ppolicy && s_ppolicy->m_prior_strict )
			{
				Runnable * pTask = NULL;
				if (!s_tasks.empty())
				{
		...
		static void * RunThread( void * pParam )
		{
			...
			s_ppolicy->OnRunThread(nThreadPrior);
				...
				while (true)
				{
...

					{
						...
						if( thcount_now < thcount_want )
						{
							...
							pthread_t th;
							pthread_create( &th, NULL, &Pool::RunThread, pParam );
						}
