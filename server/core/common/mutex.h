...
namespace Thread
{
	...
	class SpinLock
	{
#if (__GNUC__ == 4 && __USE_ASM_SPINLOCK == 0)
	...
	public:
		~SpinLock() { }
		explicit SpinLock(const char* id) : locker(0) { }
		void Lock()
		{
...
namespace Thread
{
	struct Mutex
	{
		...
		explicit Mutex(const char* id,bool recursive=false) { InitializeCriticalSection(&_cs); }
		...
		{
			~Scoped() { _m.UNLock(); }
			explicit Scoped(Mutex& m) : _m(m) { m.Lock(); }
		private:
			Mutex & _m;
		};
	private:
