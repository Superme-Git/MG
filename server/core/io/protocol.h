...
public:
	...
	public:
		...
		private:
			...
			void OnRecv()
			{
				...
				if( DecodeFlashPolicy(is) )
				{
			   		// send at OnAddSession, ignore here SendFlashPolicy();
				}
#endif
				try
				{
			...
			bool DecodeFlashPolicy(const Manager::Session::Stream &stream)
			{
				if (stream.eos())
					return false;
				...
				{
					char head;
					stream >> Marshal::Begin >> head >> Marshal::Rollback;
					if( '<' == head )
					{
						stream >> Marshal::Begin;
						stream.pop_byte( (char *)data.begin(), 23 );
						data.resize(23);
						if( 0 == memcmp( data.begin(), "<policy-file-request/>", 22 ) )
						{
							stream >> Marshal::Commit;
							Log::log(LOG_DEBUG,"DecodeFlashPolicy(sid=%d,data=%s,size=%d,stream.size=%d) find.",
									sid, (char*)data.begin(), data.size(), stream.size() );
							return true;
						}
						else
							stream >> Marshal::Rollback;
					}
				}
				catch (Marshal::Exception &)
				{
					Log::log(LOG_DEBUG,"DecodeFlashPoliy(sid=%d,stream.size=%d) Exception.", sid,stream.size());
					stream >> Marshal::Rollback;
				}
				return false;
			}
			...
			{
				const char * policy = "<?xml version=\"1.0\" encoding=\"utf-8\"?><cross-domain-policy><site-control permitted-cross-domain-policies=\"all\"/><allow-access-from domain=\"*\" to-ports=\"*\"/></cross-domain-policy>";
				Octets data( policy, strlen(policy) );
				data.insert( data.end(), "\0", 1 );
				std::string conf_policy = Conf::GetInstance()->find("FlashPolicy", "policy");
				if( conf_policy.length() > 0 )
				{
					data.replace( conf_policy.c_str(), conf_policy.length() );
					data.insert( data.end(), "\0", 1 );
				}
				bool ret = Send( data, true );
				PollIO::WakeUp();
				Log::log(LOG_DEBUG,"SendFlashPolicy(sid=%d,data=%s,size=%d) sent.ret=%d",
						sid,(char*)data.begin(),data.size(),ret );
				return ret;
			}
		};
	private:
	...
	public:
		...
		bool SendFlashPolicy(Session::ID id)
		{
			bool rc = false;
			Thread::RWLock::RDScoped l(locker_map);
			Session *session = GetSession(id);
			if (session) rc = session->SendFlashPolicy();
			return rc;
		}
		...
		virtual int  PriorPolicy(Protocol* p) const
		{
			return PriorPolicy(p->type);
		}
...
private:
	...
	public:
		...
		static void Dispatch(Manager *manager, Manager::Session::ID sid, Protocol *protocol)
		{
			...
			int priority = manager->PriorPolicy(protocol);
			if (priority)
			{
...
public:
    ...
    virtual std::ostream& trace(std::ostream & _os_) const { return _os_; }
	...
	class AutoPtrT
	{
		...
		// avoid copy
		AutoPtrT(const AutoPtrT &);	
		void operator = (const AutoPtrT &);	
	public:
		explicit AutoPtrT(Protocol * _p) { p = (T*)_p; }
		~AutoPtrT()                      { if (p) p->Destroy(); }
		T* operator -> ()                { return p; }
		T* get()                         { return p; }
		void release()                   { p = NULL; }
	};
