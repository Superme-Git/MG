...
protected:
	...
	// add by lich. 2011/5/31
	// 在不超过缓存容量的情况下，输出多少算多少。
	// @return 被输出的大小。
	size_t OutputPart(const Octets &data)
	{
		size_t curSize = obuffer.size() + osecbuf.size();
		if (curSize >= obuffer.capacity()) return 0; // 这应该是不可能的。检查一下也不坏。只因size_t是unsigned的。

		size_t freeSize = obuffer.capacity() - curSize;
		size_t copySize = std::min(freeSize, data.size());
		osecbuf.insert(osecbuf.end(), data.begin(), copySize);
		return copySize;
	}
	...
	void PollIn()
	{
		...
		do
		{
			if ((recv_bytes = read(fd, ibuf.end(), ibuf.capacity() - ibuf.size())) > 0)
			{
				...
				//memcpy(tempdata,(char*)ibuf.begin(),sizeof(tempdata));
				//Log::log(LOG_ERR,"PollIn, fd=%d ibuffer=%d recv_bytes=%d data0=%x data1=%x",fd,ibuf.size(),recv_bytes,tempdata[0],tempdata[1]);
				//Log::log(LOG_DEBUG,"PollIn, fd=%d ibuffer=%d recv_bytes=%d", fd, ibuf.size(), recv_bytes);
		...
		if (recv_bytes != -1 || errno != EAGAIN)
		{
			...
			//Log::log(LOG_ERR,"PollIn session->Close, fd=%d recv_bytes=%d errno=%d ",fd,recv_bytes,errno);
		}
	}
