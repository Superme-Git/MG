...
/**
 * 合并操作 <li>srcDB不会被修改，如何合并正常，则一次性完成所有表的合并操作 <li>
 * key冲突时，调用xmerge.xml配置中对应实现的merge方法进行合并 <li>merge回调的错误不中断合并，捕捉异常并记录日志
 * */
public final class Merger {
	...
	public void SetDestDB(String destDir, boolean notCopy) throws Exception {
		...
		if (notCopy) {
			...
			tmp = new File(XmergeConf.getInstance().getResultDbHome());
			xmerge.Util.clearDir(tmp);
			xmerge.Util.copyFolder(destDir, XmergeConf.getInstance().getResultDbHome());
			// SetResultDB(XmergeConf.getInstance().getResultDbHome());
		}
		// 此时的tmp是指向resultDBDir的,所以open打开的"destDB"其实目录是在resultDBDir
	...
	public String getResultDBDir() {
		...
		if (walkKnownFailed || walkUnknownFailed)
			...
			return XmergeConf.getInstance().getResultDbHome();
	}
	...
	public boolean doMerge() throws Exception {
		...
		try {
			for (DatabaseMetaData.Table table : srcDB.getMetaData().getTables()) {
				...
				try {
					boolean preprocessTable = XmergeConf.getInstance().isPreProcessTable(table.getName());
					// 预处理,比如像帮派key这样的情况
					if (preprocessTable) {
						Dbx.Table destTable = destDB.openTable(table.getName());
						if (!preProcess(table.getName(), src_table, destTable))
							throw new Error("try to preprocess failed.table" + destTable.getName());
						else
							mkdb.Trace.info("preprocess table success:"+table.getName());
					}
				} finally {
			...
			for (DatabaseMetaData.Table table : srcDB.getMetaData().getTables()) {
				...
				Dbx.Table src_table = srcDB.openTable(table.getName());
				try {
					boolean isCleanTable = XmergeConf.getInstance().isCleanTable(table.getName());
					if (isCleanTable) {// 如果是要清空的表
						List<OctetsStream> keyStreams = new ArrayList<OctetsStream>();
						Dbx.Table destTable = destDB.openTable(table.getName());
						CleanTableWalker walker = new CleanTableWalker(keyStreams);
						destTable.walk(walker);
						if (!keyStreams.isEmpty()) {
							for (OctetsStream octetsStream : keyStreams) {
								if (!destTable.remove(octetsStream))
									throw new Error("try to remove entry failed.table" + destTable.getName());
							}
							for (OctetsStream octetsStream : keyStreams) {
								if (destTable.exist(octetsStream))
									throw new Error("remove entry failed.table" + destTable.getName());
							}
							mkdb.Trace.info("clear table success:"+destTable.getName());
						}
					} else {// 如果不是要清空的表,走一起的knownconflict,unknown conflict逻辑
						boolean isKnown = XmergeConf.getInstance().isKnownConflict(table.getName());
						/**
						 * <li>在src_table上walk，修改dest_table <li>
						 * Known_key_conflict的表用walk
						 * ，Unknown_key_conflict的表用_walk， <li>
						 * 根据walkUnknownFailed标签来判定walk时是否出错
						 * (Unknown_key_conflict) <li>
						 * 根据walkKnownFailed标签来判定merge known_key_conflict是否发生异常
						 **/
						if (isKnown) {
							mkdb.Trace.info("Merge table start:"+table.getName());
			...
			srcDB.close();
			destDB.close();
			xmerge.Util.deleteDir(new File("asdf"));
			if (Merger.this.walkKnownFailed) {
				rollbackAll();
			}
		}
	...
	private boolean preProcess(String tableName, Dbx.Table src_table, Dbx.Table dst_table) {

		IPreprocess obj = XmergeConf.getInstance().getPreprocess(tableName);

		try {
			if (obj.preProcess(srcDB, src_table, destDB, dst_table))
				return true;
		} catch (Exception e) {
			mkdb.Trace.debug("preprocess throw exception");
			mkdb.Trace.info("preprocess throw exception", e);
...
//						throw new RuntimeException("_sys_ table has conflict key...");
	...
	public class unknownWalker implements mkdb.Storage.IWalk {
		...
		public boolean onRecord(byte[] _key, byte[] _data) {
			...
			if (this.destTable._insert(src_key, src_value)) {
				...
				mkdb.Trace.info("conflict key =" + destTable.getMetaData().getKeyType().unmarshal(src_key));
	...
	public class CleanTableWalker implements mkdb.Storage.IWalk {

		private List<OctetsStream> keyStreams;

		public CleanTableWalker(List<OctetsStream> streams) {

			super();
			this.keyStreams = streams;
		}

		/**
		 * (non-Javadoc)
		 * 
		 * @see mkdb.Storage.IWalk#onRecord(byte[], byte[])
		 */
		@Override
		public boolean onRecord(byte[] _key, byte[] _data) {

			OctetsStream src_key = OctetsStream.wrap(Octets.wrap(_key));
			this.keyStreams.add(src_key);
			return true;
		}
	...
	public class knownWalker implements mkdb.Storage.IWalk {
				...
				try {
					...
					if (mergeKnownConflictKey(src_key, src_value, dest_value)) {
						...
						if (Merger.this.walkKnownFailed) {
							 ...
							 * 目前只记录UserMergeImplement处理 出错时的key以及出错计数
							 * */
		...
		private boolean mergeKnownConflictKey(OctetsStream src_key, OctetsStream src_value, OctetsStream dest_value) throws Exception {
			...
			try {
				...
				mkdb.Trace.info("userMergeImplement throw exception", e);
				Merger.this.walkKnownFailed = true;
				return false;
			}
			return false;
