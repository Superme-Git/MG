...
public class Variable {
	...
	public void verify(Mkdb mkdb, Type bean) {
		// verify capacity
		vartype.verifyCapacity(mkdb, new Capacity(capacity, "bean." + bean.getName() + "." + getname()));
		...
		if (!foreign.isEmpty()) {
			...
			vartype.verifyForeign(mkdb, new Foreign(foreign, bean.getName() + "." + getname()));
		}
	}
	...
	public void printSelect(PrintStream ps, String K, String V) {
		...
		ps.println("		return getTable().select(key, new mkdb.TField<" + V + ", " + F + ">() {");
