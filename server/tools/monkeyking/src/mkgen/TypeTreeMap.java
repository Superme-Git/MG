...
public class TypeTreeMap extends TypeBaseMap {
	...
	public void verifyForeign(Mkdb mkdb, Foreign foreign) {
		if (null != foreign.getKey()) {
			Table table = mkdb.getTable(foreign.getKey());
	...
	public void verifyCapacity(Mkdb mkdb, Capacity capacity) {
		...
		keytype.verifyCapacity(mkdb, capacity.extractKey());
		valuetype.verifyCapacity(mkdb, capacity.extractValue());
	}
	...
	public void getter(XBean bean, Variable var, PrintStream ps, String prefix) {
		...
		ps.println(prefix + "	return mkdb.Logs.logNavigableMap(new mkdb.LogKey(this, "
	...
	public void getterConst(XBean bean, Variable var, PrintStream ps, String prefix) {
		...
		ps.println(prefix + "public " + getGetterName() + " get" + var.getName() + "() { // " + var.getComment());
		Main._xdb_verify_(ps, prefix + "	");
		ps.println(prefix + "	return mkdb.Consts.constNavigableMap(" + var.getname() + ");");
		ps.println(prefix + "}");
		ps.println("");
	...
	public String newListenable(XBean bean, Variable var) {
		return "new mkdb.logs.ListenableMap().setVarName(" + Main.quote(var.getname()) + ")";
	}
