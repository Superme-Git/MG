...
public class Table {
	...
	Table(Element self) {
		...
		if (define.isParsed())
		{
			if (self.hasAttribute("cachelow"))
				throw new RuntimeException("cachelow is not allowed for defined mkdb in table '" + name + "'");
			if (self.hasAttribute("cachehigh"))
				throw new RuntimeException("cachehigh is not allowed for defined mkdb in table '" + name + "'");
	...
	void verify(Mkdb mkdb) {
		...
		if (!foreign.isEmpty()) {
			...
			if (tableForeign.getKey() != null)
				keytype.verifyForeign(mkdb, new Foreign(tableForeign, tableForeign.getKey()));
			if (tableForeign.getValue() != null)
				valuetype.verifyForeign(mkdb, new Foreign(tableForeign, tableForeign.getValue()));
		}
		
		Capacity cap = new Capacity(capacity, "table." + this.getName());
		this.keytype.verifyCapacity(mkdb, cap.extractKey());
		this.valuetype.verifyCapacity(mkdb, cap.extractValue());
	...
	void make() {
		...
		if (isNeedExplicitCheck()) {
			...
			ps.println("		mkdb.Lockey lockey = mkdb.Lockeys.get(_Tables_.getInstance()." + name + ", key);");
		...
		if (this.autoIncrement) {
			ps.println("	public static mkdb.util.AutoKey<" + K + "> getAutoKey() {");
...
//		ps.println("");
		...
		ps.println("");
		// cache
		ps.println("	public static mkdb.TTableCache" + KV + " getCache() {");
		ps.println("		return _Tables_.getInstance()." + name + ".getCache();");
		ps.println("	}");
		ps.println("");
		ps.println("	public static mkdb.TTable" + KV + " getTable() {");
	...
	void define(PrintStream ps) {
		...
		if (this.autoIncrement) {
			ps.println("		@Override");
			ps.println("		protected mkdb.util.AutoKey<" + k + "> bindAutoKey() {");
	...
	static void make(Collection<Table> tables) {
		...
		ps.println("");
		ps.println("public class _Tables_ extends mkdb.Tables {");
		...
		ps.println("		return (_Tables_)mkdb.Mkdb.getInstance().getTables();");
		ps.println("	}");
		ps.println("");
