...
public class CTable {
	...
	void makeTableListener(PrintStream ps) {
		String output = 
			"	static class %1$sListener implements mkdb.logs.Listener {\r\n" +
			...
			"				mkdb.util.ConcurrentCaches.getInstance().add(\r\n" +
			"						new UpdateJob((%2$s)key, %4$s(xtable.%1$s.get((%2$s)key))));\r\n" +
			"		}\r\n" +
			"		@Override\r\n" +
			"		public void onChanged(Object key, String fullVarName, mkdb.logs.Note note) {\r\n" +
			"			if (CacheImpl.instance.existInCache((%2$s)key))\r\n" +
			"				mkdb.util.ConcurrentCaches.getInstance().add(\r\n" +
			"						new UpdateJob((%2$s)key, %4$s(xtable.%1$s.get((%2$s)key))));\r\n" +
			"		}\r\n" +
	...
	void compile(CCache ccache) {
		this.ccache = ccache;
		table = ccache.getMkdb().getTable(name);
		if (null == table)
