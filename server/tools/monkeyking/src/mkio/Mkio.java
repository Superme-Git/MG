 ...
 * Socket 连接
 * 
 * 网络事件处理，和读写，管理 buffer 通过接口 Filter 进行数据加解密，压缩，搭建高级协议层。
 * 
 */
public class Mkio implements Handle, java.io.Closeable, MkioMBean {
	...
	public void close(Throwable ex) {
		// 小心锁的问题，参见 Manager.close 等在 synchronized(mkios) 调用 mkio.close() 的地方。
		try {
			if (getAndClearOpen()) {
				key.channel().close();
	...
	public boolean write(ByteBuffer b) {
		try {
			...
			if (mkdb.Trace.isDebugEnabled())
				mkdb.Trace.debug(this, e); // IOError 错误日志只在debug时记录。
		} catch (Throwable e) {
	...
	/**
	 * 相对于以前的 write 操作，buffer不能被复用：加入到输出队列以后，由 Mkio 管理。
	 * 
	 * @param buffer
	 */
