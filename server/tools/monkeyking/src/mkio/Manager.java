 ...
 * 连接管理
 * 
 * DynamicMBean 由 MkioConf 在open的时候注册到MBeans中。
 * 
 */
public abstract class Manager implements DynamicMBean {
	 ...
	 * <p>
	 * <b>注意，这里可能拦截不到所有的协议。</b>如 Rpc 的处理不一定会走这条路。 如下两种情形
	 * <ol>
	 * <li>同步调用时，Rpc.Result会直接交给等待的线程。
	 * <li>如果 Rpc.Result 配置了存储过程，mkio将直接把处理交给 mkdb.Procedure。在内部把存储过程的结果发送回去。
	 * </ol>
	 * 
	 * @param p
	 *            需要执行的协议。
	...
	/**
	 * 通过重载这个方法可以实现: * IP 限制。 * 初始化filter。 * 设置 interestOps * 提供新的 Mkio 实现。
	 * 
	 * @param channel
	 */
	...
	/**
	 * 解析配置，并初始化配置。 可以重载这个方法，用来解析新增的自定义 Manager 的配置。重载时必须调用 super.parse
	 * 以初始化基本配置。
	 * 
	 * @param self
	 * @throws Exception
	...
	public Object getAttribute(String attribute) {
		if (attribute.startsWith("count."))
			return this.newMkioCount.get(); // 目前只有一个计数器。
		return "<- see this";
	}
	...
	/**
	 * 用于子类重载，并加入更多的动态 MBean 属性。 重载实现必须调用 super 的方法。否则基类的属性会被丢失。
