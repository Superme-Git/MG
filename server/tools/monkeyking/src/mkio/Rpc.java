 ...
 * 远程调用的简单实现。
 * 
 * 这个对象本身设计上不是线程安全的，原则上不允许多线程同时使用同一个实例。 内部采用不同的管理期达到多线程的安全性。 1
 * 发送请求：创建rpc，填写参数，发送请求都只在一个线程中执行。 2 结果处理：此时有timeout线程，或者io线程，此时通过上下文来实现线程互斥：
...
public abstract class Rpc<A extends Marshal, R extends Marshal> extends Protocol
	...
	//////////////////////////////////////////////////////////////
	/**
	 * 异步处理接口，如果使用 submit 方式调用 rpc，不会产生这个回调。 当客户端使用异步方式调用RPC时，需要实现。 -1
	 * 异步关闭(AsynchronousClose) -2 没有响应超时(timeout)
	 */
	...
	// 内部实现
	/**
	 * 用来传递结果给同步等待的线程。 通过继承获得保护接口的调用权利。 修改默认实现的超时设置。
	 * 
