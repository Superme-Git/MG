...
void Texture2D::commitRect(GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLboolean force)
{
    if (level < levelCount())
    {
        rx::Image *image = mImageArray[level];
        if (image->updateSurface(mTexStorage, level, xoffset, yoffset, width, height, force || (isMipmapFiltered() && isMipmapComplete())))
        {
            image->markClean();
        }
    }
...
void Texture2D::subImage(GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, GLint unpackAlignment,
						 const void *pixels, GLboolean force)
{
...
//#else
		commitRect(level, xoffset, yoffset, width, height, force);
//#endif
    }
}
