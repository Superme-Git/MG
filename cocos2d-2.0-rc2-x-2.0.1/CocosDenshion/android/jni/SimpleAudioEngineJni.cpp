...
extern "C"
{
    ...
    static JNIEnv* getJNIEnv(void)
    {
		...
		JavaVM* jvm = MT3::JniHelper::getJavaVM();
        if (NULL == jvm) {
	...
	void stopEffectByPathJNI(const char* path)
	{
		...
		if (!getStaticMethodInfo(methodInfo, "stopEffectByPath", "(Ljava/lang/String;)V"))
		{
			return;
		}
	...
	bool getEffectIsPlayingJNI(const char* path)
	{
		JniMethodInfo methodInfo;
		jboolean ret = false;

		if (!getStaticMethodInfo(methodInfo, "getEffectIsPlaying", "(Ljava/lang/String;)Z"))
		{
			return ret;
		}
    ...
    void setEffectVolumeJNI(unsigned int nSoundId, float fVolume)
    {
    	JniMethodInfo methodInfo;

    	if (! getStaticMethodInfo(methodInfo, "setEffectVolume", "(IF)V"))
    	{
    		return;
    	}

    	methodInfo.env->CallStaticVoidMethod(methodInfo.classID, methodInfo.methodID, nSoundId, fVolume);
    	methodInfo.env->DeleteLocalRef(methodInfo.classID);
    }

    bool hasEffectJNI(unsigned int nSoundId)
    {
    	JniMethodInfo methodInfo;

    	if (!getStaticMethodInfo(methodInfo, "hasEffect", "(I)Z"))
    	{
    		return false;
    	}

    	bool bRet = methodInfo.env->CallStaticBooleanMethod(methodInfo.classID, methodInfo.methodID, nSoundId);
    	methodInfo.env->DeleteLocalRef(methodInfo.classID);

    	return bRet;
    }

    bool isEffectPlayingJNI(unsigned int nSoundId)
    {
    	JniMethodInfo methodInfo;

    	if (!getStaticMethodInfo(methodInfo, "isEffectPlaying", "(I)Z"))
    	{
    		return false;
    	}
