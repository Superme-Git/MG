...
extern int lame_decoder(const byte* mp3Stream, size_t intSize, std::vector<byte>* wavStream);
...
//struct stByteBlock {
//	stByteBlock(size_t s) : size(s), pos(0), pNext(NULL) {
//		ptr = new byte[s];
//	}
//	~stByteBlock() { delete ptr; }
//	byte* ptr;
//	size_t size;
//	size_t pos;
//	stByteBlock* pNext;
...
//	stByteBlock* pTail;
//public:
//	ByteStream() : pHead(NULL), pTail(NULL) {}
//	~ByteStream() {
//		stByteBlock* pCur = pHead;
//		while (pCur) {
//			stByteBlock* pTemp = pCur->pNext;
//			delete pCur;
//			pCur = pTemp;
//		}
//	}
//	void addBuff(const char* buf, size_t len) {
//		if (!pTail) {
//			pTail = new stByteBlock(0x100);
//			pHead = pTail;
//		}
//		size_t cur = 0;
//		while (cur < len) {
//			if (pTail->size <= pTail->pos) {
//				// add block
//				stByteBlock* newBlock = new stByteBlock(pTail->size*2);
//				pTail->pNext = newBlock;
//				pTail = newBlock;
//			}
//			size_t copy = pTail->size - pTail->pos < len - cur ? pTail->size - pTail->pos : len - cur;
//			memcpy(pTail->ptr + pTail->pos, buf + cur, copy);
//			pTail->pos += copy;
//			cur += copy;
//		}
//	}
//
//	void copyToBuff(void* buf) {
//		stByteBlock* pCur = pHead;
//		size_t cur = 0;
//		while (pCur) {
//			if (pCur->pNext) {
...
//	ThrowIfFailed(E_FAIL);
	...
	const byte * mp3DataPtr = data->Data;
	
	std::vector<byte> outdata;
	
	lame_decoder(mp3DataPtr, length, &outdata);

	
	UINT32 offset = 0;

	DWORD riffDataSize = 0;
	byte* dataPtr = outdata.data();
	...
	//******* bug fixed for lame_decoder unknown error.
	//add by kyle
	if (chunkSize + chunkPos > outdata.size()) {
		chunkSize = outdata.size() - chunkPos;
	}
	//****************
