...
**重要**: MT3项目仅支持Visual Studio 2013 (v120工具集)，不支持Visual Studio 2015/2017/2019/2022及更高版本。即使v140编译成功，运行时也会崩溃。
...
| 工具 | 版本 | 下载地址 |
|------|------|---------|
| Visual Studio | 2013 Professional/Ultimate | [微软官网](https://visualstudio.microsoft.com/vs/older-downloads/) |
| Windows SDK | 8.1 | 随VS2013安装 |

### 必需组件

在 Visual Studio 2013 安装以下组件：

1. **Visual C++**
2. **Microsoft Foundation Classes for C++**
3. **Windows SDK 8.1**
4. **MSBuild 12.0**
...
应看到类似输出：
```
Microsoft (R) 生成引擎版本 12.0.xxxxx.x
```
...
**错误信息**:
```
error MSB8020: 无法找到 v120 的生成工具（平台工具集 = 'v120'）
```

**解决方案**:
确保已安装 Visual Studio 2013 Professional/Ultimate，并包含 Visual C++ 组件。
...
**现象**: 使用非v120工具集编译的 MT3.exe 在启动时崩溃

**原因**:
- MT3项目仅支持Visual Studio 2013 (v120工具集)
- 使用其他版本Visual Studio编译会导致二进制不兼容
- Release优化可能破坏了某些代码逻辑

**解决方案**:
使用Visual Studio 2013 (v120工具集)编译：
```batch
...
| 文件 | 预期大小 | 工具集 |
|------|---------|-------|
| platform.lib | ~4.8 MB | v120 |
| cauthc.lib | ~23 MB | v120 |
| engine.lib | ~37 MB | v120 |
| libcocos2d.dll | ~2.2 MB | v120 |
| libCocosDenshion.dll | ~45 KB | v120 |
| MT3.exe | ~8.4 MB | v120 Release |
